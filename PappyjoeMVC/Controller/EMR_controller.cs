...
namespace PappyjoeMVC.Controller
{
    public class EMR_controller
    {
        ...
        {
            DataTable dtb = _selectedvalue.Fill_allergygrid();
            return dtb;
        }
        public DataTable check_allergy(string note)
        {
            DataTable dt = _selectedvalue.check_allergy(note);
            return dt;
        }
        public void save_allergy(string _note)
        {
            _selectedvalue.save_allergy(_note);
        }
        public void update_allergy(string id_notes, string _note)
        {
            _selectedvalue.update_allergy(id_notes, _note);
        }
        public void delete_allergy(string id_notes)
        {
            _selectedvalue.delete_allergy(id_notes);
        }
        public DataTable search_allergy(string text)
        {
            DataTable dt2 = _selectedvalue.search_allergy(text);
            return dt2;
        }
    }
}
