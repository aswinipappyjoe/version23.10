...
namespace PappyjoeMVC.Controller
{
    public class Staff_controller
    {
        ...
        //Communication_model c/*m*/odel = new Communication_model();
        public DataTable LoadStaff()
        {
            DataTable dt = _selectedvalue.LoadStaff();
            return dt;
        }
        public int update_doctor_availability(string id)
        {
          int dt=  _selectedvalue.update_doctor_availability(id);
            return dt;
        }
        public int Save_doctor_availability(string id)
        {
            int dt = _selectedvalue.Save_doctor_availability(id);
            return dt;
        }
        ...
        public void save_staff(string _sname, string _mobile, string _email, string _registratn, string _color, string _status, string _stafftype,  string _confirmpswd, string fee, string f_fee, string period)
        {
            //GetSatffData(_selectedvalue);
            int i = _selectedvalue.Save_Staff( _sname,  _mobile,  _email,  _registratn,  _color,  _status,  _stafftype,    _confirmpswd,fee,f_fee,period);
        }
        public DataTable get_dr_maxid()
        {
            DataTable dtb = _selectedvalue.get_dr_maxid();
            return dtb;
        }
        public void save_login(string _email, string _confirmpswd, string _stafftype, string id)
        {
            _selectedvalue.save_login(_email, _confirmpswd, _stafftype, id);
        }
