...
namespace PappyjoeMVC.Controller
{
    public class Booking_controller
    {
        ...
        public void Save_patient(string PatientName, string Patient_Id, string aadhar_id, string gender, string date_of_birth, string age,string age2,string days, string blood_group, string family,  string primary_mobile_number, string secondary_mobile_number, string landline_number, string email_address, string street_address, string locality, string city, string pincode,  string refferedby, string Opticket, string Visited, string doctorname, string Occupation,string nationality,string passportno)
        {
            pmodel. Save( PatientName,  Patient_Id,  aadhar_id,  gender,  date_of_birth,  age,age2,days,  blood_group,  family,    primary_mobile_number,  secondary_mobile_number,  landline_number,  email_address,  street_address,  locality,  city,  pincode,    refferedby,  Opticket,  Visited,  doctorname,  Occupation,nationality,passportno,"","","","","","");
        }
        public void save_group(string pat_id, string medical)
        {
            pmodel.save_group(pat_id, medical);
        }
        ...
        {
            DataTable dt3 = addmodel.search_procedure(proc);
            return dt3;
        }
        public DataTable Get_procedure(string proc)
        {
            DataTable dt3 = addmodel.Get_procedure(proc);
            return dt3;
        }
        public DataTable get_DocRemsmsTime()
        {
            DataTable dt = addmodel.get_DocRemsmsTime();
            return dt;
        }
        public DataTable getDocAppntmnt(string drid,string date1,string date2)
        {
            DataTable dt = addmodel.getDocAppntmnt(drid,date1,date2);
            return dt;
        }
        public void insert_dataToDocNotif(string drid, string webres)
        {
            addmodel.insert_dataToDocNotif(drid, webres);
        }
        ...
        public string EditDocRem(string User, string password, string RID, string Mobile_Number, string Message, string SID, string Sname, string scheduledDate, string systemdate)
        {
             string val= s.SendSMS(User, password, RID, Mobile_Number, Message, SID, Sname, scheduledDate, systemdate);
            return val;
        }
        public string DeleteDocRem(string User, string password, string RID)
        {
            string val = s.SendSMS(User, password, RID);
            return val;
        }
        
        public DataTable get_docNotifInfor(string drid)
        {
            DataTable dt = addmodel.get_docNotifInfor(drid);
            return dt;
        }
        public DataTable getPatRemTime()
        {
            DataTable dt = addmodel.getPatRemTime();
            return dt;
        }
        public void updateWebresp(string drid, string webresp, string date)
        {
            addmodel.updateWebresp(drid, webresp, date);
        }
        public void insert_DocId(string drid)
        {
            addmodel.insert_DocId(drid);
        }
