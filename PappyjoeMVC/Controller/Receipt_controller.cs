...
namespace PappyjoeMVC.Controller
{
    public class Receipt_controller
    {
        ...
        {
            DataTable Payment = Model.get_paymentDate_load(patient_id);
            return Payment;
        }
        public DataTable get_paymentDate_count(string patient_id, int count)
        {
            DataTable Payment = Model.get_paymentDate_count(patient_id, count);
            return Payment;
        }
        ...
        {
            DataTable dtb = Model.payment_details_re(date, patient_id, reve, proname);
            return dtb;
        }
        public void update_refund(string status, string amount, string rece, string pro)
        {
            Model.update_refund(status, amount, rece, pro);
        }
        public DataTable get_patientDetails(string patient_id)
        {
            DataTable dtb = Model.get_patientDetails(patient_id);
            return dtb;
        }
    }
}
