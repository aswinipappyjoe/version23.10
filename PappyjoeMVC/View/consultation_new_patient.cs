...
namespace PappyjoeMVC.View
{
    public partial class consultation_new_patient : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                if (txtmobile.Text == "" || txtmobile.Text.Length <8 || txtmobile.Text.Length >13)
                {
                    MessageBox.Show("Enter the valid mobile number.. !!", "Invalid. ", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                else
                {
                    ...
                    if (i > 0)
                    {
                        ...
                        try
                        {
                            ...
                            loadptid();
                            txtmobile.Text = "";
                            var form2 = new Fasttrack_window_New(txtpatname.Text, id);// Consultation(txtpatname.Text, id);
                            form2.Closed += (sender1, args) => this.Close();
                            this.Close();
                        }
                        catch
                        {
                            var form2 = new Fast_Track(txtpatname.Text, id);// Consultation(txtpatname.Text, id);
                            form2.Closed += (sender1, args) => this.Close();
                            this.Close();
                        }
                    }
