    ...
    public partial class Consultation : Form
    {
        ...
        private void txt_Pt_search_TextChanged(object sender, EventArgs e)
        {
            if (flag == false)
            {
                ...
                {
                    // lbPatient.Show();
                    lbPatient.Location = new Point(45, 84);// txt_Pt_search.Location.X, 110);
                ...
                {
                    DataTable dtdr = this.ctrlr.srch_patient(txt_Pt_search.Text);
                    lbPatient.DataSource = dtdr;
                    lbPatient.DisplayMember = "patient";
                    lbPatient.ValueMember = "id";
                }
        ...
        private void lbPatient_MouseClick(object sender, MouseEventArgs e)
        {
            if (lbPatient.SelectedItems.Count > 0)
            {
                ...
                if (patient.Rows.Count > 0)
                {
                    ...
                    DataTable dtb = this.ctrlr.get_followup(patient_id);
                    if (dtb.Rows.Count > 0)
                    {
                        
                        cmbdoctor.Text = dtb.Rows[0]["doctorname"].ToString();
                        DataTable dt_doctor_id = this.ctrlr.get_doctorname(cmbdoctor.SelectedValue.ToString());
                        txt_visited_date.Visible = true; label4.Visible = true;
                        todate = DateTime.Now.Date;
                        txt_visited_date.Text = dtb.Rows[0]["visited_date"].ToString();
                        f_days=Convert.ToInt32(dt_doctor_id.Rows[0]["followup_period"].ToString());
                        DataTable dt_last = this.ctrlr.get_last_paid(patient_id);
                        if(dt_last.Rows.Count>0)//payment_status <>'Consultation'
                        {
...

                            {
                                ...
                                txt_consultation.Text = "FOLLOWUP";
                                chk_followup.Visible = true;
                                lb_followup.Visible = true;
                                txt_fee.Text = dt_doctor_id.Rows[0]["followup_fee"].ToString();
                            ...
                            else
                            {
                                ...
                                txt_consultation.Text = "CONSULTATION";
                                payment_status = "Consultation";
                                if (cmbdoctor.Text != "")
                                {
                                    DataTable dt_doctorid = this.ctrlr.get_doctorname(cmbdoctor.SelectedValue.ToString());
                                    if (dt_doctorid.Rows.Count > 0)
                                    {
                                        txt_fee.Text = dt_doctorid.Rows[0]["fee"].ToString();
                                    }
                                    else
                                    {
                                        txt_fee.Text = "0.00";
                    ...
                    {
                        txt_visited_date.Visible = false; label4.Visible = false;
                        lb_followup.Visible = false; chk_followup.Visible = false;
                        txt_consultation.Text = "CONSULTATION";
                        payment_status = "Consultation";
                        if (cmbdoctor.Text!="")
                        {
                            ...
                            {
                                txt_fee.Text = dt_doctor_id.Rows[0]["fee"].ToString();
                            }
                            else
                            {
                                txt_fee.Text = "0.00";
        ...
        private void txt_procedure_Click(object sender, EventArgs e)
        {
            ...
            if (txt_procedure.Text != "")
            {
                ...
                lst_procedure.Location = new Point(45, 263);// txt_procedure.Location.X, 210);
        ...
        private void lst_procedure_MouseClick(object sender, MouseEventArgs e)
        {
            if (lst_procedure.SelectedItems.Count > 0)
            {
                ...
                if (procedure.Rows.Count > 0)
                {
                    ...
                    //if(txt_procedure.Text== str)
                    //{
                    //    DataTable dt_doctor_id = this.ctrlr.get_doctorname(cmbdoctor.SelectedValue.ToString());
                    //    if(dt_doctor_id.Rows.Count>0)
                    //    {
                    //        txt_cost.Text = dt_doctor_id.Rows[0]["fee"].ToString();
                    //    }
                    //}
                    //else
                    //{
        ...
        private void btnReport_Click(object sender, EventArgs e)
        {
            ...
            {
                if ((txt_consultation.Text != "")&& (txt_consultation.Text == "CONSULTATION"))
                {
                    if (Convert.ToDecimal(txt_fee.Text) <= 0)
                    {
                ...
                using (MySqlConnection con = new MySqlConnection(cs))
                {
                    ...
                    try
                    {
                        if (dataGridView_drugnew.Rows.Count > 0)
                        {
                            prescription_check(con, trans);// Check Inventory Item
                            this.ctrlr.save_prescriptionMain(patient_id, d_id, Prescription_bill_status, txt_remarks.Text, con, trans);
                            string dt = this.ctrlr.max_presid(con, trans);
                            if (dt != "")
                            {
                            ...
                            for (int i = 0; i < count; i++)
                            {
                                ...
                                this.ctrlr.save_prescription(presid, patient_id, cmbdoctor.Text, d_id.ToString(), dataGridView_drugnew["drugname", i].Value.ToString(), dataGridView_drugnew["strength", i].Value.ToString(), dataGridView_drugnew["strengthgr", i].Value.ToString(), dataGridView_drugnew["duration", i].Value.ToString(), dataGridView_drugnew["period", i].Value.ToString(), dataGridView_drugnew["morning", i].Value.ToString(), dataGridView_drugnew["noon", i].Value.ToString(), dataGridView_drugnew["night", i].Value.ToString(), dataGridView_drugnew["food", i].Value.ToString(), dataGridView_drugnew["instruction", i].Value.ToString(), dataGridView_drugnew["drugtype", i].Value.ToString(), strstatus, dataGridView_drugnew["drugid", i].Value.ToString(), con, trans);
                            }
                        }
                        //completed id
                        ...
                        {
                            this.ctrlr.save_completedid(patient_id, con, trans);
                            string dt_CMain = this.ctrlr.max_completedid(con, trans);
                            try
                            {
                                if (dt_CMain == "")
                                {
                            ...
                            {
                                this.ctrlr.save_completed_details(j1, patient_id, dgv_treatment.Rows[i].Cells["tid"].Value.ToString(), dgv_treatment.Rows[i].Cells["Procedure"].Value.ToString(), dgv_treatment.Rows[i].Cells["Cost"].Value.ToString(), dgv_treatment.Rows[i].Cells["Totals"].Value.ToString(), dgv_treatment.Rows[i].Cells["Note"].Value.ToString(), cmbdoctor.SelectedValue.ToString(), con, trans);
                            }
                            string dt_Compl_proce = this.ctrlr.max_completeProcedure(con, trans);
                            try
                            {
                                if (dt_Compl_proce == "")
                                {
                        ...
                        {
                            this.ctrlr.update_review(dtp_nextreview.Value.ToString("yyyy-MM-dd HH:mm"), j1, con, trans);
                            this.ctrlr.update_prescription_review(dtp_nextreview.Value.ToString("yyyy-MM-dd HH:mm"), presid, con, trans);
                            DataTable dt_review = this.ctrlr.get_reviewId(patient_id, dtp_nextreview.Value.ToString("yyyy-MM-dd HH:mm"), con, trans);
                            if (dt_review.Rows.Count == 0)
                            {
                                this.ctrlr.save_review(dtp_nextreview.Value.ToString("yyyy-MM-dd HH:mm"), patient_id, con, trans);
                            }
                            this.ctrlr.save_appointment(dtp_nextreview.Value.ToString("yyyy-MM-dd HH:mm"), patient_id, txt_Pt_search.Text, cmbdoctor.SelectedValue.ToString(), patient_mobile, cmbdoctor.Text.ToString(), con, trans);
                        }
                        else
                        {
                            this.ctrlr.update_prescription_review_NO(dtp_nextreview.Value.ToString("yyyy-MM-dd HH:mm"), j1, con, trans);
                            this.ctrlr.update_prescription_review_NO(dtp_nextreview.Value.ToString("yyyy-MM-dd HH:mm"), presid, con, trans);
                        }
                        //followup
                        this.ctrlr.save_followup(patient_id, txt_Pt_search.Text, DateTime.Now.Date.ToString("yyyy-MM-dd"), DateTime.Now.Date.ToString("yyyy-MM-dd"), cmbdoctor.Text, txt_fee.Text, payment_status, con, trans);
                        string invoice = "";
                        ...
                        if (dgv_treatment.Rows.Count == 0)
                        {
                            if (txt_consultation.Text != "" && Convert.ToDecimal(txt_fee.Text) > 0)
                            {
                                this.ctrlr.save_invoice_main(patient_id, txt_Pt_search.Text, invoice, con, trans);///savin invoice no to invoice_main table
                                string dt1 = this.ctrlr.get_invoiceMain_maxid(con, trans);
                                //long Invoice_main_id = 0;
                                try
                                {
                                    if (dt1 == "")
                                ...
                                this.ctrlr.save_invoice_details(invoice, txt_Pt_search.Text, patient_id, "0", txt_consultation.Text, txt_fee.Text, "0", cmbdoctor.SelectedValue.ToString(), Invoice_main_id, 0, con, trans);
                                string invoauto = this.ctrlr.get_invoicenumber(con, trans);
                                int invoautoup = int.Parse(invoauto) + 1;
                                this.ctrlr.update_invnumber(invoautoup, con, trans);
                            }
                        }
                        else
                        {
                            ...
                            for (int i = 0; i < dgv_treatment.Rows.Count; i++)
                            {
                                this.ctrlr.save_invoice_details(invoice, txt_Pt_search.Text, patient_id, dgv_treatment.Rows[i].Cells["tid"].Value.ToString(), dgv_treatment.Rows[i].Cells["Procedure"].Value.ToString(), dgv_treatment.Rows[i].Cells["Cost"].Value.ToString(), "0", cmbdoctor.SelectedValue.ToString(), Invoice_main_id, completed_procedures_id, con, trans);
                            }
                            string invoauto = this.ctrlr.get_invoicenumber(con, trans);
                            int invoautoup = int.Parse(invoauto) + 1;
                            this.ctrlr.update_invnumber(invoautoup, con, trans);
                        ...
                        if (dgv_treatment.Rows.Count == 0)
                        {
                            if (txt_consultation.Text != "" && Convert.ToDecimal(txt_fee.Text) >= 0)//jinu >= change
                            {
                                DataTable rec_receipt = this.ctrlr.receipt_number(con, trans);
                                if (rec_receipt.Rows.Count > 0)
                                {
                                    receipt = rec_receipt.Rows[0]["receipt_prefix"].ToString() + rec_receipt.Rows[0]["receipt_number"].ToString();
                                }
                                DataTable cmd22 = this.ctrlr.Get_Advance(patient_id, con, trans);
                                decimal advance = 0;
                                ...
                                this.ctrlr.save_receipt(receipt, advance, txt_fee.Text, invoice, txt_consultation.Text, patient_id, cmbdoctor.SelectedValue.ToString(), txt_fee.Text, txt_fee.Text, txt_Pt_search.Text, Invoice_main_id, con, trans);

                                //for (int i = 0; i < dgv_treatment.Rows.Count; i++)
                                //{
                                //    this.ctrlr.save_receipt(receipt, advance, dgv_treatment.Rows[i].Cells["Totals"].Value.ToString(), invoice, dgv_treatment.Rows[i].Cells["Procedure"].Value.ToString(), patient_id, cmbdoctor.SelectedValue.ToString(), dgv_treatment.Rows[i].Cells["Totals"].Value.ToString(), dgv_treatment.Rows[i].Cells["Cost"].Value.ToString(), txt_Pt_search.Text, Invoice_main_id);
                                //}
                                string rec = this.ctrlr.receipt_autoid(con, trans);
                                int receip = int.Parse(rec) + 1;
                                this.ctrlr.update_receiptAutoid(receip, con, trans);
                            }
                        }
                        else
                        {
                            ...
                            for (int i = 0; i < dgv_treatment.Rows.Count; i++)
                            {
                                this.ctrlr.save_receipt(receipt, advance, dgv_treatment.Rows[i].Cells["Totals"].Value.ToString(), invoice, dgv_treatment.Rows[i].Cells["Procedure"].Value.ToString(), patient_id, cmbdoctor.SelectedValue.ToString(), dgv_treatment.Rows[i].Cells["Totals"].Value.ToString(), dgv_treatment.Rows[i].Cells["Cost"].Value.ToString(), txt_Pt_search.Text, Invoice_main_id, con, trans);
                            }
                            string rec = this.ctrlr.receipt_autoid(con, trans);
                            int receip = int.Parse(rec) + 1;
                            this.ctrlr.update_receiptAutoid(receip, con, trans);
                        ...
                        //this.ctrlr.save_log(doctor_id, "Consultation", "logged User Add Consultation", dt11, Convert.ToString(Timeonly.ToString("hh:mm tt")), "Add", con, trans);
                        trans.Commit();
                        con.Close();
                        ...
                        txt_fee.Visible = false;
                        txt_consultation.Visible = false;
                        label4.Visible = false;
                        txt_visited_date.Visible = false;
        ...
        public void printboth(int Prescription_id)
        {
            try
            {
                    ...
                    if (includelogo == "1")
                    {
                        if (logo != null || logo_name != "")
                        {
                            ...
                            if (File.Exists(Appath + "\\" + logo_name))
                            {
                                ...
                                sWrite.WriteLine("<td width='870px' align='center' height='25px'><FONT  COLOR=black  face='Segoe UI' SIZE=4><b>&nbsp;" + clinicn + "</font> <br><FONT  COLOR=black  face='Segoe UI' SIZE=2>&nbsp;" + streetaddress + "<br>Phone Number:&nbsp;" + contact_no + " </b></td></tr>");
                ...
                sWrite.WriteLine("<td align='right' ><FONT COLOR=black FACE='Geneva, Arial' SIZE=2>Doctor Name:&nbsp;" + cmbdoctor.Text + " </font></td>");
                ...
                if (dt_cf.Rows.Count > 0)
                {
                    ...
                    sWrite.WriteLine("<td align='right' ><FONT COLOR=black FACE='Geneva, Arial' SIZE=2> <FONT COLOR=black><b>Date :</b> </FONT>" + Convert.ToDateTime(tdate).ToString("dd MMM yyyy") + "</font></td>");
                    sWrite.WriteLine("</tr>");
                    sWrite.WriteLine("</table>");
                    sWrite.WriteLine("<br>");
                    sWrite.WriteLine("<table   align='center' style='width:700px;border: 1px ;border-collapse: collapse;' >");
                    ...
                    for (int i = 0; i < dt_payment.Rows.Count; i++)
                    {
                        ...
                        sWrite.WriteLine("<td align='right'   width='99'><FONT COLOR=black FACE='Geneva, Arial' SIZE=2>&nbsp;" + decimal.Parse(dt_payment.Rows[i]["amount_paid"].ToString()) + " </font></td>");
                        sWrite.WriteLine("</tr>");
                    ...
                    sWrite.WriteLine("<td></td>");
                    sWrite.WriteLine("<td></td>");
                    sWrite.WriteLine("<td align='right' ><FONT COLOR=black FACE='Geneva, Arial' SIZE=3><b> INR " + decimal.Parse(total.ToString()) + " </b></font></td>");
                    sWrite.WriteLine("</tr>");
            ...
            try
            {
                ...
                if (dt_cf.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt_payment.Rows.Count; i++)
                    {
                        ...
                        sWrite.WriteLine("<td align='center'   width='147'><FONT COLOR=black FACE='Geneva, Arial' SIZE=2>&nbsp;" + dt_payment.Rows[i]["invoice_no"].ToString() + " </font></td>");
                        sWrite.WriteLine("<td align='left'   width='259'><FONT COLOR=black FACE='Geneva, Arial' SIZE=2>&nbsp;" + dt_payment.Rows[i]["procedure_name"].ToString() + " </font></td>");
                        sWrite.WriteLine("<td align='right'   width='99'><FONT COLOR=black FACE='Geneva, Arial' SIZE=2>&nbsp;" + Convert.ToDecimal( dt_payment.Rows[i]["amount_paid"]).ToString("0.00") + " </font></td>");
                        sWrite.WriteLine("</tr>");
                        total = total + decimal.Parse(dt_payment.Rows[i]["amount_paid"].ToString());
                    }
                    sWrite.WriteLine("<tr><td align='left' colspan=5><hr/></td></tr>");
                    ...
                    sWrite.WriteLine("<td></td>");
                    sWrite.WriteLine("<td></td>");
                    sWrite.WriteLine("<td align='right' ><FONT COLOR=black FACE='Geneva, Arial' SIZE=3><b> INR " + total.ToString("0.00") + " </b></font></td>");
                    sWrite.WriteLine("</tr>");
                    sWrite.WriteLine("<tr><td align='right' colspan=5><FONT COLOR=black FACE='Geneva, Arial' SIZE=2>(<i>" + NumWordsWrapper(double.Parse(total.ToString())) + "</i>)</fount> </td></tr>");
                    sWrite.WriteLine("</table>");
                }
        ...
        public void printprescriptionhtml(int Prescription_id)
        {
            ...
            int Dexist = 0;
            string sexage = ""; string age = "";
            string address = ""; string days = "";
            address = "";
            string strNote = "";
            string strreview = "NO";
            string strreview_date = "";
            ...
            if (dt1.Rows.Count > 0)
            {
                ...
                sWrite.WriteLine("<tr>");
                sWrite.WriteLine("<td align='left' ><FONT COLOR=black FACE='Geneva, Segoe UI' SIZE=2>Phone Number:&nbsp;" + dt1.Rows[0]["primary_mobile_number"].ToString() + " </font></td>");
                sWrite.WriteLine(" </tr>");
                if (dt1.Rows[0]["email_address"].ToString() != "")
                {
                    sWrite.WriteLine("<tr>");
        ...
        public void prescription_check(MySqlConnection con,MySqlTransaction trans)
        {
            try
            {
                if (dataGridView_drugnew.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        DataTable dt4 = this.ctrlr.get_invid(dataGridView_drugnew["drugid", i].Value.ToString(),con,trans);
                        if (dt4.Rows.Count > 0)
                        {
                ...
                if (lbPatient.SelectedItems.Count > 0)
                {
                    ...
                    if (patient.Rows.Count > 0)
                    {
                        ...
                        if (dtb.Rows.Count > 0)
                        {
                            ...
                            {
                                if (dt_last.Rows[0]["payment_date"].ToString() != "")
                                {
                                    paid_date = Convert.ToDateTime(dt_last.Rows[0]["payment_date"].ToString());
                                    TimeSpan t = todate - paid_date;
                                    days = t.TotalDays;
                                ...
                                {
                                    lb_followup.Visible = true;
                                    txt_fee.Text =Convert.ToDecimal(  dt_doctor_id.Rows[0]["followup_fee"].ToString()).ToString("0.00");
                                    chk_followup.Visible = true;
                                }
                                else
                                {
                                    ...
                                    if (cmbdoctor.Text != "")
                                    {
                                        ...
                                        if (dt_doctorid.Rows.Count > 0)
                                        {
                                            txt_fee.Text = Convert.ToDecimal(dt_doctorid.Rows[0]["fee"].ToString()).ToString("0.00");
                                        }
                                        else
                                        {
                                            txt_fee.Text = "0.00";
                        ...
                        else
                        {
                            ...
                            if (cmbdoctor.Text != "")
                            {
                                ...
                                if (dt_doctor_id.Rows.Count > 0)
                                {
                                    txt_fee.Text =Convert.ToDecimal( dt_doctor_id.Rows[0]["fee"].ToString()).ToString("0.00");
                                }
                                else
                                {
                                    txt_fee.Text = "0.00";
                ...
                if (drugspanel.Visible == true)
                {
                    ...
                    if (yesno == DialogResult.Yes)
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            txtStrengthno.Text = dt.Rows[0]["strength"].ToString();
                            strengthcombo.Text = dt.Rows[0]["strength_gr"].ToString();
        ...
        private void btn_addtrtmnt_Click(object sender, EventArgs e)
        {
            ...
            if (txt_procedure.Text != "" && txtQuantity.Text != "")
            {
                ...
                //if (txt_procedure.Text== str)
                //{
                //    //if(lb_followup.Visible==true || )
                //}
                lb_total.Show();
                Decimal total = 0;
                dgv_treatment.Rows.Add(txtID.Text, txt_procedure.Text, txtQuantity.Text, txt_cost.Text, txtTotal.Text, txt_instruction.Text, PappyjoeMVC.Properties.Resources.deleteicon);
                for (int i = 0; i < dgv_treatment.Rows.Count; i++)
                {
                    total = total + Convert.ToDecimal(dgv_treatment.Rows[i].Cells["Totals"].Value.ToString());
                }
                lb_total.Text = total.ToString();
                txtQuantity.Text = "";
                txtTotal.Text = "";
                txt_instruction.Text = "";
                txtID.Text = ""; txt_cost.Text = ""; txt_procedure.Text = "";
            }
        }
                ...
                else
                {
                    ...
                    if (newptid != "")
                    {
                        ...
                        if (listindex > 0)
                        {
                            ...
                            dr_id = cmbdoctor.SelectedValue.ToString();
                        }
                        DataTable dt_doctor_id = this.ctrlr.get_doctorname(dr_id);
                        //string str = this.ctrlr.get_consultation();
                        //if(str!="")
                        //{
        ...
        private void lst_procedure_MouseCaptureChanged(object sender, EventArgs e)
        {
        ...
        private void chk_followup_CheckedChanged(object sender, EventArgs e)
        {
            if(chk_followup.Checked==true)
            {
                ...
                if (cmbdoctor.Text!="")
                {
                    string dr_id = cmbdoctor.SelectedValue.ToString();
                    DataTable dt_doctor_id = this.ctrlr.get_doctorname(dr_id);
                    if(dt_doctor_id.Rows.Count>0)
                    {
                        txt_fee.Text =Convert.ToDecimal( dt_doctor_id.Rows[0]["followup_fee"].ToString()).ToString("0.00");
        ...
        private void cmbdoctor_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                DataTable dt_doctor_id = this.ctrlr.get_doctorname(cmbdoctor.SelectedValue.ToString());
                DataTable dt_patient_doctor = this.ctrlr.get_patient_doctor(cmbdoctor.Text, patient_id);
                ...
                if (newptid == "")
                {
                    if (patient_id != "")
                    {
                        ...
                        {
                            if (txt_consultation.Text == "FOLLOWUP")
                            {
                                DialogResult yesno = MessageBox.Show("He/She is not a patient of Dr" + cmbdoctor.Text + " , Do you want to take an appointment with Dr" + cmbdoctor.Text + " ?", "Change Consultation", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
                                if (yesno == DialogResult.No)
                                {
                                    payment_status = "Followup";
                                }
                        ...
                        else
                        {
                            if (txt_consultation.Text != "")
                            {
                                ...
                                else if (txt_consultation.Text == "CONSULTATION")
                                {
                                    if (cmbdoctor.Text == dt_patient_doctor.Rows[0]["doctorname"].ToString())
                                    {
                                        txt_consultation.Text = "FOLLOWUP";
                                        txt_fee.Text = dt_doctor_id.Rows[0]["followup_fee"].ToString();
                                        payment_status = "Followup"; chk_followup.Visible = true; lb_followup.Visible = true;
