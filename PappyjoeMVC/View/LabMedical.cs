    ...
    public partial class LabMedical : Form
    {
        ...
        private void btnSaveunit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    DataTable dtExist = this.cntrl.tpMain_unit_exist(txtUnitadd.Text);
                    if (dtExist.Rows.Count>0)
                    {
                        MessageBox.Show("Unit already exist", "Duplication", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
        ...
        private void btnsavetmp_Click(object sender, EventArgs e)
        {
            try
            {
                if (btnsavetmp.Text == "SAVE")
                {
                    if (dgvtempitem.Rows.Count>0)
                    {
                        if (!string.IsNullOrWhiteSpace(txttemp.Text))
                        {
                            ...
                            for (int r = 0; r < dgvtempitem.Rows.Count; r++)
                            {
                                ...
                                //{
                                //    Normal = Normavalue.Rows[0][0].ToString();
                                //}
                                string Mnorm = ""; string Fnorm = "";
                                if (dgvtempitem.Rows[r].Cells[5].Value.ToString() != "")
                                {
                                    Mnorm = dgvtempitem.Rows[r].Cells[5].Value.ToString();
                                }
                                if (dgvtempitem.Rows[r].Cells[6].Value.ToString() != "")
                                {
                                    Fnorm = dgvtempitem.Rows[r].Cells[6].Value.ToString();
                                }
                                //string NormalValue = Mnorm + "," + Fnorm;
                                string NormalValue = "";
                                if (Mnorm != "" && Fnorm != "")
                                {
                                ...
                                this.cntrl.Insert_mediTemplate(Convert.ToInt32(id), Convert.ToInt32(dgvtempitem.Rows[r].Cells[7].Value.ToString()), dgvtempitem.Rows[r].Cells[8].Value.ToString()/*tests*/, dgvtempitem.Rows[r].Cells[9].Value.ToString(), dgvtempitem.Rows[r].Cells[4].Value, NormalValue);//
                            }
                            MessageBox.Show("Successfully Saved", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ...
                    {
                        MessageBox.Show("please add templates", "Data Missing", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
        ...
        private void btnedit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int r = 0; r < dgvtempitem.Rows.Count; r++)
                {
                    ...
                    this.cntrl.Insert_mediTemplate(Convert.ToInt32(Convert.ToInt32(txtId.Text)), Convert.ToInt32(dgvtempitem.Rows[r].Cells[7].Value.ToString()), dgvtempitem.Rows[r].Cells[8].Value.ToString()/*test*/, dgvtempitem.Rows[r].Cells[9].Value.ToString(), dgvtempitem.Rows[r].Cells[4].Value.ToString(), NormalValue.ToString() );//,
                }
                txtId.Clear();
                txttemp.Clear();
                DataTable dt= this.cntrl.tpMain_template();
