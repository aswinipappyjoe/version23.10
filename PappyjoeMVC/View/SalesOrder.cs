...
namespace PappyjoeMVC.View
{
    public partial class SalesOrder : Form
    {
        ...
        string FormName = "";
        public static int rowindex; public string doctor_id = "0";
        bool flagSup = false;
        public SalesOrder()
        {
            InitializeComponent();
        ...
        public void Load_master(DataTable dtb_SalesOrder_Master)
        {
            if (dtb_SalesOrder_Master.Rows.Count > 0)
            {
                ...
                Txt_TotalAmount.Text = Convert.ToDecimal( dtb_SalesOrder_Master.Rows[0]["TotalAmount"].ToString()).ToString("##.00");
                txt_Discount.Text = Convert.ToDecimal(dtb_SalesOrder_Master.Rows[0]["Discount_inr"].ToString()).ToString();
                txt_DiscAmount.Text = Convert.ToDecimal(dtb_SalesOrder_Master.Rows[0]["Discount_Amount"].ToString()).ToString("##.00");
                txt_GrandTotal.Text = Convert.ToDecimal(dtb_SalesOrder_Master.Rows[0]["GrandTotal"].ToString()).ToString("##.00");
            }
        }
        public void load_items(DataTable dtb_SalesOrder)
        {
            ...
            if (dtb_SalesOrder.Rows.Count > 0)
            {
                ...
                {
                    string item_code = this.cntrl.get_itemcode(dtb_SalesOrder.Rows[i]["ItemCode"].ToString());
                    dgv_SalesItem.Rows.Add();
                    dgv_SalesItem.Rows[i].Cells["id"].Value= dtb_SalesOrder.Rows[i]["ItemCode"].ToString();
                    dgv_SalesItem.Rows[i].Cells["colItemCode"].Value = item_code;
                    dgv_SalesItem.Rows[i].Cells["colDiscription"].Value = dtb_SalesOrder.Rows[i]["Discription"].ToString();
                    dgv_SalesItem.Rows[i].Cells["col_strength"].Value = dtb_SalesOrder.Rows[i]["Strength"].ToString();
                    dgv_SalesItem.Rows[i].Cells["col_unit"].Value = dtb_SalesOrder.Rows[i]["Unit"].ToString();
                    dgv_SalesItem.Rows[i].Cells["ColQty"].Value = dtb_SalesOrder.Rows[i]["Qty"].ToString();
                    dgv_SalesItem.Rows[i].Cells["colUnitcost"].Value =Convert.ToDecimal( dtb_SalesOrder.Rows[i]["Cost"].ToString()).ToString("00.00");
                    dgv_SalesItem.Rows[i].Cells["colAmount"].Value = Convert.ToDecimal(dtb_SalesOrder.Rows[i]["TotalAmount"].ToString()).ToString("00.00");
        ...
        {
            DataTable strgth = this.cntrl.strenth(itemid);
            string unit1 = "", unit2 = "";
            ...
            if (dtb.Rows.Count > 0)
            {
                ...
                if (unit1 != "")
                {
                    cmb_Unit.Items.Add(unit1); cmb_Unit.SelectedIndex = 0;
                }
                ...
                if (strgth.Rows.Count > 0)
                {
                    txt_strength.Text = strgth.Rows[0]["strength"].ToString();
                }
        ...
        private void btn_AddtoGrid_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrWhiteSpace(itemid) && !string.IsNullOrWhiteSpace(txt_ItemCode.Text) && !string.IsNullOrWhiteSpace(txt_Discription.Text) && Convert.ToDecimal(txt_Qty.Text) > 0)
            {
                ...
                {
                    dgv_SalesItem.Rows.Add(itemid,txt_ItemCode.Text, txt_Discription.Text,txt_strength.Text,cmb_Unit.Text, txt_Qty.Text, txt_UnitCost.Text, txt_Amount.Text, PappyjoeMVC.Properties.Resources.editicon, PappyjoeMVC.Properties.Resources.deleteicon);
                }
                else
                {
                    if (rowindex >= 0)
                    {
                        ...
                        dgv_SalesItem.Rows[rowindex].Cells["col_strength"].Value = txt_strength.Text;
                        dgv_SalesItem.Rows[rowindex].Cells["col_unit"].Value = cmb_Unit.Text;
                        ...
                        btn_cancel.Visible = false; txt_ItemCode.Enabled = true;
                        btn_item_Choose.Enabled = true;
                    }
                }
                decimal d, totalamount=0, totaldisper=0, totaldis=0;
        ...
        private void dgv_SalesItem_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            Decimal TotalAmount = 0, DiscAmount=0;
            string itmCode = "";
            if (e.RowIndex >= 0)
            {
                ...
                if (dgv_SalesItem.CurrentCell.OwningColumn.Name == "colEdit")
                {
                    ...
                    txt_strength.Text= dgv_SalesItem.Rows[rowindex].Cells["col_strength"].Value.ToString();
                    ...
                    DataTable dtb = this.cntrl.get_salesrate(itemid);
                    load_unitcost(dtb);
                    cmb_Unit.Text= dgv_SalesItem.Rows[rowindex].Cells["col_unit"].Value.ToString();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            using (MySqlConnection con = new MySqlConnection(cs))
            {
                ...
                try
                {
                    ...
                    {
                        i = this.cntrl.save_master(txtDocumentNumber.Text, dtpDocumentDate.Value.ToString("yyyy-MM-dd"), txt_OrderNo.Text, DTP_OrderDate.Value.ToString(), txtPatient.Text, txtPatientID.Text, txt_Street.Text, txt_Locality.Text, txt_City.Text, txt_PhoneNo.Text, Txt_TotalAmount.Text, txt_Discount.Text, txt_DiscAmount.Text, txt_GrandTotal.Text, con,trans);
                        if (i > 0)
                        {
                            foreach (DataGridViewRow dr in dgv_SalesItem.Rows)
                            {
                                j = this.cntrl.save_items(txtDocumentNumber.Text, dtpDocumentDate.Value.ToString("yyyy-MM-dd"), dr.Cells["id"].Value.ToString(), dr.Cells["colDiscription"].Value.ToString(), dr.Cells["col_strength"].Value.ToString(), dr.Cells["col_unit"].Value.ToString(), dr.Cells["ColQty"].Value.ToString(), dr.Cells["colUnitcost"].Value.ToString(), dr.Cells["colAmount"].Value.ToString(), con,trans);
                            }
                        }
                    }
                    else
                    {
                        this.cntrl.delete_order(invnum_Edit,con,trans);
                        i = this.cntrl.save_master(txtDocumentNumber.Text, dtpDocumentDate.Value.ToString("yyyy-MM-dd"), txt_OrderNo.Text, DTP_OrderDate.Value.ToString(), txtPatient.Text, txtPatientID.Text, txt_Street.Text, txt_Locality.Text, txt_City.Text, txt_PhoneNo.Text, Txt_TotalAmount.Text, txt_Discount.Text, txt_DiscAmount.Text, txt_GrandTotal.Text, con,trans);
                        if (i > 0)
                        {
                            foreach (DataGridViewRow dr in dgv_SalesItem.Rows)
                            {
                                j = j = this.cntrl.save_items(txtDocumentNumber.Text, dtpDocumentDate.Value.ToString("yyyy-MM-dd"), dr.Cells["id"].Value.ToString(), dr.Cells["colDiscription"].Value.ToString(), dr.Cells["col_strength"].Value.ToString(), dr.Cells["col_unit"].Value.ToString(), dr.Cells["ColQty"].Value.ToString(), dr.Cells["colUnitcost"].Value.ToString(), dr.Cells["colAmount"].Value.ToString(), con,trans);
                            }
                        }
                        btnSave.Text = "SAVE";
                    }
                    ...
                    if (i > 0 && j > 0)
                    {
                        ...
                        if (btnSave.Text == "SAVE")
                        {
                            this.cntrl.save_log(doctor_id, "Sales Order", " Adds Sales Order", dt, Convert.ToString(Timeonly.ToString("hh:mm tt")), "Add", txtDocumentNumber.Text);
       ...
       MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                        ...
                        {
                            this.cntrl.save_log(doctor_id, "Sales Order", " Edit Sales Order", dt, Convert.ToString(Timeonly.ToString("hh:mm tt")), "Edit", txtDocumentNumber.Text);
                            DialogResult res = MessageBox.Show("Data updated Successfully,Do you want to print ?", "",
       MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                            if (res == DialogResult.Yes)
                            {
                                printhtml();
        ...
        public void printhtml()
        {
            if(dgv_SalesItem.Rows.Count>0)
            {
                ...
                StreamWriter sWrite = new StreamWriter(Apppath + "\\salesOrder_print.html");
                ...
                sWrite.WriteLine("<td colspan=4  align='right' ><FONT COLOR=black FACE='Segoe UI' SIZE=2>Sales Order No:" + txtDocumentNumber.Text + " </font></td>");
                sWrite.WriteLine("</tr>");
                sWrite.WriteLine("<tr><td align='left' colspan=7><hr/></td></tr>");
                sWrite.WriteLine("<tr>");
                sWrite.WriteLine("<td align='left'  width='90'><FONT COLOR=black FACE='Geneva,  Sego UI' SIZE=3><b>Item Code</b></font></td>");
                sWrite.WriteLine("<td align='left'  width='150'><FONT COLOR=black FACE='Geneva,  Sego UI' SIZE=3><b>Description</b></font></td>");
                sWrite.WriteLine("<td align='left'  width='50'><FONT COLOR=black FACE='Geneva,  Sego UI' SIZE=3><b>Strength</b></font></td>");
                sWrite.WriteLine("<td align='left'  width='50'><FONT COLOR=black FACE='Geneva,  Sego UI' SIZE=3><b>Unit</b></font></td>");
                sWrite.WriteLine("<td align='right'  width='65'><FONT COLOR=black FACE='Geneva,  Sego UI' SIZE=3><b>Quantity</b></font></td>");
                sWrite.WriteLine("<td align='right'  width='110'><FONT COLOR=black FACE='Geneva,  Sego UI' SIZE=3><b>Cost</b></font></td>");
                sWrite.WriteLine("<td align='right'  width='120'><FONT COLOR=black FACE='Geneva,  Sego UI' SIZE=3><b>Total Amount</b></font></td>");
                sWrite.WriteLine("</tr>");
                sWrite.WriteLine("<tr><td align='left' colspan=7><hr/></td></tr>");
                for (int i = 0; i < dgv_SalesItem.Rows.Count; i++)
                {
                    ...
                    sWrite.WriteLine("<td align='left'><FONT COLOR=black FACE='Geneva,  Sego UI' SIZE=2>" + dgv_SalesItem.Rows[i].Cells["col_strength"].Value.ToString() + "</font></td>");
                    sWrite.WriteLine("<td align='left'><FONT COLOR=black FACE='Geneva,  Sego UI' SIZE=2>" + dgv_SalesItem.Rows[i].Cells["col_unit"].Value.ToString() + "</font></td>");
                ...
                sWrite.WriteLine("<tr><td align='left'  colspan=7><hr/></td></tr>");
                sWrite.WriteLine("<tr>");
                sWrite.WriteLine("<td align='right'  colspan=6 ><FONT COLOR=black FACE='Geneva,  Sego UI' SIZE=2> Total Items :</font></td>");
                sWrite.WriteLine("<td colspan='7' align='right'><FONT COLOR=black FACE='Segoe UI'   SIZE=2>   " + txt_totalItems.Text + "</font></right></td>");
                sWrite.WriteLine("</tr>");
                sWrite.WriteLine("<tr>");
                sWrite.WriteLine("<td align='right'  colspan=6 ><FONT COLOR=black FACE='Geneva,  Sego UI' SIZE=2>Total Amount :  </font></td>");
                sWrite.WriteLine("<td colspan='7' align='right'><FONT COLOR=black FACE='Segoe UI'   SIZE=2>  INR  " + decimal.Parse(Txt_TotalAmount.Text).ToString("##0.00") + "</font></right></td>");
                sWrite.WriteLine("</tr>");
                sWrite.WriteLine("<tr>");
                sWrite.WriteLine("<td align='right'  colspan=6 ><FONT COLOR=black FACE='Geneva,  Sego UI' SIZE=2>Discount(%) :  </font></td>");
                sWrite.WriteLine("<td colspan='7' align='right'><FONT COLOR=black FACE='Segoe UI'   SIZE=2>   " + txt_Discount.Text + "</font></right></td>");
                sWrite.WriteLine("</tr>");
                sWrite.WriteLine("<tr>"); 
                sWrite.WriteLine("<td align='right'  colspan=6 ><FONT COLOR=black FACE='Geneva,  Sego UI' SIZE=2>Discount Amount :   </font></td>");
                sWrite.WriteLine("<td colspan='7' align='right'><FONT COLOR=black FACE='Segoe UI'   SIZE=2>  INR " + txt_DiscAmount.Text + "</font></right></td>");
                sWrite.WriteLine("</tr>");
                sWrite.WriteLine("<tr>");
                sWrite.WriteLine("<td align='right'  colspan=6 ><FONT COLOR=black FACE='Geneva,  Sego UI' SIZE=2><b>Grand Total :  </b> </font></td>");
                sWrite.WriteLine("<td colspan='7' align='right'><FONT COLOR=black FACE='Segoe UI'   SIZE=3><b>  INR " + decimal.Parse(txt_GrandTotal.Text).ToString("##0.00") + "</b></font></right></td>");
                sWrite.WriteLine("</tr>");
                sWrite.WriteLine("</table>");
                ...
                sWrite.WriteLine("</html>");
                sWrite.Close();
                System.Diagnostics.Process.Start(Apppath + "\\salesOrder_print.html");
