    ...
    public partial class Expense : Form
    {
        ...
        public void Fill_dgv_credit(DataTable dtb)
        {
            if (dtb.Rows.Count > 0)
            {
                ...
                foreach (DataRow dr in dtb.Rows)
                {
                    ...
                    dgv_credit.Rows[row].Cells["c_tagname"].Value = dr["tag_name"].ToString();
                    row++;
                }
                AddSLNO();
        ...
        private void btnincomesubmit_Click(object sender, EventArgs e)
        {
            try
            {
                if (!string.IsNullOrWhiteSpace(txtamountincome.Text) && !string.IsNullOrWhiteSpace(textBoxnameofincome.Text) && !string.IsNullOrWhiteSpace(dateTimePickerincome.Value.ToString()))
                {
                    ...
                    {
                        int i = this.cntrl.submit_credit(dateTimePickerincome.Value.ToString("yyyy-MM-dd"), comboBoxincomacc.Text, textBoxdescincome.Text, textBoxnameofincome.Text, txtamountincome.Text,comboBoxtag.Text);//comboBoxtag.Text
                        if (i > 0)
                        {
                    ...
                    {
                        this.cntrl.update_dgv_credit(Incom_ID, dateTimePickerincome.Value.ToString("yyyy-MM-dd"), comboBoxincomacc.Text, textBoxdescincome.Text, txtamountincome.Text, textBoxnameofincome.Text,comboBoxtag.Text);//comboBoxtag.Text
        ...
        private void btnsubmit_Click(object sender, EventArgs e)
        {
            try
            {
                if (!string.IsNullOrWhiteSpace(textamount.Text))
                {
                    ...
                    {
                        int i = this.cntrl.submit_debit(dateTimePickerdate.Value.ToString("yyyy-MM-dd"), comboaccountname.Text, textBox_add_template.Text, textamount.Text, textperson.Text, combotagname.Text);//
                        if (i > 0)
                        {
                            MessageBox.Show("Successfully Saved !!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ...
                    {
                        int i = this.cntrl.update_dgv_debit(rowindex, dateTimePickerdate.Value.ToString("yyyy-MM-dd"), comboaccountname.Text, textBox_add_template.Text, textamount.Text, textperson.Text,combotagname.Text);//,combotagname.Text
                        if (i > 0)
                        {
        ...
        public void fill_dgv_debit(DataTable dtb)
        {
            if (dtb.Rows.Count > 0)
            {
                ...
                foreach (DataRow dr in dtb.Rows)
                {
                    ...
                    Dgv_Debit.Rows[row].Cells["ctagname"].Value = dr["tag_name"].ToString();
                    row++;
                }
                PutSLNO();
            }
        ...
        private void btnsearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //bahja.......
                if (check_Type.Checked && check_Account.Checked && checkTag.Checked)
                {
                    if (Grid_expense.Columns.Contains("Column"))
                    {
                        Grid_expense.Columns.Remove("Column");
                    }
                    DataTable dtb = this.cntrl.expense_search(comboSearch.Text, exp_Type, combotagsearch.Text, dateTimeEntrydate.Value.ToString("yyyy-MM-dd"), dateTimePickertodate.Value.ToString("yyyy-MM-dd"));
                    fill_search(dtb);
                }
                if (check_Type.Checked && !check_Account.Checked && !checkTag.Checked)
                {
                ...
                if (!check_Type.Checked && check_Account.Checked && checkTag.Checked)
                {
                    ...
                    DataTable dtb = this.cntrl.expense_search3(comboSearch.Text, combotagsearch.Text, dateTimeEntrydate.Value.ToString("yyyy-MM-dd"), dateTimePickertodate.Value.ToString("yyyy-MM-dd"));
                    fill_search2(dtb);
                }
                if (!check_Type.Checked && !check_Account.Checked && !checkTag.Checked)
                {
                ...
                if (!check_Type.Checked && check_Account.Checked && !checkTag.Checked)
                {
                    ...
                    DataTable dtb = this.cntrl.expense_search5(comboSearch.Text, dateTimeEntrydate.Value.ToString("yyyy-MM-dd"), dateTimePickertodate.Value.ToString("yyyy-MM-dd"));
                    fill_search2(dtb);
                }
                if (!check_Type.Checked && !check_Account.Checked && checkTag.Checked)
                {
                    ...
                    DataTable dtb = this.cntrl.expense_search6(combotagsearch.Text, dateTimeEntrydate.Value.ToString("yyyy-MM-dd"), dateTimePickertodate.Value.ToString("yyyy-MM-dd"));
                    fill_search2(dtb);
                }
                if (check_Type.Checked && !check_Account.Checked && checkTag.Checked)
                {
                    ...
                    DataTable dtb = this.cntrl.expense_search7(exp_Type, combotagsearch.Text, dateTimeEntrydate.Value.ToString("yyyy-MM-dd"), dateTimePickertodate.Value.ToString("yyyy-MM-dd"));
                    fill_search(dtb);
                }
                if (check_Type.Checked && check_Account.Checked && !checkTag.Checked)
                {
                    ...
                    DataTable dtb = this.cntrl.expense_search8(comboSearch.Text, exp_Type, dateTimeEntrydate.Value.ToString("yyyy-MM-dd"), dateTimePickertodate.Value.ToString("yyyy-MM-dd"));
                    fill_search(dtb);
                }
        ...
        private void linktagaddnewdebit_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            var form2 = new New_Debit_TagName();
            form2.ShowDialog();
            form2.Dispose();
        }

        private void linktagaddnewcredit_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            var form2 = new NewCredit_TagName();
            form2.ShowDialog();
            form2.Dispose();
        }

        private void combotagname_Click(object sender, EventArgs e)
        {
            try
            {
                DataTable dtag = this.cntrl.debit_tagname();
                combotagname.DisplayMember = "tag";
                combotagname.ValueMember = "id";
                combotagname.DataSource = dtag;
        ...
        private void comboBoxtag_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                DataTable dtag = this.cntrl.credit_tagname();
                comboBoxtag.DisplayMember = "tag";
                comboBoxtag.ValueMember = "id";
                comboBoxtag.DataSource = dtag;
        ...
        private void checkTag_checkedChanged(object sender, EventArgs e)
        {
            ...
            {
                combotagsearch.Enabled = true;
            }
            else
            {
                combotagsearch.Enabled = false;
            }
        }

        public void fill_combotagsearch_debit()
        {
            DataTable d_tag = this.cntrl.tag_expense_checked();
            combotagsearch.DisplayMember = "tag";
            combotagsearch.ValueMember = "id";
            combotagsearch.DataSource = d_tag;
        }
        public void fill_combotagsearch_credit()
        {
            DataTable d_tag = this.cntrl.tag_income_checked();
            combotagsearch.DisplayMember = "tag";
            combotagsearch.ValueMember = "id";
            combotagsearch.DataSource = d_tag;
        }
