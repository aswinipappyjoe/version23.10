...
namespace PappyjoeMVC.View
{
    public partial class Dentalwork : Form
    {
        ...
        private void dentalwork_Load(object sender, EventArgs e)
        {
            try
            {
                dateTimePickerdue.MinDate = dateTimePickerassgndate.Value;
                ...
                if (den.Rows.Count > 0)
                {
                    dateTimePickerdue.MinDate = Convert.ToDateTime(den.Rows[0]["duedate"].ToString()); 
        ...
        private void btnSaveorder_Click(object sender, EventArgs e)
        {
            try
            {
                int j = this.ctrlr.labupdate(cmblabname.Text, dateTimePickerassgndate.Value.ToString("yyyy-MM-dd"), dateTimePickerdue.Value.ToString("yyyy-MM-dd"), patient_id, txtjobno.Text);
                if (j > 0)
                {
