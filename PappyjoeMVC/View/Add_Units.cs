    ...
    public partial class Add_Units : Form
    {
        ...
        private void dgv_Units_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                {
                    DataTable dt_unit = this.cntrl.get_batch_unit(dgv_Units.CurrentRow.Cells["units"].Value.ToString());
                    if(dt_unit.Rows.Count>0)
                    {
                        MessageBox.Show("This Unit have stock,can,t edit !..", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                    else
                    {
                ...
                if (dgv_Units.CurrentCell.OwningColumn.Name == "del")
                {
                    ...
                    if (dt_unit.Rows.Count > 0)
                    {
                        MessageBox.Show("This Unit have stock,can,t delete !..", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
