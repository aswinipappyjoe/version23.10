...
namespace PappyjoeMVC.View
{
    public partial class EMR_Settings : Form
    {
        ...
        string id_diag, id_invest, id_notes, id_allergy;
        public EMR_Settings()
        {
            InitializeComponent();
        }
        private void EMR_settings_Load(object sender, EventArgs e)
        {
            ...
            dgvAllergy.ColumnHeadersDefaultCellStyle.BackColor = Color.DimGray;
            dgvAllergy.ColumnHeadersDefaultCellStyle.ForeColor = Color.White;
            dgvAllergy.ColumnHeadersDefaultCellStyle.Font = new System.Drawing.Font("Sego UI", 9, FontStyle.Regular);
            dgvAllergy.EnableHeadersVisualStyles = false;
            foreach (DataGridViewColumn cl in dgvAllergy.Columns)
            {
                cl.SortMode = DataGridViewColumnSortMode.NotSortable;
            }
        }
           ...
           foreach (DataRow dr in dt2.Rows)
            {
                try
                {
                    dataGridView_diag.Rows.Add(dr["id"].ToString(), dr["diagnosis"].ToString());
                    //i++;
                }
                catch (Exception ex)
                {
        ...
        private void btnSaveAllerg_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (btnSaveAllerg.Text == "Save")
                    {
                        DataTable dt = this.cntrl.check_allergy(txtAllergy.Text);
                        if (dt.Rows.Count > 0)
                        {
                            MessageBox.Show("This Record " + txtAllergy.Text + "  already exists", "Duplication encountered ", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            return;
                        }
                        else
                        {
                            this.cntrl.save_allergy(txtAllergy.Text);
                            txtAllergy.Clear();
                            DataTable dtb = this.cntrl.Fill_allergygrid();
                            FillAllergy(dtb);
                        }
                    }
                    else
                    {
                        this.cntrl.update_allergy(id_allergy, txtAllergy.Text);
                        txtAllergy.Clear();
                        DataTable dtb = this.cntrl.Fill_allergygrid();
                        FillAllergy(dtb);
                        btnSaveAllerg.Text = "Save";
                    }
                }
            }
            catch (Exception ex)
        ...
        private void txtSearch_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            this.cntrl.search_allergy(txtSearch.Text);
        }
        ...
        private void tabControl1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                DataTable dtb_allergy = this.cntrl.Fill_allergygrid();
                FillAllergy(dtb_allergy);
            }
        }

        private void dgvAllergy_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if(e.RowIndex>=0)
            {
                id_allergy = dgvAllergy.Rows[e.RowIndex].Cells[0].Value.ToString();
                if (dgvAllergy.CurrentCell.OwningColumn.Name == "coled")
                {
                    try
                    {
                        txtAllergy.Text = dgvAllergy.CurrentRow.Cells[1].Value.ToString();
                        btnSaveAllerg.Text = "Update";
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show(ex.Message, "Error!..", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                if (dgvAllergy.CurrentCell.OwningColumn.Name == "coldel")
                {
                    if (!String.IsNullOrWhiteSpace(id_allergy))
                    {
                        DialogResult res = MessageBox.Show("Are you sure you want to delete this record?", "Delete confirmation",
                                   MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                        if (res == DialogResult.No)
                        ...
                        {
                            this.cntrl.delete_allergy(id_allergy);
                            DataTable dtb = this.cntrl.Fill_allergygrid();
                            FillAllergy(dtb);
                        }
                    }
        ...
        public void FillAllergy(DataTable dt2)
        {
            ...
            while (i < dt2.Rows.Count)
            {
                try
                {
                    dgvAllergy.Rows.Add(dt2.Rows[i]["id"].ToString(), dt2.Rows[i]["allergies"].ToString());
                    i++;
                }
                catch (Exception ex)
                {
