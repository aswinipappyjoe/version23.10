...
namespace PappyjoeMVC.View
{
    public partial class Sales_List : Form
    {
        ...
        public void loadSup()
        {
            ...
            cmbSupname.Items.Add("All Patients");
            cmbSupname.ValueMember = "0";
            cmbSupname.DisplayMember = "All Patients";
            System.Data.DataTable sup = this.cntrl.supname();
            if (sup.Rows.Count > 0)
            {
                foreach (DataRow dr in sup.Rows)
                {
                    cmbSupname.Items.Add(dr["pt_name"].ToString());
                    cmbSupname.ValueMember = dr["id"].ToString();
                    cmbSupname.DisplayMember = dr["pt_name"].ToString();
                }
            }
        ...
        private void Sales_List_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (flag_fromInventory == true)
                {
                    ...
                    DataTable dt = this.cntrl.get_salesDetails(Convert.ToDateTime(dateTo).ToString("yyyy-MM-dd"), Convert.ToDateTime(dateFrom).ToString("yyyy-MM-dd"),type);
                    Fill_dgvSale(dt);
                }
                else
                {
                    DataTable dtb = this.cntrl.invDetailsbyDate(DateTime.Now.ToString("yyyy-MM-dd"),type);
                    Fill_dgvSale(dtb);
                }
        ...
        public void Fill_dgvSale(DataTable dtb)
        {
            ...
            if (dtb.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dtb.Rows.Count; i++)
                {
                    ...
                    dgv_sales.Rows[i].Cells["colinvDate"].Value = Convert.ToDateTime(dtb.Rows[i]["InvDate"].ToString()).ToString("dd/MM/yyyy");
                    ...
                    dgv_sales.Rows[i].Cells["colTotalAmount"].Value = Convert.ToDecimal(dtb.Rows[i]["TotalAmount"].ToString()).ToString("##.00");
        ...
        private void rad_Cash_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                if (rad_Cash.Checked)
                {
                    ...
                    DataTable dt = this.cntrl.invDetailsbyDateBtwn(DTP_From.Value.ToString("yyyy-MM-dd"), DTP_To.Value.ToString("yyyy-MM-dd"),type);
                    Fill_dgvSale(dt);
                }
            }
            catch (Exception ex)
        ...
        private void rad_Credit_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                if (rad_Credit.Checked)
                {
                    ...
                    Lab_Msg.Visible = false;
                    DataTable dt = this.cntrl.InvDetailsBtwnDates(DTP_From.Value.ToString("yyyy-MM-dd"), DTP_To.Value.ToString("yyyy-MM-dd"),type);
                    Fill_dgvSale(dt);
                }
            }
            catch (Exception ex)
        ...
        private void BtnShow_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    //if (cmbSupname.Text!= "All Patients")
                    //{
                    //    dt = this.cntrl.get_salesDetails(DTP_From.Value.ToString("yyyy-MM-dd"), DTP_To.Value.ToString("yyyy-MM-dd"), type,cmbSupname.Text);
                    //}
                    //else
                    //{
                    dt = this.cntrl.get_salesDetails(DTP_From.Value.ToString("yyyy-MM-dd"), DTP_To.Value.ToString("yyyy-MM-dd"), type);
        ...
        private void btnexport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    DataTable dt = this.cntrl.get_salesDetails(from,to,type);
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        if (saveFileDialog1.ShowDialog() == DialogResult.OK)
                        {
                            ...
                            ExcelApp.Range[ExcelApp.Cells[5, 7], ExcelApp.Cells[6, 7]].Merge();
                            ExcelApp.Cells[5, 7] = "Net Amount(Includes VAT)";
                            ExcelApp.Cells[5, 7].ColumnWidth = 24;
                            ExcelApp.Cells[5, 7].EntireRow.Font.Bold = true;
                            ExcelApp.Cells[5, 7].Interior.Color = Color.FromArgb(0, 102, 204);
                            ExcelApp.Cells[5, 7].Font.Size = 10;
                            ExcelApp.Cells[5, 7].Font.Name = "Arial";
                            ...
                            for (int i = 0; i < dt.Rows.Count; i++)
                            {
                                ...
                                ExcelApp.Cells[row, 7] = Convert.ToDouble(dt.Rows[i]["TotalAmount"].ToString());
                            ...
                            ExcelApp.Range[ExcelApp.Cells[5, 1], ExcelApp.Cells[row, 7]].BorderAround(true);
                            ExcelApp.Range[ExcelApp.Cells[5, 1], ExcelApp.Cells[row, 7]].Borders.Color = Color.FromArgb(0, 0, 0);
                            ...
                            ExcelApp.Cells[row, 7].EntireRow.Font.Bold = true;
                            ExcelApp.Cells[row, 7].Font.Size = 10;
                            ...
                            ExcelApp.Cells[row, 7] = total_net; 
                            Lab_Msg.Visible = false;
        ...
        private void cmbSupname_Click(object sender, EventArgs e)
        {
            loadSup();
        }
    }
