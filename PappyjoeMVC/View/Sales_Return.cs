...
namespace PappyjoeMVC.View
{
    public partial class Sales_Return : Form
    {
        ...
        private void lstInvNo_MouseClick(object sender, MouseEventArgs e)
        {
            if (lstInvNo.SelectedItems.Count > 0)
            {
                ...
                if (value != "")
                {
                    ...
                    {
                        MessageBox.Show("Please note :This is a discounted bill !..","Warning !!", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }
            }
        }
        private void btn_item_Choose_Click(object sender, EventArgs e)
        {
            if (txt_InvoiceNum.Text != "")
            {
                ...
                if (dtb_itemlist.Rows.Count > 0)
                {
                    ...
                    form2.ShowDialog();
                    form2.Dispose();
                    if (ItemCode_From_List != "" && ItemCode_From_List !=null)
                    {
                        ...
                        if (itemcheck() == 0)
                        {
                            ...
                            if (dtb.Rows.Count > 0)
                            {
                                ...
                                if(dtb.Rows[0]["Discount"].ToString()!="")
                                {
                                    txt_disc.Text = dtb.Rows[0]["Discount"].ToString();
                                }
                                else
                                {
                                    txt_disc.Text = "0";
        ...
        {
            Decimal gst = 0, unitMf = 0, igst = 0, gst_Amount = 0, Amount = 0, igst_Amount = 0, qty = 0, unitcost = 0, TotalAmount = 0,discount=0,disc=0;
            decimal d;
            ...
            if(decimal.TryParse(txt_disc.Text,out d))
            {
            ...
            if (Convert.ToDecimal(txt_disc.Text)>0)
            {
                if (TotalAmount > 0)
                {
            ...
            if (dgv_SalesItem.Rows.Count > 0)
            {
                ...
                using (MySqlConnection con = new MySqlConnection(cs))
                {
                    ...
                    try
                    {
                        int i = 0, j = 0;
                        decimal GST = 0;
                        DataTable dtb = this.cntrl.get_salesdetails(txt_InvoiceNum.Text,con,trans);
                        GST = Convert.ToDecimal(txt_CGST.Text) + Convert.ToDecimal(txt_SGST.Text);
                        i = this.cntrl.save_master(DateTime.Now.ToString("yyyy-MM-dd"), txt_InvoiceNum.Text, dtp_invDate.Value.ToString("yyyy-MM-dd"), txtPatientID.Text, txt_patient.Text, txt_GrandTotal.Text, doctor_id.ToString(), txt_TotalCost.Text, GST, Txt_TotalIGST.Text, con, trans);
                        if (i > 0)
                        {
                            string dt_retnumber = this.cntrl.max_retnumber(con, trans);
                            foreach (DataGridViewRow dr in dgv_SalesItem.Rows)
                            {
                                string unit2; decimal bat_RetQty_sales = 0, Totalquantity = 0;
                                DataTable dtUnit2 = this.cntrl.Get_unites(dr.Cells["id"].Value.ToString(), con, trans);
                                if (dtUnit2.Rows[0][0].ToString() == dr.Cells["ColUnit"].Value.ToString())
                                {
                                    ...
                                    unit2 = "No";
                                this.cntrl.save_returnitems(dt_retnumber, dr.Cells["id"].Value.ToString(), dr.Cells["ColQty"].Value.ToString(), dr.Cells["ColUnit"].Value.ToString(), dr.Cells["colUnitcost"].Value.ToString(), unit2, dr.Cells["ColFree"].Value.ToString(), con, trans);
                                string dt_UpdateRetqty_sales = this.cntrl.get_sales_retqty(txt_InvoiceNum.Text, dr.Cells["id"].Value.ToString(), con, trans);
                                DataTable dt_salesRetqty = this.cntrl.get_sales_qty(dt_retnumber, dr.Cells["id"].Value.ToString(), con, trans);
                                ...
                                this.cntrl.update_sales_retqty(bat_RetQty_sales, dr.Cells["id"].Value.ToString(), txt_InvoiceNum.Text, con, trans);
                            }
                            int rowaffected = 0; decimal bat_RetQty = 0;
                            for (int k = 0; k < dgv_BatchSale.Rows.Count; k++)
                            {
                                string retqty = this.cntrl.retqty_from_batchsale(dgv_BatchSale.Rows[k].Cells["colentry"].Value.ToString(), con, trans);
                                if (Convert.ToDecimal(retqty) > 0)
                                {
                                    ...
                                    {
                                        this.cntrl.update_batchsale(bat_RetQty, dgv_BatchSale.Rows[k].Cells["colentry"].Value.ToString(), con, trans);
                                    }
                                }
                                else
                                {
                                    if (Convert.ToDecimal(dgv_BatchSale.Rows[k].Cells["colReturnQty"].Value.ToString()) > 0)
                                    {
                                        this.cntrl.update_batchsale(Convert.ToDecimal(dgv_BatchSale.Rows[k].Cells["colentry"].Value.ToString()), dgv_BatchSale.Rows[k].Cells["colReturnQty"].Value.ToString(), con, trans);
                                    }
                                }
                                rowaffected = this.cntrl.update_batchnumber(dgv_BatchSale.Rows[k].Cells["currentStock"].Value.ToString(), dgv_BatchSale.Rows[k].Cells["colBatchentry"].Value.ToString(), con, trans);
                            }
                            trans.Commit();
                            con.Close();
                            if (rowaffected > 0)
                            {
                                ...
                                this.cntrl.save_log(doctor_id, "Sales Return", " Add Sales Return", dt, Convert.ToString(Timeonly.ToString("hh:mm tt")), "Add", dt_retnumber);
                    ...
                    {
                        trans.Rollback();
                        MessageBox.Show(ex.Message,"Error Occured, Please try again !..",MessageBoxButtons.OK,MessageBoxIcon.Error);
                    }
                }
            } 
        }
        ...
        public void printhtml()
        {
            ...
            StreamWriter sWrite = new StreamWriter(Apppath + "\\salesReturn_print.html");
            ...
            else
            {
                ...
                sWrite.WriteLine("<tr><td align='left' ><FONT COLOR=black FACE='Segoe UI' SIZE=2> Patient Name :&nbsp" + txt_patient.Text + "</font></td></tr>");
                sWrite.WriteLine("<tr><td align='left' ><FONT COLOR=black FACE='Segoe UI' SIZE=2> Patient ID :&nbsp" + txtPatientID.Text + "</font></td></tr>");
                sWrite.WriteLine("<tr><td align='left' ><FONT COLOR=black FACE='Segoe UI' SIZE=2> Phone :&nbsp" + txt_PhoneNo.Text + "</font></td></tr>");
                sWrite.WriteLine("</table>");
            }
            sWrite.WriteLine("<table align='center' style='width:700px;border: 1px ;border-collapse: collapse;'>");
            ...
            sWrite.WriteLine("</table>");
            sWrite.WriteLine("<tr>");
            sWrite.WriteLine("<th colspan=11><center><b><FONT COLOR=black FACE='Segoe UI'  SIZE=3> SALES RETURN BILL </font></center></b></td>");
            sWrite.WriteLine("</tr>");
            sWrite.WriteLine("<table align='center' style='width:700px;border: 1px ;border-collapse: collapse;'>");
            sWrite.WriteLine("<tr>");
            ...
            sWrite.WriteLine("<td align='right'  width='65'><FONT COLOR=black FACE='Segoe UI' SIZE=2><b>Gst</b></font></td>");
            sWrite.WriteLine("<td align='right'  width='65'><FONT COLOR=black FACE='Segoe UI' SIZE=2><b>Discount</b></font></td>");
            ...
            if (dgv_SalesItem.Rows.Count > 0)
            {
                for (int i = 0; i < dgv_SalesItem.Rows.Count; i++)
                {
                    ...
                    sWrite.WriteLine("<td align='right' ><FONT COLOR=black FACE='Segoe UI' SIZE=2>" + dgv_SalesItem.Rows[i].Cells["ColGST"].Value.ToString() + "</font></td>");
                    sWrite.WriteLine("<td align='right' ><FONT COLOR=black FACE='Segoe UI' SIZE=2>" + dgv_SalesItem.Rows[i].Cells["c_disc"].Value.ToString() + "</font></td>");
            ...
            sWrite.WriteLine("<td align='right'  colspan=4 ><FONT COLOR=black FACE='Segoe UI' SIZE=3>&nbsp<b> " + String.Format("{0:C}", Convert.ToDecimal(txt_GrandTotal.Text)) + "</b> </font></td>");//decimal.Parse(txt_GrandTotal.Text).ToString("##0.00")
            sWrite.WriteLine("</tr>");
            sWrite.WriteLine("</table>");
            ...
            sWrite.WriteLine("</html>");
            sWrite.Close();
            System.Diagnostics.Process.Start(Apppath + "\\salesReturn_print.html");
        }
                    ...
                    else
                    {
                        ...
                        if (txt_CGST.Text != "" && txt_SGST.Text != "")
                        {
                            if (dgv_SalesItem.CurrentRow.Cells["ColGST"].Value != null && dgv_SalesItem.Rows[index].Cells["ColGST"].Value.ToString() != "")
                            {
                                ...
                                txt_SGST.Text = Convert.ToDecimal(GST).ToString("##0.00");
                            }
                        }
        ...
        private void btn_AddtoGrid_Click(object sender, EventArgs e)
        {
            ...
            if (Convert.ToDecimal(txt_Qty.Text) > 0)
            {
                ...
                if (dtFor_QtyReturn != null)
                {
                    if (flagset == true)
                    {
                        dgv_SalesItem.Rows.Add(ItemCode_From_List, txt_ItemCode.Text, txt_Discription.Text, txt_Packing.Text, cmb_Unit.Text, txt_GST.Text, txt_IGST.Text, txt_Qty.Text,txt_disc.Text, txt_Free.Text, txt_UnitCost.Text, txt_Amount.Text, qty_Check, PappyjoeMVC.Properties.Resources.deleteicon);
                        fill_batchGrid(); dtFor_QtyReturn = null;
                        flagset = false;
                    }
                    if (dgv_SalesItem.Rows.Count > 0)
                    {
                        decimal disc = 0, dis_amount = 0, grand = 0;
                        foreach (DataGridViewRow dr in dgv_SalesItem.Rows)
                        {
                        ...
                        if (TotalCost > 0)
                        {
                            txt_TotalCost.Text = Convert.ToDecimal(TotalCost).ToString();// TotalCost.ToString("##.00");
                        }
                        if (TotalAmount > 0)
                        {
                            txt_GrandTotal.Text = Convert.ToDecimal(TotalAmount).ToString();
                            //  Convert.ToDecimal(TotalAmount).ToString("##.00");
                            txt_GrandTotal.ForeColor = Color.Red;
                        }
                        if (TotalGst > 0)
                        {
                        ...
                        //if (disc > 0)
                        //{
                        //    dis_amount = ((disc * TotalCost) / 100); //txtSgst.Text = cgstcal1.ToString();
                        //    grand = (TotalCost - dis_amount);
                        //    txt_GrandTotal.Text = grand.ToString("##.00");
                        //}
                        //else
                        //{
                        //    txt_GrandTotal.Text = Convert.ToDecimal(TotalAmount).ToString();
                ...
                if (Ret_Numbr > 0)
                {
                    ...
                    if (dtb_main.Rows.Count > 0)
                    {
                        ...
                        if (dtb_Items.Rows.Count > 0)
                        {
                            ...
                            txt_GrandTotal.Text = String.Format("{0:C}", Convert.ToDecimal(TotalAmount));
// TotalAmount.ToString("##.00");
                            txt_GrandTotal.ForeColor = Color.Red;
                            if (ToatalGST > 0)
                            {
                                decimal sgst = ToatalGST / 2;
                                txt_SGST.Text = sgst.ToString("##.00");
                                txt_CGST.Text = sgst.ToString("##.00");
                            }
                            else
                            {
                                txt_SGST.Text = "0";
                            ...
                            if (TotalIGST > 0)
                            {
                                Txt_TotalIGST.Text = TotalIGST.ToString("##.00");
