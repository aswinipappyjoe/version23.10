...
namespace PappyjoeMVC.View
{
    public partial class Purchase_Return : Form
    {
        ...
        {
            decimal qty = 0, retqty = 0, newqty = 0,stock=0;
            if (txt_pur_no.Text != "")
            {
                ...
                if (flag == true)
                {
                    if (check() == 0)
                    {
                        ...
                        if(dtitems.Rows[0]["Unit1"].ToString().Trim()==cmbUnit.Text)
                        {
                            qty = Convert.ToDecimal(dt_stock.Rows[0]["qty"].ToString());
                            stock = qty / Convert.ToDecimal(dt_unit.Rows[0]["UnitMF"].ToString());
                        }
                        else if(dtitems.Rows[0]["Unit2"].ToString().Trim() == cmbUnit.Text)
                        {
                            stock= Convert.ToDecimal(dt_stock.Rows[0]["qty"].ToString());
                        }
                        if(Convert.ToDecimal( dtitems.Rows[0]["RetQty"].ToString())>0)
                        {
                            txt_qty.Text = dtitems.Rows[0]["RetQty"].ToString();
                        }
                        else
                        {
                            txt_qty.Text = dtitems.Rows[0]["Qty"].ToString();
                        }
                        // stock.ToString();
                        ...
                        txt_disc.Text = dtitems.Rows[0]["Discount"].ToString();//bhj
                        ...
                        {
                            decimal qtyy = decimal.Parse(txt_qty.Text);
                            decimal unitcost = Convert.ToDecimal(dtitems.Rows[0]["Rate"].ToString());
                            decimal amt = 0;
                            decimal gst = Convert.ToDecimal(dtitems.Rows[0]["GST"].ToString());
                            decimal igst = Convert.ToDecimal(dtitems.Rows[0]["IGST"].ToString());
                            if (txtGst.Text != "0.0")
                            {
                                amt = (((qtyy * unitcost) * gst) / 100) + (qtyy * unitcost);
                            }
                            if (txtIgst.Text != "0.00")
                            {
                                amt = (((qtyy * unitcost) * igst) / 100) + (qtyy * unitcost);
                            }
                            if (txtGst.Text == "0.0" && txtIgst.Text == "0.00" +
                                "")
                            {
                                amt = qtyy * unitcost;
                            }
        ...
        private void Btn_Add_Click(object sender, EventArgs e)
        {
            ...
            if (txt_pur_no.Text != "" && txt_Itemcode.Text != "")
            {
                ...
                if (txt_Itemcode.Text != "" && txtDescription.Text != "" && txt_qty.Text != "0" && txtUnitCost.Text != "0.000")
                {
                    if (flagBatch == true)
                    {
                        if (dgvItemData.Rows.Count > 0)
                        ...
                        if (flagchk == true)
                        {
                            if (batch1 != null)
                            {
                                ...
                                dgvItemData.Rows.Add(itemid, txt_Itemcode.Text, txtDescription.Text, txtPacking.Text, cmbUnit.Text, txtGst.Text, txtIgst.Text, txt_qty.Text, txt_disc.Text,txt_free.Text, txtUnitCost.Text, txtAmount.Text);
                                int qty = 0;
                                if (batch1.Rows.Count > 0)
                                {
                                    for (int i = 0; i < batch1.Rows.Count; i++)
                                    {
                                        if (batch1.Rows[i][0].ToString() != "")
                                        {
                                            ...
                                            dt = this.cntrl.get_batchentry_qty(itemid, batch1.Rows[i]["Branch_No"].ToString());//txt_pur_no.Text
                                            ...
                                            decimal ttlamount = 0, ttlamount1 = 0, igstcal = 0, igstcal1 = 0, cgstcalc=0, cgstcalc2=0, cgstcal = 0, cgstcal1 = 0, amtcal = 0, amtcal1 = 0;
                                            foreach (DataGridViewRow row1 in dgvItemData.Rows)
                                            {
                                                if (row1.Cells["Amount"].Value != null && row1.Cells["Amount"].Value.ToString() != "")
                                                {
                                                    ...
                                                    cgstcal = Convert.ToDecimal(row1.Cells["gst"].Value.ToString());
                                                    cgstcalc = (((unitcost * retqty) * cgstcal) / 100);
                                                    cgstcal1 = cgstcal1 + cgstcalc;
                                                    cgstcalc2 = cgstcal1 / 2;
                                                }
                                            }
                                            txtTotalCost.Text = amtcal1.ToString("##.00");
                                            txt_TotalAmount.Text = ttlamount1.ToString("##.00");
                                            txtCgst.Text = cgstcalc2.ToString("##.00");// (cgstcal1 / 2).ToString();
                                            txtIgstResult.Text = igstcal1.ToString();
                                            txtSgst.Text = cgstcalc2.ToString("##.00");// (cgstcal1 / 2).ToString();
                                            //txtSgst.Text = cgstcal1.ToString();
                                        }
                                    }
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            using (MySqlConnection con = new MySqlConnection(cs))
            {
                ...
                try
                {
                    ...
                    this.cntrl.save_purReturn(DateTime.Now.ToString("yyyy-MM-dd"), txt_pur_no.Text, dtpPurchDate.Value.ToString("yyyy-MM-dd"), txt_SupplierId.Text, txt_TotalAmount.Text, txtTotalCost.Text, con, trans);
                    string max_retno = this.cntrl.get_maxReturnNo(con, trans);
                    foreach (DataGridViewRow row in dgvItemData.Rows)
                    {
                        string unit2 = this.cntrl.get_unit2(row.Cells["Item_Id"].Value.ToString(), con, trans);
                        if (unit2 != "null")
                        {
                        ...
                        this.cntrl.save_returnItems(max_retno, row.Cells["Item_Id"].Value.ToString(), row.Cells["col_qty"].Value.ToString(), row.Cells["note"].Value.ToString(), row.Cells["Unit_Cost"].Value.ToString(), unit2Is, row.Cells["free"].Value.ToString(), row.Cells["gst"].Value.ToString(), row.Cells["igst"].Value.ToString(),row.Cells["c_disc"].Value.ToString() ,con, trans);
                    }
                    DataTable dtunit = new DataTable();
                    DataTable dt_stock = new DataTable()
;                    foreach (DataGridViewRow rw in dgvGridData.Rows)
                    {
                        dtunit = this.cntrl.Get_unites(rw.Cells["tempItem_code"].Value.ToString(), con, trans);
                        dt_stock = this.cntrl.dt_get_stock(rw.Cells["tempItem_code"].Value.ToString(), rw.Cells["Branch_No"].Value.ToString());
                        decimal QTY = 0,b=0;
                        if (cmbUnit.Text == dtunit.Rows[0]["Unit1"].ToString() && dtunit.Rows[0]["UnitMF"].ToString() != "0")
                        {
                            ...
                            QTY1 = Convert.ToDecimal(rw.Cells["Return_Qty"].Value.ToString()); // UpdateQty
                             b = QTY1 * mf;
                            QTY = Convert.ToDecimal(dt_stock.Rows[0]["qty"].ToString()) - b;
                        }
                        else
                        {
                            b = Convert.ToDecimal(rw.Cells["Return_Qty"].Value.ToString());
                            QTY = Convert.ToDecimal(dt_stock.Rows[0]["qty"].ToString()) - b;
                        }
                        if (rw.Cells["Return_Qty"].Value.ToString() != "0")
                        {
                            ...
                            this.cntrl.upadte_batchnumber(QTY, rw.Cells["tempItem_code"].Value.ToString(), rw.Cells["Branch_No"].Value.ToString(), con, trans);
                            string dtretqty = this.cntrl.get_returnqty(txt_pur_no.Text, rw.Cells["tempItem_code"].Value.ToString(), con, trans);
                            decimal ret = 0;
                            ...
                            retBatch = retqty;
                            this.cntrl.update_purchit(retnew, txt_pur_no.Text, rw.Cells["tempItem_code"].Value.ToString(), con, trans);
                            string dtBatch = this.cntrl.get_dte(txt_pur_no.Text, rw.Cells["tempItem_code"].Value.ToString(), rw.Cells["Branch_No"].Value.ToString(),con,trans);
                            batchQty = Convert.ToDecimal(dtBatch);
                            batchNewQty = batchQty + retBatch;
                            this.cntrl.update_batchpurchase(batchNewQty, txt_pur_no.Text, rw.Cells["tempItem_code"].Value.ToString(), rw.Cells["Branch_No"].Value.ToString(),con,trans);
                        }
                    }
                    trans.Commit();
                    con.Close();
                    ...
                    this.cntrl.save_log(doctor_id, "Purchase Return", " Adds Purchase Return", dt11, Convert.ToString(Timeonly.ToString("hh:mm tt")), "Add", max_retno);
        ...
        public void calculatonsss()
        {
            if (txt_qty.Text != "" && txtUnitCost.Text != "")
            {
                decimal gstamt = 0, igstamt = 0, unitcost = 0, igst = 0, gst = 0, qty = 0, discount = 0, disc = 0;
        ...
        private void FrmPurchase_Return_Load(object sender, EventArgs e)
        {
            if (flagpurchRet == true)
            {
                ...
                decimal cgstcalc = 0, cgstcalc1 = 0, cgstcalc2 = 0, unitcostcal = 0, qtycal = 0, cgstcal=0;
                DataTable dt_unit = new DataTable();
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    dgvItemData.Rows[i].Cells["itemcode"].Value = dt.Rows[i]["item_name1"].ToString();
                    ...
                    if (dt.Rows[i]["UNIT2"].ToString().Trim() != "")
                    {
                        dgvItemData.Rows[i].Cells["note"].Value = dt_unit.Rows[0]["Unit2"].ToString();
                    }
                    else if (dt.Rows[i]["UNIT2"].ToString().Trim() == "")
                    {
                    ...
                    if (dt.Rows[i]["UNIT2"].ToString().Trim() != "")
                        dgvItemData.Rows[i].Cells["c_disc"].Value = dt.Rows[i]["Discount"].ToString();
                    else
                        dgvItemData.Rows[i].Cells["c_disc"].Value = "0";
                    ...
                    if (Convert.ToDecimal(dgvItemData.Rows[i].Cells["gst"].Value.ToString()) > 0)
                    {
                        ...
                        unitcostcal = Convert.ToDecimal(dt.Rows[i]["Rate"].ToString());
                        qtycal = Convert.ToDecimal( dt.Rows[i]["Qty"].ToString());
                        cgstcal = Convert.ToDecimal(dt.Rows[i]["Gst"].ToString());
        ...
        private void print()
        {
            if (dgvItemData.Rows.Count > 0)
            {
                ...
                StreamWriter sWrite = new StreamWriter(Apppath + "\\PurcahseReturn_print.html");
                ...
                if (dgvItemData.Rows.Count > 0)
                {
                    ...
                    sWrite.WriteLine("    <td align='right' width='230px' style='border:1px solid #000;background:#999999'><FONT COLOR=black FACE='Geneva, Segoe UI' SIZE=3>Discount</font></td>");//bhj
                    ...
                    while (c < dgvItemData.Rows.Count)
                    {
                        ...
                        sWrite.WriteLine("    <td align='right' style='border:1px solid #000' ><FONT COLOR=black FACE='Geneva, Segoe UI' SIZE=2>" + dgvItemData.Rows[c].Cells["c_disc"].Value.ToString() + "</font></td>");//bhj
                    ...
                    sWrite.WriteLine("</tr >");
                    sWrite.WriteLine("<tr >");
                    sWrite.WriteLine("<td colspan=10 align='right'><right><FONT COLOR=black FACE='Segoe UI' SIZE=2>   &nbsp; Total Cost:  " +  Convert.ToDecimal(txt_TotalAmount.Text).ToString("0.00") + " </font></right></td>");
                    sWrite.WriteLine("</tr >");
                }
                sWrite.WriteLine("</table>");
                sWrite.WriteLine("</div>");
                ...
                sWrite.WriteLine("</html>");
                sWrite.Close();
                System.Diagnostics.Process.Start(Apppath + "\\PurcahseReturn_print.html");
            }
            else
            {
