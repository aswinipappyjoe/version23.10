    ...
    public partial class Treatment_Plans : Form
    {
        ...
        private void TreatmentPlans_Load(object sender, EventArgs e)
        {
            ...
            //{
                ...
                appointment_count();
                vitals_count();
                clinical_count();//dt_cf_main_load_count
                treatment_count();
                fnsdtrt_count();
                prescr_count();
                lab_count();
        ...
        {
            System.Data.DataTable dt_cf_main = this.clinical_cntrl.dt_cf_main_load_count(patient_id);
            if (dt_cf_main.Rows.Count > 0)
            {
        ...
        {
            DataTable dtb = this.cntrl.Load_treatments_sidelabel_count(patient_id);
            if (dtb.Rows.Count > 0)
            {
                lb_trtment_cnt.Text = dtb.Rows.Count.ToString();
            }
            if (dtb.Rows.Count > 10)
            {
                label1.Visible = true;
            }
            else
                ...
                if (dt_pt_main.Rows.Count > 0)
                {
                    ...
                    for (int j = 0; j < dt_pt_main.Rows.Count; j++)
                    {
                        ...
                        if (dt_enablecost.Rows.Count > 0)
                        {
                            ...
                            {
                                dataGridView1_treatment_paln.Rows.Add(dt_pt_main.Rows[j]["id"].ToString(), "", "TREATMENTS", "", "", "", "NOTE", "0", "");
        ...
        public void mprintosave()
        {
            ...
            if (dt_Treat_Plan.Rows.Count > 0)
            {
                ...
                for (i = 0; i < dt_Treat_Plan.Rows.Count; i++)
                {
                    ...
                    sWrite.WriteLine("<td align='right'  width='250px'><FONT COLOR=black FACE='Geneva, Segoe UI' SIZE=2>&nbsp;" + String.Format("{0:C3}", Convert.ToDecimal(dt_Treat_Plan.Rows[i]["cost"].ToString())) + "</font></td>");
                    ...
                    sWrite.WriteLine("<td align='right'   width='250px'><FONT COLOR=black FACE='Geneva, Segoe UI' SIZE=2>&nbsp;" + String.Format("{0:C3}", Convert.ToDecimal(totalcost)) + " </font></td>");
                    sWrite.WriteLine("<td align='right'  width='250px' ><FONT COLOR=black FACE='Geneva, Segoe UI' SIZE=2>&nbsp;" + String.Format("{0:C3}", Convert.ToDecimal(dt_Treat_Plan.Rows[i]["total"].ToString())) + "</font></td>");
                ...
                sWrite.WriteLine("<td align='Right'   colspan='5'><FONT COLOR=black FACE='Geneva, Segoe UI' SIZE=2>&nbsp;" + "Estimated Amount : <b>" + String.Format("{0:C3}", Convert.ToDecimal(totalEst)) + "</b> </font></td>");
                sWrite.WriteLine("</tr>");
                sWrite.WriteLine("</table>");
                sWrite.WriteLine("</body>");
                sWrite.WriteLine("</html>");
        ...
        public void printhtml()
        {
            ...
            string Apppath = System.IO.Directory.GetCurrentDirectory();
            StreamWriter sWrite = new StreamWriter(Apppath + "\\Treatment_print.html");
            sWrite.WriteLine("<html>");
            sWrite.WriteLine("<head>");
            sWrite.WriteLine("</head>");
            sWrite.WriteLine("<body >");
            ...
            sWrite.WriteLine("<table align='center'   style='width:700px;border: 1px ;border-collapse: collapse;' >");
            sWrite.WriteLine("<tr>");
            sWrite.WriteLine("<tr><td colspan=10><center><FONT COLOR=black FACE='Geneva, Segoe UI' SIZE=5>Treatment Plan</FONT></center></td></tr>");
            ...
            sWrite.WriteLine("<td align='left' width='136px' bgcolor='#CCCCCC'><FONT COLOR=black FACE=' Segoe UI' SIZE=3>&nbsp;Notes </font></td>");
            sWrite.WriteLine("</tr>");
            System.Data.DataTable dt_Treat_Plan = this.cntrl.treatment_sub_details(treatment_plan_id);
            if (dt_Treat_Plan.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dt_Treat_Plan.Rows.Count; i++)
                {
                    ...
                    sWrite.WriteLine("<td align='right'><FONT COLOR=black FACE='Geneva, Segoe UI' SIZE=2>&nbsp;" + String.Format("{0:C3}", Convert.ToDecimal(dt_Treat_Plan.Rows[i]["cost"].ToString())) + "</font></td>");
                    sWrite.WriteLine("<td align='right'><FONT COLOR=black FACE='Geneva, Segoe UI' SIZE=2>&nbsp;" + String.Format("{0:C3}", Convert.ToDecimal(dt_Treat_Plan.Rows[i]["discount_inrs"].ToString())) + "</font></td>");
                    sWrite.WriteLine("<td align='right'><FONT COLOR=black FACE='Geneva, Segoe UI' SIZE=2>&nbsp;" + String.Format("{0:C3}", Convert.ToDecimal(totalcost)) + " </font></td>");
                    sWrite.WriteLine("<td align='right'><FONT COLOR=black FACE='Geneva, Segoe UI' SIZE=2>&nbsp;" + String.Format("{0:C3}", Convert.ToDecimal(dt_Treat_Plan.Rows[i]["total"].ToString())) + "</font></td>");
                    sWrite.WriteLine("<td align='char' style='text-align:left'><FONT COLOR=black FACE='Geneva, Segoe UI' SIZE=2>&nbsp;" + dt_Treat_Plan.Rows[i]["note"].ToString() + " " + dt_Treat_Plan.Rows[i]["tooth"].ToString() + "</font></td>");
                    sWrite.WriteLine("</tr>");
                ...
                sWrite.WriteLine("<td align='Right'   colspan=2><FONT COLOR=black FACE='Geneva, Segoe UI' SIZE=3>&nbsp;" + String.Format("{0:C3}", Convert.ToDecimal(totalcot_treat)) + " </font></td>");
                sWrite.WriteLine("</tr>");
                sWrite.WriteLine("</table>");
        ...
        public void data_load_shomore(DataTable dt_pt_main)
        {
            try
            {
                if (dt_pt_main.Rows.Count > 0)
                {
                    int rowcount = dataGridView1_treatment_paln.Rows.Count;
                    foreach (DataRow dr in dt_pt_main.Rows)
                    {
                        dataGridView1_treatment_paln.Rows.Add();
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[0].Value = "0";
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[1].Value = "";
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[2].Value = String.Format("{0:dddd, MMMM d, yyyy}", Convert.ToDateTime(dr["date"].ToString()));
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[3].Value = "";
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[4].Value = "";
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[5].Value = "";
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[0].Value = "";
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[0].Value = "0";
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[2].Style.Font = new System.Drawing.Font("Segoe UI", 7, FontStyle.Bold);
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[2].Style.ForeColor = Color.DarkGreen;
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[1].Value = PappyjoeMVC.Properties.Resources.blank;
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[8].Value = PappyjoeMVC.Properties.Resources.blank;
                        rowcount = rowcount + 1;
                        dataGridView1_treatment_paln.Rows.Add();
                        DataTable dt_enablecost = this.cntrl.get_enablecost();
                        if (dt_enablecost.Rows.Count > 0)
                        {
                            dataGridView1_treatment_paln.Rows[rowcount].Cells[0].Value = dr["id"].ToString();// dt_pt_main.Rows[j]["id"].ToString();
                            dataGridView1_treatment_paln.Rows[rowcount].Cells[1].Value = "";
                            dataGridView1_treatment_paln.Rows[rowcount].Cells[2].Value = "TREATMENTS";
                            dataGridView1_treatment_paln.Rows[rowcount].Cells[3].Value = "COST";
                            dataGridView1_treatment_paln.Rows[rowcount].Cells[4].Value = "DISCOUNT";
                            dataGridView1_treatment_paln.Rows[rowcount].Cells[5].Value = "TOTAL";
                            dataGridView1_treatment_paln.Rows[rowcount].Cells[6].Value = "NOTE";
                            dataGridView1_treatment_paln.Rows[rowcount].Cells[7].Value = "0";
                            if (dt_enablecost.Rows[0][1].ToString() == "Yes")
                            {
                                dataGridView1_treatment_paln.Rows[rowcount].Cells[0].Value = dr["id"].ToString();
                                dataGridView1_treatment_paln.Rows[rowcount].Cells[1].Value = "";
                                dataGridView1_treatment_paln.Rows[rowcount].Cells[2].Value = "TREATMENTS";
                                dataGridView1_treatment_paln.Rows[rowcount].Cells[3].Value = "COST";
                                dataGridView1_treatment_paln.Rows[rowcount].Cells[4].Value = "DISCOUNT";
                            ...
                            else
                            {
                                ...
                                dataGridView1_treatment_paln.Rows[rowcount].Cells[6].Value = "NOTE";
                                dataGridView1_treatment_paln.Rows[rowcount].Cells[7].Value = "0";
                                dataGridView1_treatment_paln.Rows[rowcount].Cells[8].Value = "";
                            }
                        }
                        else
                        {
                        ...
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[1].Value = PappyjoeMVC.Properties.Resources.gry;
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[2].Style.Font = new System.Drawing.Font("Segoe UI", 10, FontStyle.Bold);
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[2].Style.ForeColor = Color.White;
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[3].Style.Font = new System.Drawing.Font("Segoe UI", 9, FontStyle.Bold);
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[3].Style.ForeColor = Color.White;
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[4].Style.Font = new System.Drawing.Font("Segoe UI", 9, FontStyle.Bold);
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[4].Style.ForeColor = Color.White;
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[5].Style.Font = new System.Drawing.Font("Segoe UI", 9, FontStyle.Bold);
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[5].Style.ForeColor = Color.White;
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[6].Style.Font = new System.Drawing.Font("Segoe UI", 9, FontStyle.Bold);
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[6].Style.ForeColor = Color.White;
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[1].Style.BackColor = Color.DarkGray;
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[2].Style.BackColor = Color.DarkGray;
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[3].Style.BackColor = Color.DarkGray;
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[4].Style.BackColor = Color.DarkGray;
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[5].Style.BackColor = Color.DarkGray;
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[6].Style.BackColor = Color.DarkGray;
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[3].Style.Alignment = DataGridViewContentAlignment.MiddleRight;
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[4].Style.Alignment = DataGridViewContentAlignment.MiddleRight;
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[5].Style.Alignment = DataGridViewContentAlignment.MiddleRight;
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[8].Value = PappyjoeMVC.Properties.Resources.Bill;

                        DataTable dt_pt_sub = this.cntrl.treatment_sub_details(dr["id"].ToString());
                        Double totalEst = 0;
                        foreach (DataRow drow in dt_pt_sub.Rows) 
                        {
                            string discount_string = "";
                            if (drow["discount_type"].ToString() == "INR")
                            {
                                discount_string = "";
                            }
                            else
                            {
                                discount_string = "(" + drow["discount"].ToString() + "%)";
                            }
                            Decimal totalcost = Convert.ToDecimal(drow["cost"].ToString()) * Convert.ToDecimal(drow["quantity"].ToString());
                            rowcount = rowcount + 1;// i = i + 1;
                            dataGridView1_treatment_paln.Rows.Add();
                            if (dt_enablecost.Rows.Count > 0)
                            {
                                if (dt_enablecost.Rows[0][1].ToString() == "Yes")
                                {
                                    dataGridView1_treatment_paln.Rows[rowcount].Cells[0].Value = drow["id"].ToString();
                                    dataGridView1_treatment_paln.Rows[rowcount].Cells[1].Value = "";
                                    dataGridView1_treatment_paln.Rows[rowcount].Cells[2].Value = drow["procedure_name"].ToString();
                                    dataGridView1_treatment_paln.Rows[rowcount].Cells[3].Value = String.Format("{0:C2}", Convert.ToDecimal(totalcost));
                                    dataGridView1_treatment_paln.Rows[rowcount].Cells[4].Value = String.Format("{0:C2}", Convert.ToDecimal(drow["discount_inrs"].ToString())) + discount_string;
                                    dataGridView1_treatment_paln.Rows[rowcount].Cells[5].Value = String.Format("{0:C2}", Convert.ToDecimal(drow["total"].ToString()));
                                    dataGridView1_treatment_paln.Rows[rowcount].Cells[6].Value = drow["note"].ToString() + " " + drow["tooth"].ToString();
                                    dataGridView1_treatment_paln.Rows[rowcount].Cells[7].Value = drow["status"].ToString();
                                }
                                else
                                {
                                    dataGridView1_treatment_paln.Rows[rowcount].Cells[0].Value = drow["id"].ToString();
                            ...
                            dataGridView1_treatment_paln.Rows[rowcount].Height = 30;
                            dataGridView1_treatment_paln.Columns[6].Width = 200;
                            dataGridView1_treatment_paln.Rows[rowcount].Cells[3].Style.Alignment = DataGridViewContentAlignment.MiddleRight;
                            dataGridView1_treatment_paln.Rows[rowcount].Cells[4].Style.Alignment = DataGridViewContentAlignment.MiddleRight;
                            dataGridView1_treatment_paln.Rows[rowcount].Cells[5].Style.Alignment = DataGridViewContentAlignment.MiddleRight;
                            ...
                            {
                                dataGridView1_treatment_paln.Rows[rowcount].Cells[1].Value = PappyjoeMVC.Properties.Resources.TickRed;
                            }
                            else
                            {
                                dataGridView1_treatment_paln.Rows[rowcount].Cells[1].Value = PappyjoeMVC.Properties.Resources.Borderblank;
                            }
                            totalEst = totalEst + Convert.ToDouble(drow["total"].ToString());
                            dataGridView1_treatment_paln.Rows[rowcount].Cells[8].Value = PappyjoeMVC.Properties.Resources.blank;
                        }
                        rowcount = rowcount + 1;
                        dataGridView1_treatment_paln.Rows.Add();
                        if (dt_enablecost.Rows.Count > 0)
                        {
                            if (dt_enablecost.Rows[0][0].ToString() == "Yes")
                            {
                                dataGridView1_treatment_paln.Rows[rowcount].Cells[0].Value = "";
                                dataGridView1_treatment_paln.Rows[rowcount].Cells[1].Value = "";
                                dataGridView1_treatment_paln.Rows[rowcount].Cells[2].Value = "Planned by " + dr["dr_name"].ToString();
                                dataGridView1_treatment_paln.Rows[rowcount].Cells[3].Value = "";
                                dataGridView1_treatment_paln.Rows[rowcount].Cells[4].Value = "Estimated amount:";
                                dataGridView1_treatment_paln.Rows[rowcount].Cells[5].Value = String.Format("{0:C2}", Convert.ToDecimal(totalEst));
                                dataGridView1_treatment_paln.Rows[rowcount].Cells[6].Value = "";
                                dataGridView1_treatment_paln.Rows[rowcount].Cells[7].Value = "0";
                            }
                            else
                            {
                        ...
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[2].Style.Font = new System.Drawing.Font("Segoe UI", 9, FontStyle.Bold);
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[5].Style.ForeColor = Color.Red;
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[5].Style.Font = new System.Drawing.Font("Segoe UI", 10, FontStyle.Bold);
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[1].Value = PappyjoeMVC.Properties.Resources.blank;
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[8].Value = PappyjoeMVC.Properties.Resources.blank;
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[5].Style.Alignment = DataGridViewContentAlignment.MiddleRight;
                        rowcount = rowcount + 1;
                        ...
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[0].Value = "";
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[1].Value = "";
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[2].Value = "";
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[3].Value = "";
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[4].Value = "";
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[5].Value = "";
                        dataGridView1_treatment_paln.Rows[rowcount].Cells[6].Value = "";
        ...
        {
            int count = row_count + 10;
            DataTable dtb = this.cntrl.Load_treatments_count(patient_id, count);
            data_load_shomore(dtb);
            row_count = count;
        }
    }
}
