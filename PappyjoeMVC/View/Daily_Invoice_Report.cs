...
namespace PappyjoeMVC.View
{
    public partial class Daily_Invoice_Report : Form
    {
            ...
            try
            {
                ...
                else
                {
                    ...
                    for (int z = 0; z < invMain.Rows.Count; z++)
                    {
                        ...
                        string doctor = invMain.Rows[z]["doctor_name"].ToString();// combodoctors.SelectedItem.ToString();
                        //decimal tax = Convert.ToDecimal(invMain.Rows[z]["doctor_name"].ToString());
                        ...
                        due = due + amdue;
                        Grvsummary.Rows.Add(slno, Patient, invoiceno, " ", details, tax, doctor, "", "", "", String.Format("{0:C}", credit),"0.00", String.Format("{0:C}", amdue));//totalcredit
            ...
            try
            {
                ...
                else
                {
                    ...
                    for (int z = 0; z < invMain.Rows.Count; z++)
                    {
                        ...
                        totalcredit = totalcredit + credit;
                        totalinvoice = totalinvoice + credit;
                        Grvsummary.Rows.Add(slno, Patient, invoiceno, " ", details, tax, doctor, "", "", "", String.Format("{0:C}", credit), "0.00",  String.Format("{0:C}", credit));//totalcredit
                        Grvsummary.Rows[z].Cells[1].Style.ForeColor = Color.Blue;
                        Grvsummary.Rows[z].Cells[10].Style.ForeColor = Color.Blue;
                        Grvsummary.Rows[z].Cells[12].Style.ForeColor = Color.Red;
                        Grvsummary.Rows[z].Cells[12].Style.Font = new System.Drawing.Font("Segoe UI", 10, FontStyle.Regular);
                        slno = slno + 1;
                        invcount = invMain.Rows.Count;
                    }
                }
        ...
        public void getpay(DataTable payMain)
        {
            try
            {
                ...
                for (int u = 0; u < payMain.Rows.Count; u++)
                {
                    ...
                    due = due + balance;
                    string tax_inrs = "";// payMain.Rows[u]["tax_inrs"].ToString();
                    Grvsummary.Rows.Add(slno, Patient, invoiceno, recpno, details, tax_inrs, doctor, mode, "", "", "0.00",  Convert.ToDecimal(total).ToString("#0.00"), Convert.ToDecimal(balance).ToString("#0.00"));
                    sl = slno;
                    if (Grvsummary.Rows[invcount + u].Cells["modeofpayment"].Value != null && Grvsummary.Rows[invcount + u].Cells["modeofpayment"].Value.ToString() != "")
                    {
                        ...
                        else if (Grvsummary.Rows[invcount + u].Cells["modeofpayment"].Value.ToString().Trim() == "PayPal")
                        {
                            paypal = paypal + Convert.ToDecimal(total);
                        }
                    }
                    Grvsummary.Rows[invcount + u].Cells[10].Style.ForeColor = Color.Red;
                    Grvsummary.Rows[invcount + u].Cells[11].Style.ForeColor = Color.ForestGreen;
                    Grvsummary.Rows[invcount + u].Cells[11].Style.Font = new System.Drawing.Font("Segoe UI", 10, FontStyle.Regular);
                    Grvsummary.Rows[invcount + u].Cells[1].Style.ForeColor = Color.ForestGreen;
                    slno = slno + 1;
                }
                ////advance and return
                ///
                int ad_row = Grvsummary.Rows.Count;
                int sl_no = 0;
                sl_no = 1 + sl;
                decimal advance = 0, adv_returned = 0, total_advan = 0, total_returned = 0;
                Grvsummary.Rows.Add("", "Advance", "", "", "", "", "", "", "", "", "", "", "");//done
                Grvsummary.Rows[ad_row].Cells[1].Style.ForeColor = Color.Blue;
                Grvsummary.Rows[ad_row].Cells[1].Style.Font = new System.Drawing.Font("Segoe UI", 14, FontStyle.Bold);
                DataTable dt_advance = this.ctrlr.dt_advance(dateTimePickerdaily1.Value.ToString("yyyy-MM-dd"));

                if (dt_advance.Rows.Count > 0)
                {
                    for (int k = 0; k < dt_advance.Rows.Count; k++)
                    {
                        if (dt_advance.Rows[k]["Credit_Debit"].ToString() == "Debit")
                        {
                            advance = Convert.ToDecimal(dt_advance.Rows[k]["Amount"].ToString());
                            adv_returned = 0;
                            total_advan = total_advan + advance;
                        }
                        else
                        {
                            adv_returned = Convert.ToDecimal(dt_advance.Rows[k]["Amount"].ToString());
                            advance = 0;
                            total_returned = total_returned + adv_returned;
                        }
                        Grvsummary.Rows.Add(sl_no, dt_advance.Rows[k]["pt_name"].ToString(), "", "", "", "", dt_advance.Rows[k]["PaymentMethod"].ToString(), advance, adv_returned, "", "", "", "");//done
                ...
                Grvsummary.Rows.Add("", "", "", "", "", "", "", "", "", "", "", "", "");
                rowcount = Grvsummary.Rows.Count;
                Grvsummary.Rows[rowcount - 1].Cells["TOTAL_COST"].Style.ForeColor = Color.Blue;
                Grvsummary.Rows[rowcount - 1].Cells["TOTAL_COST"].Style.Font = new System.Drawing.Font("Segoe UI", 14, FontStyle.Bold);
                ...
                if (paypal > 0)
                {
                    Grvsummary.Rows.Add("", "", "", "", "", "", "", "", "", "", "PayPal", Convert.ToDecimal(paypal).ToString("#0.00"), "");
                    rowcount = Grvsummary.Rows.Count;
                    Grvsummary.Rows[rowcount - 1].Cells["TOTAL_COST"].Style.ForeColor = Color.DarkSlateGray;
                    Grvsummary.Rows[rowcount - 1].Cells["TOTAL_COST"].Style.Font = new System.Drawing.Font("Segoe UI", 10, FontStyle.Bold);
                }
                if (total_advan > 0)
                {
                    Grvsummary.Rows.Add("", "", "", "", "", "", "Total Advance", Convert.ToDecimal(total_advan).ToString("#0.00"), "", "", "", "", "");
                    rowcount = Grvsummary.Rows.Count;
                    Grvsummary.Rows[rowcount - 1].Cells["TOTAL_COST"].Style.ForeColor = Color.DarkSlateGray;
                    Grvsummary.Rows[rowcount - 1].Cells["TOTAL_COST"].Style.Font = new System.Drawing.Font("Segoe UI", 10, FontStyle.Bold);
                }
                if (total_returned > 0)
                {
                    Grvsummary.Rows.Add("", "", "", "", "", "", "Total Returned", Convert.ToDecimal(total_returned).ToString("#0.00"), "", "", "", "", "");
                    rowcount = Grvsummary.Rows.Count;
                    Grvsummary.Rows[rowcount - 1].Cells["TOTAL_COST"].Style.ForeColor = Color.DarkSlateGray;
                    Grvsummary.Rows[rowcount - 1].Cells["TOTAL_COST"].Style.Font = new System.Drawing.Font("Segoe UI", 10, FontStyle.Bold);
                }
        ...
        public void getpay2(DataTable payMain)
        {
            try
            {
                decimal cash = 0, cheque = 0, card = 0, dd = 0, PayTM = 0, Tez = 0, paypal = 0, UPI = 0, NEFT = 0, Netbanking = 0, Wallets = 0, CCAvenue = 0; ;
                for (int u = 0; u < payMain.Rows.Count; u++)
                {
                    string Patient = payMain.Rows[u]["pt_name"].ToString();
                    string Patient_id = payMain.Rows[u]["pt_id"].ToString();
                    ...
                    if (Grvsummary.Rows[invcount + u].Cells["modeofpayment"].Value != null && Grvsummary.Rows[invcount + u].Cells["modeofpayment"].Value.ToString() != "")
                    {
                        ...
                        else if (Grvsummary.Rows[invcount + u].Cells["modeofpayment"].Value.ToString().Trim().Trim() == "PayPal")
                        {
                            paypal = paypal + Convert.ToDecimal(total);
                        }
                    }
                    ...
                    Grvsummary.Rows[invcount + u].Cells[10].Style.Font = new System.Drawing.Font("Segoe UI", 10, FontStyle.Regular);
                    Grvsummary.Rows[invcount + u].Cells[1].Style.ForeColor = Color.ForestGreen;
                    slno = slno + 1;
                }
                ////advance and return
                ...
                int ad_row = Grvsummary.Rows.Count;
                int sl = 1;
                //             sl = sl + Convert.ToInt32(Grvsummary.Rows[ad_row - 1].Cells["SL_NO"].ToString());
                decimal advance = 0, adv_returned = 0, total_advan = 0, total_returned = 0;
                Grvsummary.Rows.Add("", "Advance", "", "", "", "", "", "", "", "", "", "", "");//done
                Grvsummary.Rows[ad_row].Cells[1].Style.ForeColor = Color.Blue;
                Grvsummary.Rows[ad_row].Cells[1].Style.Font = new System.Drawing.Font("Segoe UI", 18, FontStyle.Bold);
                DataTable dt_advance = this.ctrlr.dt_advance(dateTimePickerdaily1.Value.ToString("yyyy-MM-dd"));
                if (dt_advance.Rows.Count > 0)
                {
                    for (int k = 0; k < dt_advance.Rows.Count; k++)
                    {
                        ...
                        Grvsummary.Rows.Add(sl, dt_advance.Rows[k]["pt_name"].ToString(), "", "", "", "", dt_advance.Rows[k]["PaymentMethod"].ToString(), advance, adv_returned, "", "", "", "");//done
        ...
        private void combodoctors_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                Grvsummary.Columns[9].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleRight;
                Grvsummary.Columns[10].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleRight;
                Grvsummary.Columns[8].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleRight;
                Grvsummary.Columns[11].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleRight;
                Grvsummary.Columns[12].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleRight;
                Grvsummary.Columns[9].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                Grvsummary.Columns[10].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                Grvsummary.Columns[8].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                Grvsummary.Columns[11].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                Grvsummary.Columns[12].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                string date = Convert.ToDateTime(dateTimePickerdaily1.Value).ToString("yyyy-MM-dd");
                //string date1 = Convert.ToDateTime(dateTimePickerdaily1.Value).ToString("yyyy-MM-dd");
                ...
                if (select_dr_id == "0")
                {
                    DataTable inv = this.ctrlr.getinvoice(date);
                    getinvoice(inv);
                    DataTable pay = this.ctrlr.getpay(date);
                    //getpay(pay);
                }
                else
                {
                    DataTable inv2 = this.ctrlr.getinvoice2(date, select_dr_id);
                    getinvoice2(inv2);
                    DataTable pay2 = this.ctrlr.getpay2(date, select_dr_id);
                    //getpay2(pay2);
                }
                ...
                else
                {
                    ...
                    lbltotal3.Text = Convert.ToDecimal(due ).ToString("#0.00");//balance
                }
                totalpayment = 0;
                totalcredit = 0;
                balance = 0; due = 0;
        ...
        private void btnprint_Click(object sender, EventArgs e)
        {
            try
            {
                if (Grvsummary.Rows.Count > 0)
                {
                    ...
                    {
                        sWrite.WriteLine("<tr>");
                        sWrite.WriteLine("<th colspan=13><center><b><FONT COLOR=black FACE='Segoe UI'  SIZE=3>DAILY INVOICE REPORT (All Doctor) </font></center></b></td>");
                        sWrite.WriteLine("</tr>");
                    }
                    if (combodoctors.SelectedIndex != 0)
                    {
                        sWrite.WriteLine("<tr>");
                        sWrite.WriteLine("<th colspan=13><center><b><FONT COLOR=black FACE='Segoe UI'  SIZE=3> DAILY INVOICE REPORT (" + combodoctors.Text + ")  </font></center></b></td>");
                        sWrite.WriteLine("</tr>");
                    }
                    sWrite.WriteLine("<br>");
                    ...
                    if (Grvsummary.Rows.Count > 0)
                    {
                        ...
                        sWrite.WriteLine("    <td align='left' width='9%' style='border:1px solid #000;background:#999999'><FONT COLOR=black FACE='Segoe UI' SIZE=3><b>Tax No</b></font></td>");
                        ...
                        sWrite.WriteLine("    <td align='right' width='10%' style='border:1px solid #000;background:#999999'><FONT COLOR=black FACE='Segoe UI' SIZE=3><b>Advance</b></font></td>");
                        sWrite.WriteLine("    <td align='right' width='15%' style='border:1px solid #000;background:#999999'><FONT COLOR=black FACE='Segoe UI' SIZE=3><b>Advance Returned</b></font></td>");
                        ...
                        while (c < Grvsummary.Rows.Count)
                        {
                            ...
                            sWrite.WriteLine("    <td align='left' style='border:1px solid #000' ><FONT COLOR=black FACE='Segoe UI' SIZE=2>&nbsp;" + Grvsummary.Rows[c].Cells["tax"].Value.ToString() + "</font></td>");
                            ...
                            sWrite.WriteLine("    <td align='right' style='border:1px solid #000' ><FONT COLOR=black FACE='Segoe UI' SIZE=2>" + Grvsummary.Rows[c].Cells["advance"].Value.ToString() + "&nbsp;</font></td>");
                            sWrite.WriteLine("    <td align='right' style='border:1px solid #000' ><FONT COLOR=black FACE='Segoe UI' SIZE=2>" + Grvsummary.Rows[c].Cells["returned"].Value.ToString() + "&nbsp;</font></td>");
