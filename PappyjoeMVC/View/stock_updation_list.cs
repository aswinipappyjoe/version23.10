...
{
    public partial class stock_updation_list : Form
    {
        stock_updation_controller cntrl = new stock_updation_controller();
        public stock_updation_list()
        {
            InitializeComponent();
        }

        private void stock_updation_list_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //{
                    dt = this.cntrl.get_stockupdate_details(Convert.ToDateTime(DTP_From.Value).ToString("yyyy-MM-dd"), Convert.ToDateTime(DTP_To.Value).ToString("yyyy-MM-dd"));
                //}
        ...
        public void Fill_dgvSale(DataTable dtb)
        {
            ...
            if (dtb.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dtb.Rows.Count; i++)// RefNo,stock_date,LabName,Action,TotalAmount
                {
                    ...
                    dgv_sales.Rows[i].Cells["date"].Value = Convert.ToDateTime(dtb.Rows[i]["updation_date"].ToString()).ToString("MM/dd/yyyy");
                    dgv_sales.Rows[i].Cells["labname"].Value = dtb.Rows[i]["Action"].ToString();
                    dgv_sales.Rows[i].Cells["cost"].Value = Convert.ToInt32(dtb.Rows[i]["TotalAmount"].ToString()).ToString("##.00");
                    num = num + 1;
                }
            }
            else
