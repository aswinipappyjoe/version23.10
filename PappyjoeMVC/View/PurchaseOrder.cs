...
namespace PappyjoeMVC.View
{
    ...
    public partial class PurchaseOrder : Form
    {
        ...
        int Rowindex = 0; //flag1
        public int flag1 = 0;
        public PurchaseOrder()  //dts
        {
            InitializeComponent();
        }
            ...
            try
            {
                if (flag1 == 1)//start sanoop
                {
                    ...
                    if (dts.Rows.Count > 0)
                    {
                        ...
                        txt_SupplierId.Text = supplierid.ToString();
                        for (int i = 0; i < dts.Rows.Count; i++)
                        {
                            string ar = dts.Rows[i]["id"].ToString();
                            DataTable dt = this.cntrl.getrate(ar);
                            dgvItemData.Rows.Add();
                            dgvItemData.Rows[i].Cells["Id"].Value = dts.Rows[i]["id"].ToString();
                            dgvItemData.Rows[i].Cells["Item_Id"].Value = dts.Rows[i]["item_code"].ToString();
                            dgvItemData.Rows[i].Cells["col_qty"].Value = "0";
                            dgvItemData.Rows[i].Cells["description"].Value = dts.Rows[i]["item_name"].ToString();
                            dgvItemData.Rows[i].Cells["Unit_Cost"].Value = dt.Rows[0]["Purch_Rate"].ToString();
                            dgvItemData.Rows[i].Cells["Amount"].Value ="0";
                        }
                    }
                }
            }
        ...
        public void Load_item_details(DataTable Purchase)
        {
            ...
            txtTotalAmount.Text = (Convert.ToDecimal(totalamt).ToString());
            //totalamt.ToString();
                ...
                try
                {
                    if (BtnSave.Text == "SAVE")
                    {
                        if (txt_SupplierId.Text != "")
                        {
                            if (dgvItemData.Rows.Count > 0)
                            {
                                this.cntrl.save_master(txtPurchOrderNo.Text, dtpPurchDate.Value.ToString("yyyy-MM-dd"), txt_SupplierId.Text, con, trans);
                                foreach (DataGridViewRow row in dgvItemData.Rows)
                                {
                                    
                                    this.cntrl.save_items(txtPurchOrderNo.Text, row.Cells["id"].Value.ToString(), row.Cells["description"].Value.ToString(), row.Cells["col_qty"].Value.ToString(), row.Cells["Unit_Cost"].Value.ToString(), row.Cells["Amount"].Value.ToString(), con, trans);
                                }
                    ...
                    if (BtnSave.Text == "UPDATE")
                    {
                        if (txt_SupplierId.Text != "")
                        {
                            if (dgvItemData.Rows.Count > 0)
                            {
                                this.cntrl.delete_order(txtPurchOrderNo.Text, con, trans);
                                this.cntrl.save_master(txtPurchOrderNo.Text, dtpPurchDate.Value.ToString("yyyy-MM-dd"), txt_SupplierId.Text, con, trans);
                                foreach (DataGridViewRow row in dgvItemData.Rows)
                                {
                                    this.cntrl.save_items(txtPurchOrderNo.Text, row.Cells["id"].Value.ToString(), row.Cells["description"].Value.ToString(), row.Cells["col_qty"].Value.ToString(), row.Cells["Unit_Cost"].Value.ToString(), row.Cells["Amount"].Value.ToString(), con, trans);
                    ...
                    if (BtnSave.Text == "SAVE")
                    {
                        if (txt_SupplierId.Text != "")
                        {
                            if (dgvItemData.Rows.Count > 0)
                            {
                                ...
                                this.cntrl.save_log(doctor_id, "Purchase Order", " Add Purchase Order", dt, Convert.ToString(Timeonly.ToString("hh:mm tt")), "Add", txtPurchOrderNo.Text);
                                clear();
                    ...
                    if (BtnSave.Text == "UPDATE")
                    {
                        if (txt_SupplierId.Text != "")
                        {
                            if (dgvItemData.Rows.Count > 0)
                            {
                                string dt = DateTime.Now.Date.ToString("yyyy-MM-dd");
                                DateTime Timeonly = DateTime.Now;
                                this.cntrl.save_log(doctor_id, "Purchase Order", " Edit Purchase Order", dt, Convert.ToString(Timeonly.ToString("hh:mm tt")), "Edit", txtPurchOrderNo.Text);
                                clear();
        ...
        private void btn_clear_Click(object sender, EventArgs e)
        {
            ...
            txtAmount.Text = "0.00";
            txtTotalAmount.Text = "0.00";
            txt_SupplierId.Text = ""; txtTotal_item.Text = "0";
            txtSupplierName.Text = "";
            dgvItemData.Rows.Clear();
        }
        private void print()
        {
            if (dgvItemData.Rows.Count > 0)
            {
                ...
                StreamWriter sWrite = new StreamWriter(Apppath + "\\PurcahseOrder_print.html");
                ...
                sWrite.WriteLine("</html>");
                sWrite.Close();
                System.Diagnostics.Process.Start(Apppath + "\\PurcahseOrder_print.html");
            }
        }
        ...
        private void lstbox_Supplier_SelectedIndexChanged(object sender, EventArgs e)
        {
