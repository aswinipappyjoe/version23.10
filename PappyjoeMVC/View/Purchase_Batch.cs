...
namespace PappyjoeMVC.View
{
    public partial class Purchase_Batch : Form
    {
        ...
        public string unit1;
        public int qtyE;public decimal batchrate = 0;
        int Count;
        string itemid1;
        ...
        int flag = 0;
        string Itemcode = "", purNo = ""; public string batchentry = "";
        ...
        {
            InitializeComponent();//from purchase order
            unit1 = text1;
            qtyE = qty;
            Itemcode = text2;
            itemid1 = item_id;
        ...
        public Purchase_Batch(string text1, int qty, string text2, string item_id, string text, string text3) 
        {
            InitializeComponent();
            unit1 = text1;
            qtyE = qty;
            ...
            itemid1 = item_id;
            purNo = text;
            batchrate =Convert.ToDecimal( text3);
            Addflag = true;
            this.text3 = text3;
        }
        ...
        private void frmpurchaseBatch_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                cmb_Period.Name = "prd";
                dgvPurchaseBatch.Columns.Insert(7, cmb_Period);
                ...
                if (loadflag==true)
                {
                    ...
                    if (Addflag == true)
                    {
                        ...
                        dgvPurchaseBatch.Rows[0].Cells["rate"].Value = batchrate;
                    ...
                    else if (editflag == true)
                    {
                        ...
                        for (int i = 0; i < editgrid1.Rows.Count; i++)
                        {
                            ...
                            dgvPurchaseBatch.Rows[i].Cells["Quantity"].Value = qtyE;// editgrid1.Rows[i]["col_temp_qty"].ToString();//qtyE.ToString();
                            ...
                            if (editgrid1.Rows[i]["batchentry"].ToString() != "")
                            {
                                dgvPurchaseBatch.Rows[i].Cells["entryno"].Value = editgrid1.Rows[i]["batchentry"].ToString();
                            }
                            else
                            {
                                dgvPurchaseBatch.Rows[i].Cells["entryno"].Value = "0";
            ...
            if (e.RowIndex >= 0)
            {
                ...
                if (dgvPurchaseBatch.CurrentCell.OwningColumn.Name == "Branch_No")
                {
                    if(dgvPurchaseBatch.Rows[dgvPurchaseBatch.CurrentRow.Index].Cells["Branch_No"].Value !=null&& dgvPurchaseBatch.Rows[dgvPurchaseBatch.CurrentRow.Index].Cells["Branch_No"].Value.ToString()!="")
                    {
                        ...
                        DataTable dt_batch = this.cntrl.get_batchrate_exp(itemid1, batch);
                        if (dt_batch.Rows.Count > 0)//
                        {
                            unit = dgvPurchaseBatch.Rows[dgvPurchaseBatch.CurrentRow.Index].Cells["col_Unit"].Value.ToString();
                            DataTable dtb_item = this.cntrl.itemdetails(itemid1);
                            DataTable dtb = this.cntrl.batchdetails(batch, itemid1);
        ...
        private void dgvPurchaseBatch_CellLeave(object sender, DataGridViewCellEventArgs e)
        {
            //if(e.RowIndex>=0)
            //{
            //    if(dgvPurchaseBatch.CurrentCell.OwningColumn.Name== "Branch_No")
            ...
            //        //if(dt_batch.Rows.Count>0)
            //        //{
            //        //    dgvPurchaseBatch.CurrentRow.Cells["rate"].Value = dt_batch.Rows[0]["batch_rate"].ToString();
            //        //    dgvPurchaseBatch.CurrentRow.Cells["Prd_Date"].Value = dt_batch.Rows[0]["PrdDate"].ToString();
            //        //    dgvPurchaseBatch.CurrentRow.Cells["Exp_Date"].Value = dt_batch.Rows[0]["ExpDate"].ToString();
            //        //}
            //    }
        ...
        private void Save()
        {
            try
            {
                ...
                for (int i = 0; i < Count; i++)
                {
                    ...
                    if (dgvPurchaseBatch.Rows[i].Cells["Prd_Date"].Value == null || dgvPurchaseBatch.Rows[i].Cells["Prd_Date"].Value =="")
                    {
                        flagdateError = true;
                    }
                    else
                ...
                if (editflag == true)
                {
                    if (flagbatchError != true && flagdateError != true)
                    {
                        ...
                        if (flag == 1)
                        {
                            ...
                            {
                                var frm = new Purchase(gridData, total_rate);
                ...
                else if (editflag == false)
                {
                    if (flagbatchError != true && flagdateError != true)
                    {
                        ...
                        else if (flag == 0)
                        {
                            if (ermsg == true)
                            {
                                MessageBox.Show("Quantity Mismatched!.", "Invalid", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            }
                            ermsg = false;
                        }
                    }
