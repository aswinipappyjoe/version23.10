...
namespace PappyjoeMVC.View
{
    public partial class Patients : Form
    {
        ...
        user_privillage_model privi = new user_privillage_model();
        Common_model mdl = new Common_model();
        Connection db = new Connection();
        Contacts ad = new Contacts(); Patients_model pmdl = new Patients_model();
        ...
        bool AllPatient_Flag = false, Recently_Visited_Flag = false, RecentalyAdded_flag = false, upcommingAppoinments_Flag = false, Upcomingbirthday_flag = false, CancelledAppoinmnt_Flag = false,ipPatient_Flag=false,dischargePatient_Flag=false;
        ...
        private void labelappointment_Click(object sender, EventArgs e)
        {
            ...
            {
                b_normal();
                panl_lab_notification.Visible = false;
        ...
        public void Appointment_Data(DataTable dt_pt5)
        {
            if (dt_pt5.Rows.Count > 0)
            {
                ...
                {
                    string reason = "";
                    if (dt_pt5.Rows[j]["status"].ToString() == "CANCELLED")
                    {
                        reason = "Reason = " + "(" + dt_pt5.Rows[j]["cancelation_reason"].ToString() + ")";
                    }
                    else
                        reason = "";
        ...
        public void b_normal()
        {
            ...
            lab_nurse_notification.ForeColor = Color.DimGray;
            lab_nurse_notification.BackColor = Color.White;
            lab_notification.ForeColor = Color.DimGray;
            lab_notification.BackColor = Color.White;
        }
            ...
            try
            {
                ...
                if (dt_cf_main.Rows.Count > 0)
                {
                    ...
                    for (int j = 0; j < dt_cf_main.Rows.Count; j++)
                    {
                        ...
                        DGV_Patients.Rows.Add(dt_cf_main.Rows[j]["pt_id"].ToString(), String.Format("{0:dddd, MMMM d, yyyy}"  , Convert.ToDateTime(dt_cf_main.Rows[j]["date"].ToString()) )  +   dt_cf_main.Rows[j]["time"].ToString(), "","", "");
                       ...
                       // DGV_Patients.Rows[i].Cells[2].Style.ForeColor = Color.DarkGreen;//bhj
                        DGV_Patients.Rows[i].Cells[2].Style.BackColor = Color.Gainsboro;//Gainsboro
            ...
            try
            {
                ...
                if (dt_invoice_main.Rows.Count > 0)
                {
                    ...
                    for (int j = 0; j < dt_invoice_main.Rows.Count; j++)
                    {
                        ...
                        if (pat.Rows.Count > 0)
                        {
                            patientName = pat.Rows[0][1].ToString();
                            Pt_id = pat.Rows[0][0].ToString();
                        }
        ...
        public void Payments(DataTable Payment)
        {
            try
            {
                ...
                for (int i = 0; i < Payment.Rows.Count; i++)
                {
                    ...
                    {
                        patientName = pat.Rows[0][1].ToString();
                        patientId = pat.Rows[0][0].ToString();
                    }
        ...
        private void btn_AllPatient_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Inactive_Flag = false; 
                txt_Search.Text = "Search Patient Name,Patient Id, Mobile No";
                ...
                if (doctor_id == "1")
                {
                    ...
                    DataTable drtb = this.cntrl.Get_all_Patients(); search_grid_load(drtb);
                    Design_Datagrid();
                }
                else
                {
                    ...
                    //txt_Search.Text = "Search Patient Id, Patient Name, Mobile No, Address";
                    SetPatient_SearchControlls();
                    DataTable dtb = this.cntrl.Get_all_Patients(); search_grid_load(dtb);
                    //griddata(dtb);
                    Design_Datagrid();
                }
        ...
        private void btnPaymentDue_Click(object sender, EventArgs e)
        {
            ...
            {
                panl_lab_notification.Visible = false;
                var form2 = new PappyjoeMVC.View.Inactive_patients();
        ...
        private void grgroup_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                if (grgroup.Rows.Count > 0)
                {
                    ...
                    CancelledAppoinmnt_Flag = false;
                    Group_flag = true;
                    txt_Search.Text = "Search Patient Name,Patient Id,Mobile";
                    ClearAll_grid2_Properties();
                    id4 = "";
                    lab_7.Visible = true;
                    lab_7.Text = "";
        ...
        private void addnewpatient_Click(object sender, EventArgs e)
        {
            ...
            {
                string id;
                id = privi.add_patients(doctor_id);// mdl.doctr_privillage_for_addnewPatient(doctor_id);
                if (int.Parse(id) > 0)
                {
        ...
        private void DGV_Patients_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (DGV_Patients.RowCount > 0)
            {
                if (left_button_click == 1)
                {
                    ...
                    {
                        string id;
                        id = privi.privilege_D(doctor_id);
                        if (int.Parse(id) > 0)
                        {
                ...
                else if (left_button_click == 6)
                {
                    if (e.ColumnIndex == 6 && e.RowIndex >= 0)
                    {
                        ...
                        if (p != null)
                        {
                            Patient_Profile_Details myForm = new Patient_Profile_Details();
                            int r = e.RowIndex;
                            id1 = DGV_Patients.Rows[e.RowIndex].Cells[0].Value.ToString();
                            myForm.FormBorderStyle = FormBorderStyle.None;
                            myForm.TopLevel = false;
                            ...
                            myForm.patient_id = id1.ToString();
                            myForm.Show();
                            this.Close();
                        }
                    }
        ...
        private void txt_Search_TextChanged(object sender, EventArgs e)
        {
            ...
            DateTime startDateTime = Convert.ToDateTime(DateTime.Today.ToString("d") + " " + DateTime.Now.ToString("hh:mm:ss tt"));
            ClearAll_grid2_Properties();
            if(txt_Search.Text == "Search Patient Name,Patient Id,Mobile")
            {
                return;
            }
        ...
        {
            int rowcount = DGV_Patients.Rows.Count;
            int count = rowcount + 100;
            DataTable dt = this.cntrl.patient_showmore(count);
            griddata(dt);
            Design_Datagrid();
        }
        ...
        private void txt_Search_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                btn_patientsearch_Click(this, new EventArgs());
            }
        }

        private void button1_Click(object sender, EventArgs e)
        {
            var form2 = new Rooms_details();
            form2.ShowDialog();
        }
        ...
        private void lab_nurse_notification_Click(object sender, EventArgs e)
        {
            b_normal();
            panl_lab_notification.Visible = false;
            lab_nurse_notification.BackColor = Color.DodgerBlue;
            lab_nurse_notification.ForeColor = Color.White;
            var form2 = new Nurse_notification();
            form2.ShowDialog();
            labelprofile.BackColor = Color.DodgerBlue;
            labelprofile.ForeColor = Color.White;
            lab_nurse_notification.ForeColor = Color.DimGray;
        ...
        private void lab_notification_Click(object sender, EventArgs e)
        {
            b_normal();
            lab_notification.BackColor = Color.DodgerBlue;
            lab_notification.ForeColor = Color.White;
            panl_lab_notification.Visible = true;
            load_grid(DateTime.Now.Date.ToString("yyyy-MM-dd"));
        }
        public void load_grid(string date)
        {
            DataTable dt_getinvoice = this.lab_cntrl.get_invoiceid(date);
            DGV_notify.Rows.Clear();
            if (dt_getinvoice.Rows.Count > 0)
            {
                int k = 1; int row = 0;
                for (int i = 0; i < dt_getinvoice.Rows.Count; i++)
                {
                    DataTable dt_invoice = this.lab_cntrl.get_patient_invoice_notification(dt_getinvoice.Rows[i]["pt_id"].ToString(), dt_getinvoice.Rows[i]["id"].ToString());
                    if (dt_invoice.Rows.Count > 0)
                    {
                        DataTable dt_doctor = this.lab_cntrl.docname(dt_invoice.Rows[0]["dr_id"].ToString());
                        DGV_notify.Rows.Add();
                        DGV_notify.Rows[row].Cells["slno"].Value = k;
                        DGV_notify.Rows[row].Cells["ptid"].Value = dt_getinvoice.Rows[i]["patientid"].ToString();
                        DGV_notify.Rows[row].Cells["name"].Value = dt_getinvoice.Rows[i]["pt_name"].ToString();
                        DGV_notify.Rows[row].Cells["date"].Value = Convert.ToDateTime(dt_getinvoice.Rows[i]["date"].ToString()).ToString("dd-MM-yyyy");
                        DGV_notify.Rows[row].Cells["phone"].Value = dt_getinvoice.Rows[i]["primary_mobile_number"].ToString();
                        DataTable dt_status = this.lab_cntrl.get_status(dt_getinvoice.Rows[i]["pt_id"].ToString(), dt_invoice.Rows[0]["dr_id"].ToString(), date);
                        if (dt_status.Rows.Count > 0)
                        {
                            if (dt_status.Rows[0][0].ToString() == "Completed")
                            {
                                DGV_notify.Rows[row].Cells["status"].Value = "Completed";

                            }
                            else
                                DGV_notify.Rows[row].Cells["status"].Value = "Pending";
                        }
                        else
                            DGV_notify.Rows[row].Cells["status"].Value = "Pending";
                        DGV_notify.Rows[row].Cells["pt_id"].Value = dt_getinvoice.Rows[i]["pt_id"].ToString();
                        DGV_notify.Rows[row].Cells["plan_id"].Value = "";//  dt_invoice.Rows[i]["work_id"].ToString();
                        DGV_notify.Rows[row].Cells["dr_name"].Value = dt_doctor.Rows[0]["doctor_name"].ToString();
                        //if (dt_getinvoice.Rows.Count > 0)
                        //{
                        if (dt_getinvoice.Rows[row]["status"].ToString() == "0")
                        {
        ...
        {
            panl_visa_expiry.Visible = true;
            DataTable dt_patient = this.pmdl.visa_details();
            if (dt_patient.Rows.Count > 0)
            {
                dgv_expiry.Rows.Clear(); int i = 0;
                foreach (DataRow dr in dt_patient.Rows)//
                {
                    int diff = 0;
                    DateTime expiry = Convert.ToDateTime(dr["latest_visa_expiry_date"].ToString());
                    DateTime to_date = DateTime.Now.Date;
                    double Days = (expiry - to_date).TotalDays;
                    diff = Convert.ToInt32(Days);
                    dgv_expiry.Rows.Add();
                    dgv_expiry.Rows[i].Cells["id"].Value = dr["id"].ToString();
                    dgv_expiry.Rows[i].Cells["patid"].Value = dr["pt_id"].ToString();
                    dgv_expiry.Rows[i].Cells["visa_name"].Value = dr["pt_name"].ToString();
                    dgv_expiry.Rows[i].Cells["mobile"].Value = dr["primary_mobile_number"].ToString();
                    dgv_expiry.Rows[i].Cells["expirydate"].Value = dr["latest_visa_expiry_date"].ToString();
                    dgv_expiry.Rows[i].Cells["days"].Value = diff;
                    i++;
                }
            }
        }

        private void dgv_expiry_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                string pat_id = dgv_expiry.CurrentRow.Cells["id"].Value.ToString();
                if (pat_id != "")
                {
                    Patient_Profile_Details form = new Patient_Profile_Details();
                    form.doctor_id = doctor_id;
                    form.patient_id = pat_id;
                    //this.Close();
                    //form.ShowDialog();
                    openform(form);
                }
        ...
        {
            panl_visa_expiry.Visible = false;
        }
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                if (doctor_id == "1")
                {
                    ...
                    ////txt_Search.Text = "Search Patient Id, Patient Name, Mobile No, Address";
                    //SetPatient_SearchControlls();
                    DataTable drtb = this.cntrl.Get_all_ipPatients();
                    search_grid_load(drtb);
                    Design_Datagrid();
                }
                else
                {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (doctor_id == "1")
                {
                    ...
                    ////txt_Search.Text = "Search Patient Id, Patient Name, Mobile No, Address";
                    //SetPatient_SearchControlls();
                    DataTable drtb = this.cntrl.Get_all_dischargePatients();
                    search_grid_load(drtb);
                    Design_Datagrid();
                }
                else
        ...
        private void patients_Load(object sender, EventArgs e)
        {
            ...
            DataTable dt_count = this.cntrl.Get_all_Patients_count();
            search_grid_load(dtb);
            Design_Datagrid();
            //-----Group Add------
            ...
            if (PappyjoeMVC.Model.Connection.MyGlobals.project_type == "Pharmacy")
            {
                ...
                lbl_imprt_patients.Visible = false;
                lab_nurse_notification.Visible = false;
                lab_notification.Visible = false;
                lb_nurse_notify_cnt.Visible = false;
                lbl_lab_notify_count.Visible = false;
                btn_Upcomming.Visible = false;
                btn_Birthday.Visible = false;
                bt_cancelled_appointment.Visible = false;
                btnPaymentDue.Visible = false;

                labellog.Visible = true;
                labellog.Location = new Point(17, 65);
            }
            else
            {
                DataTable dtn = this.cntrl.get_nnotify_count();
                DataTable dtl = this.cntrl.get_lab_notify();
                nncount(dtn);
                lab_notify(dtl);
                if (doctor_id == "1")
                {
        ...
        // bhj..........nurse notification count
        public void nncount(DataTable dtn)
        {
            if (dtn.Rows.Count > 0)
            {
                lb_nurse_notify_cnt.Text = dtn.Rows.Count.ToString();
            }
        }

        //bhj.............lab notification
        public void lab_notify(DataTable dtl)
        {
            if (dtl.Rows.Count > 0)
            {
                lbl_lab_notify_count.Text = dtl.Rows.Count.ToString();
            }
        }
        ...
        public void Create_Datagrid(DataTable dtb1)
        {
            if (dtb1.Columns.Count > 0)
            {
                if (dtb1.Rows.Count > 0)
                {
                    lab_7.Location = new Point(73, 7);
                    lab_7.Text = dtb1.Rows.Count.ToString() + "Patient(s)";
                }
                else
                {
                    lab_LongMsg.Show();
                ...
                DGV_Patients.Columns.Add("id", "Id");
                DGV_Patients.Columns.Add("pt_id", "Id");
                DGV_Patients.Columns.Add("name", "Patient Name");
                DGV_Patients.Columns.Add("gender", "Gender");
                DGV_Patients.Columns.Add("age", "Age");
                DGV_Patients.Columns.Add("mobile", "Mobile");
                DGV_Patients.Columns.Add("address", "Street Address");
                DGV_Patients.Columns.Add("locality", "Locality");
                DGV_Patients.Columns.Add("visited", "Visited");
                DGV_Patients.Columns.Add("file", "File");
                //DGV_Patients.Columns.Add("Room No", "Room_No");
                if (DGV_Patients.Columns.Count > 0)
                {
                    for (int j = 0; j < dtb1.Rows.Count; j++)
                    {
                        DGV_Patients.Rows.Add();
                        DGV_Patients.Rows[j].Cells["id"].Value = dtb1.Rows[j]["Pid"].ToString();
                        DGV_Patients.Rows[j].Cells["pt_id"].Value = dtb1.Rows[j]["Id"].ToString();
                        DGV_Patients.Rows[j].Cells["name"].Value = dtb1.Rows[j]["Patient Name"].ToString();
                        DGV_Patients.Rows[j].Cells["gender"].Value = dtb1.Rows[j]["Gender"].ToString();
                        if (dtb1.Rows[j]["Age"].ToString() != "")
                        {
                            DGV_Patients.Rows[j].Cells["age"].Value = dtb1.Rows[j]["Age"].ToString() + " " + dtb1.Rows[j]["days"].ToString();
                        }
                        else if (dtb.Rows[j]["age2"].ToString() != "")
                        {
                            DGV_Patients.Rows[j].Cells["age"].Value = dtb1.Rows[j]["age2"].ToString() + " " + dtb1.Rows[j]["days"].ToString();
                        }
                        DGV_Patients.Rows[j].Cells["mobile"].Value = dtb1.Rows[j]["Mobile"].ToString();
                        DGV_Patients.Rows[j].Cells["address"].Value = dtb1.Rows[j]["Street Address"].ToString();
                        DGV_Patients.Rows[j].Cells["locality"].Value = dtb1.Rows[j]["Locality"].ToString();
                        DGV_Patients.Rows[j].Cells["visited"].Value = dtb1.Rows[j]["Visited"].ToString();
                        DGV_Patients.Rows[j].Cells["file"].Value = dtb1.Rows[j]["File No"].ToString(); 
                        DGV_Patients.Rows[j].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft;
                    }
                    DGV_Patients.Columns[0].Visible = false;
                    ...
                    DGV_Patients.DefaultCellStyle.Font = new System.Drawing.Font("Segoe UI", 8, FontStyle.Regular);
                }
            }
            else
            {
        ...
        public void search_grid_load(DataTable dtb)
        {
            int grid_count = 0;
            if (dtb.Columns.Count > 0)
            {
                    if (dtb.Rows.Count < 250)
                    {
                        grid_count = dtb.Rows.Count;
                        lab_7.Location = new Point(73, 7);
                        lab_7.Text = dtb.Rows.Count + " Patient(s)";
                    }
                    else
                    {
                        grid_count = 250;
                ...
                DGV_Patients.Columns.Add("visited", "Visited");
                DGV_Patients.Columns.Add("file", "File");
                DGV_Patients.Columns.Add("room_no", "Room No");//bhj
                if (DGV_Patients.Columns.Count > 0)
                {
                    if (dtb.Rows.Count > 0)
                    {
                        if(dtb.Rows[0]["Profile_Status"].ToString()=="Active")
                        {
                            for (int j = 0; j < grid_count; j++)
                            {
                                DGV_Patients.Rows.Add();
                                DGV_Patients.Rows[j].Cells["id"].Value = dtb.Rows[j]["Pid"].ToString();
                                DGV_Patients.Rows[j].Cells["pt_id"].Value = dtb.Rows[j]["Id"].ToString();
                                DGV_Patients.Rows[j].Cells["name"].Value = dtb.Rows[j]["Patient Name"].ToString();
                                DGV_Patients.Rows[j].Cells["gender"].Value = dtb.Rows[j]["Gender"].ToString();
                                if (dtb.Rows[j]["Age"].ToString() != "")
                                {
                                    DGV_Patients.Rows[j].Cells["age"].Value = dtb.Rows[j]["Age"].ToString();
                                }
                                else if (dtb.Rows[j]["age2"].ToString() != "")
                                {
                                    DGV_Patients.Rows[j].Cells["age"].Value = dtb.Rows[j]["age2"].ToString() + " " + dtb.Rows[j]["days"].ToString();
                                }
                                DGV_Patients.Rows[j].Cells["mobile"].Value = dtb.Rows[j]["Mobile"].ToString();
                                DGV_Patients.Rows[j].Cells["address"].Value = dtb.Rows[j]["Street Address"].ToString();
                                DGV_Patients.Rows[j].Cells["locality"].Value = dtb.Rows[j]["Locality"].ToString();
                                DGV_Patients.Rows[j].Cells["visited"].Value = dtb.Rows[j]["Visited"].ToString();
                                DGV_Patients.Rows[j].Cells["file"].Value = dtb.Rows[j]["File No"].ToString();
                                if (dtb.Rows[j]["Room_no"].ToString() != "")//bhj
                                {
                                    DGV_Patients.Rows[j].Cells["room_no"].Value = dtb.Rows[j]["Room_no"].ToString();
                                }
                                DGV_Patients.RowsDefaultCellStyle.BackColor = Color.WhiteSmoke;
                                DGV_Patients.Rows[j].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft;
                            }
