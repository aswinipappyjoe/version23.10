    ...
    public partial class Reports : Form
    {
        ...
        private void Reports_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    lblfirstappoint.Visible = false;
                    LAB_IPPATIENT.Visible = false;
                    lb_oppatient_report.Visible = false;
                    lb_consultation.Visible = false;
                    lb_patient_wise_consultation.Visible = false;
                    //LAB_IPPATIENT.Visible = false;
                    int i = 0;
                    if (doctor_id == "1")
                    {
                    ...
                    else
                    {
                        ...
                        string id = this.cntrl.patient_combo(doctor_id);
                        if (int.Parse(id) > 0)
                        {
                            comborepcategory.Items.Insert(i++, "PATIENTS");
                        }
                ...
                {
                    lblfirstappoint.Visible = true;
                    LAB_IPPATIENT.Visible = true;
                    lb_oppatient_report.Visible = true;
                    lb_consultation.Visible = true;
                    lb_patient_wise_consultation.Visible = true;
                    if (doctor_id != "1")
                    {
                        int i = 0;
        ...
        private void comborepcategory_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                textamount.Text = todate.ToString("dd/MM/yyyy");
                if (PappyjoeMVC.Model.Connection.MyGlobals.project_type.TrimEnd() == "Pharmacy")
                {
                    ...
                    //    DataTable dt = this.cntrl.Patient_invMain(todate.ToString("yyyy-MM-dd"));
                ...
                if (comborepcategory.Text == "INVOICES")
                    {
                        ...
                        if (select_dr_id != "0")
                        {
                            DataTable dt = this.cntrl.invoice_main_doctor_wise(todate.ToString("yyyy-MM-dd"), select_dr_id);//dateTimePickerrepo2.Value.ToString("yyyy-MM-dd")
                            Load_invoiceReport(dt);

                        }
                        else
                        {
                            DataTable dt = this.cntrl.inv_main(todate.ToString("yyyy-MM-dd"));
                            Load_invoiceReport(dt);
                    ...
                    else if (comborepcategory.Text == "RECEIPT")
                    {
                        ...
                        ////combodoctors.Visible = true;
                        //to.Text = "To";
                        DataTable dt = this.cntrl.reciept(todate.ToString("yyyy-MM-dd"));
                    ...
                    else if (comborepcategory.Text == "APPOINTMENTS")
                    {
                        ...
                        //to.Text = "To";
                        btn_Show.Visible = false;
                    DateTime startdate = Convert.ToDateTime(todate.ToString("yyyy-MM-dd") + " 00:01");
                    DateTime enddate = Convert.ToDateTime(todate.ToString("yyyy-MM-dd") + " 23:59");
                    DataTable dt = this.cntrl.appointment_invMain(startdate,enddate);
                    ...
                    else if (comborepcategory.Text == "EMR")
                    {
                        ...
                        //to.Text = "To";
                        btn_Show.Visible = false;
                        DataTable dt = this.cntrl.EMR_invMain(todate.ToString("yyyy-MM-dd"));
                        Load_EMRReport(dt);
                    }
                    else if (comborepcategory.Text == "EXPENSE")
                    {
                        ...
                        if (Chk_Type.Checked)
                        {
                            ...
                            {
                                DataTable dt = this.cntrl.expence_checked(todate.ToString("yyyy-MM-dd"), "Expense");
                                Load_expenseReport(dt);
                            }
                            else if (rad_income.Checked)
                            {
                                DataTable dt = this.cntrl.expence_checked(todate.ToString("yyyy-MM-dd"), "Income");
                                Load_expenseReport(dt);
                            }
                            else
                            {
                                DataTable dt = this.cntrl.expense_income_notChecked(todate.ToString("yyyy-MM-dd"));
                                Load_expenseReport(dt);
                            }
                        }
                        else
                    ...
                    else if (comborepcategory.Text == "INVENTORY")
                    {
                        ...
                        Panel_inventory.Height = 891;////293, 443
                        Panel_inventory.Show();
                        dateTimePickerepo1.Visible = false;
                        //from.Visible = false;
                        dateTimePickerrepo2.Visible = false;
                        to.Visible = false;
                        btn_Show.Visible = false;
                        btnprint.Visible = false;
                        btn_Export.Visible = false;
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    else
                    {
                        for (int g = 0; g < dt.Rows.Count; g++)
                        {
                            decimal due = 0,totalcost=0;
                            totalcost=
                                due = decimal.Parse(dt.Rows[g]["AMOUNT PAID"].ToString())-
                            Grvreports.Rows.Add(dt.Rows[g]["PATIENT NAME"].ToString(), dt.Rows[g]["doctor_name"].ToString(), dt.Rows[g]["RECEIPT NO"].ToString(), dt.Rows[g]["INVOICE NO"].ToString(), dt.Rows[g]["TREATMENT"].ToString(), dt.Rows[g]["AMOUNT PAID"].ToString(), DateTime.Parse(dt.Rows[g]["PAYMENT DATE"].ToString()).ToString("dd/MM/yyyy"), dt.Rows[g]["DUE AFTER PAYMENT"].ToString());
                            decimal amount_paid = decimal.Parse(dt.Rows[g]["AMOUNT PAID"].ToString());
                            string discount = dt.Rows[g]["DUE AFTER PAYMENT"].ToString();
                            totaltax = totaltax + amount_paid;
                ...
                Lab_DueAftrPaymnt.Text = totaldiscount.ToString("#0.00");// Convert.ToDecimal(fullTotalCost - totaltax).ToString("#0.00");
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Error !...", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        public void Load_inventoryReport(DataTable dt_stock)
        {
            try
            {
                if (dt_stock.Rows.Count > 0)
                {
                    ...
                    Grvreports.Columns[1].HeaderText = "ITEM id";
                    Grvreports.Columns[2].HeaderText = "ITEM CODE";
                    Grvreports.Columns[3].HeaderText = "ITEM NAME";
                    Grvreports.Columns[4].HeaderText = "UNIT";
                    Grvreports.Columns[5].HeaderText = "STOCK";
                    Grvreports.Columns[6].HeaderText = "GROSS AMOUNT";
                    Grvreports.Columns[0].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleLeft;
                    Grvreports.Columns[2].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleLeft;
                    Grvreports.Columns[3].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleLeft;
                    Grvreports.Columns[4].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleLeft;
                    Grvreports.Columns[5].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleLeft;
                    Grvreports.Columns[6].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleLeft;
                    ...
                    foreach (DataRow dr in dt_stock.Rows)
                    {
                        ...
                        decimal gross_amount = 0, gross_total = 0;
                        DataTable dt_gross = this.cntrl.stock_grossvalue(dr["item_code"].ToString());
                        //Grvreports.Rows.Add(num, dr["item_code"].ToString(), dr["ItemCode"].ToString(), dtunit.Rows[0]["item_name"].ToString(), dtunit.Rows[0]["Unit1"].ToString(), current_Stock, gross_total.ToString("##.000"));

                        if (dt_gross.Rows.Count > 0)
                        {
                            for (int h = 0; h < dt_gross.Rows.Count; h++)
                            {
                                gross_amount = Convert.ToDecimal(dt_gross.Rows[h]["qty"].ToString()) * Convert.ToDecimal(dt_gross.Rows[h]["rate"].ToString());
                                gross_total = gross_total + gross_amount;
                            }
                        }
                        if (dtunit.Rows.Count > 0)
                        {
                            if (dtunit.Rows[0]["OneUnitOnly"].ToString() == "False")
                            {
                                ...
                                Grvreports.Rows.Add(num, dr["item_code"].ToString(), dr["ItemCode"].ToString(), dr["item_name"].ToString(), dr["unit"].ToString(), current_Stock, gross_total.ToString("##.000"));
                            }
                            else
                            {
                    ...
                    this.Grvreports.Columns[5].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
                    this.Grvreports.Columns[1].Visible = false;
                }
                else
                {
                    int x = (panel2.Size.Width - Lab_Msg.Size.Width) / 2;
                ...
                if (Grvreports.Rows.Count > 0)
                {
                    ...
                    if (comborepcategory.Text == "INVENTORY")
                    {
                        ...
                        if (Grvreports.Rows.Count > 0)
                        {
                            ...
                            sWrite.WriteLine("    <td align='left' width='22%' style='border:1px solid #000;background:#999999'><FONT COLOR=black FACE='Segoe UI' SIZE=3>&nbsp;<b> Gross Amount</b></font></th>");
                            sWrite.WriteLine("</tr>");
                            for (c = 0; c < Grvreports.Rows.Count; c++)
                            {
                                sWrite.WriteLine("<tr>");
                                ...
                                sWrite.WriteLine("    <td align='left' style='border:1px solid #000' ><FONT COLOR=black FACE='Segoe UI' SIZE=2>&nbsp;" + Grvreports.Rows[c].Cells[3].Value.ToString() + "</font></th>");
                                sWrite.WriteLine("    <td align='left' style='border:1px solid #000' ><FONT COLOR=black FACE='Segoe UI' SIZE=2>&nbsp;" + Grvreports.Rows[c].Cells[4].Value.ToString() + "</font></th>");
                                sWrite.WriteLine("    <td align='left' style='border:1px solid #000' ><FONT COLOR=black FACE='Segoe UI' SIZE=2>&nbsp;" + Grvreports.Rows[c].Cells[5].Value.ToString() + "</font></th>");
                                sWrite.WriteLine("</tr >");
                            }
                            sWrite.WriteLine("</table>");
                            sWrite.WriteLine("</div>");
        ...
        private void LAB_IPPATIENT_Click(object sender, EventArgs e)
        {
            var form = new IP_patients_reports();
            form.ShowDialog();
        }

        private void Grvreports_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (Grvreports.Rows.Count > 0 && e.RowIndex>-1)
            {
                if (comborepcategory.Text == "INVENTORY")
                {
                    panl_batch.Visible = true;
                    string itmename = Grvreports.CurrentRow.Cells[1].Value.ToString();
                    DataTable dtb_batch = this.cntrl.get_batch_stock(itmename);
                    if (dtb_batch.Rows.Count > 0)
                    {
                        int j = 1;
                        dgv_batch_stock.Rows.Clear();
                        for (int i = 0; i < dtb_batch.Rows.Count; i++)
                        {
                            dgv_batch_stock.Rows.Add();
                            dgv_batch_stock.Rows[i].Cells["slno"].Value = j;
                            dgv_batch_stock.Rows[i].Cells["batch"].Value = dtb_batch.Rows[i]["BatchNumber"].ToString();
                            dgv_batch_stock.Rows[i].Cells["stock"].Value = dtb_batch.Rows[i]["Qty"].ToString();
                            j++;
                        }
                    }
                }
        ...
        //private void dgv_batch_stock_CellClick(object sender, DataGridViewCellEventArgs e)
        //{
        //    //panl_batch.Visible = false;
        //}

        //private void btn_batch_Cancel_Click(object sender, EventArgs e)
        //{
        //    //panl_batch.Visible = false;
        //}
        ...
        private void dgv_batch_stock_CellClick_1(object sender, DataGridViewCellEventArgs e)
        {
        ...
        {
            var form = new P_L_Account_Report();
            form.ShowDialog();
        }

        private void LAB_OPPATIENT_Click(object sender, EventArgs e)
        {
            var form = new OPpatient_report();
            form.ShowDialog();
        }
        ...
        private void lb_oppatient_report_Click(object sender, EventArgs e)
        {
            var form = new OPpatient_report();
            form.ShowDialog();
        }
        ...
        {
            var form = new GST_Report();
            form.ShowDialog();
        }

        private void label13_Click(object sender, EventArgs e)
        {
            var form = new daily_GST_report();
            form.ShowDialog();
        }

        private void lb_monthlyGSTReport_Click(object sender, EventArgs e)
        {
            var form = new Monthly_gst_report();
            form.ShowDialog();
        }

        private void label13_Click_1(object sender, EventArgs e)
        {
            //var form2 = new stock_adjustment_report();
            //form2.ShowDialog();
        }

        private void lb_StockTransferReport_Click(object sender, EventArgs e)
        {
            var form = new StockTransfer_Report();
            form.ShowDialog();
        }

        private void lb_stockadjustment_report_Click(object sender, EventArgs e)
        {
            var form2 = new stock_adjustment_report();
            form2.ShowDialog();
        }

        private void lb_consultation_Click(object sender, EventArgs e)
        {
            var form = new Patient_consultation_report();
            form.ShowDialog();
        }

        private void lb_patient_wise_consultation_Click(object sender, EventArgs e)
        {
            var form = new patient_wise_consultation_report();
            form.ShowDialog();
        }
        ...
        {
            var form2 = new AlreadyExpired();
            form2.ShowDialog();
        }

        private void label13_Click_2(object sender, EventArgs e)
        {
            var form2 = new GST_Summery_Report();
            form2.ShowDialog();
        }
        ...
        {
            var frm = new MonthlyHSN_Tax_Payment();
            frm.ShowDialog();
        }

        private void button1_Click(object sender, EventArgs e)
        {
            DayWiseInvoice_Report d = new DayWiseInvoice_Report();
            //d.doctor_id = doctor_id;
            d.ShowDialog();
        }
