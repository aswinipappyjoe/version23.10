...
namespace PappyjoeMVC.View
{
    public partial class Procedure_Catalog : Form
    {
        ...
        private void Procedure_catalog_Load(object sender, EventArgs e)
        {
            ...
            DataTable dtb_cost = this.cntrl.get_enablecost();
            if(dtb_cost.Rows.Count>0)
            {
                if(dtb_cost.Rows[0][1].ToString().Trim()=="Yes")
                {
                    chk_disply_cost.Checked = true;
                }
                else
                {
                    chk_disply_cost.Checked = false;
                }
            }
        }
        public void FormLoad(DataTable dt)
        {
            ...
            for (int j = 0; j < dt.Rows.Count; j++)
            {
                ...
                string sac = dt.Rows[j]["sac"].ToString();
                ...
                else
                {
                    ...
                    Dgv_Procedure.Rows.Add(items[0], items1[1], items2[1], items[1], items4[1], items3[1],items5[1]);
                    rowNum++;
                }
        ...
        public void FormLoad_showmore(DataTable dt)
        {
            Dictionary<string, int> dict = new Dictionary<string, int>();
            int rowNum = 0;
            //Dgv_Procedure.Rows.Clear();
...

        {
            try
            {
                ...
                //else
                //{
                    int i = this.cntrl.update_addprocedure(txt_procedurename.Text, txt_procedurecost.Text,txtsac.Text, comboaddunder.Text, richnotes.Text, procedure_id);
        ...
        public void GetProcedureName(DataTable checkdatacc)
        {
            try
            {
                ...
                {
                    int i = this.cntrl.save_addprocedure(txt_procedurename.Text, txt_procedurecost.Text, txtsac.Text, comboaddunder.Text, richnotes.Text);
                    DataTable dtb = this.cntrl.get_procedureid(txt_procedurename.Text);
                    string p = dtb.Rows[0]["id"].ToString();
                    int pid = int.Parse(p);
                    if (chk_gst.Checked == true)
            ...
            try
            {
                if (e.RowIndex >= 0)
                {
                    ...
                    else if (Dgv_Procedure.CurrentCell.OwningColumn.Name == "col_edit")
                    {
                        ...
                        txtsac.Text = Dgv_Procedure.CurrentRow.Cells["sac"].Value.ToString();//
                        ...
                        if (xlWorkSheet.Cells[1, 1].value == "Procedure Name" && xlWorkSheet.Cells[1, 2].value == "Procedure Cost")
                        {
                            for (iRow = 2; iRow <= xlWorkSheet.Rows.Count; iRow++)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (dtb.Rows.Count == 0)
                                    {
                                        int i = this.cntrl.save_addprocedure(procedure_name, procedure_cost, sac,comboadd, notes);
                                    }
                                }
                            }
                            DataTable dt = this.cntrl.FormLoad();
        ...
        private void btn_save_cost_chk_Click(object sender, EventArgs e)
        {
            string disply_cost = "";
            if (chk_disply_cost.Checked)
            {
                disply_cost = "Yes";
            }
            else
            ...
            DataTable dtb= this.cntrl.get_enablecost();
            if(dtb.Rows.Count>0)
            {
                this.cntrl.update_enablecost(disply_cost);
                MessageBox.Show("Save data successfully ", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                this.cntrl.save_enablecost(disply_cost);
                MessageBox.Show("Save data successfully ", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);

            }
        }

        private void txt_procedurecost_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        {
            int count = row_count + 5;
            DataTable dtb = this.cntrl.FormLoad_ShMre(count);
            FormLoad_showmore(dtb);// load_treatment(dtb);
            row_count = count;
            //int limit = Dgv_Procedure.Rows.Count + 100;
            //DataTable dt = this.cntrl.FormLoad_ShMre(limit);
            //FormLoad(dt);
        }
    }
}
