   ...
   public class MainCalendar_Model
    {
        ...
        {
            string i = db.scalar("select id from tbl_User_Privilege where UserID=" + doctor_id + " and Category='APT' and Permission='S'");
        ...
        public DataTable doctor_appointments_admin_load(DateTime startDateTime, DateTime startDateTime1)
        {
            DataTable dtb = db.table("SELECT id,pt_name,start_datetime,plan_New_procedure ,status,EHR_status,duration,dr_id FROM tbl_appointment where start_datetime between  '" + Convert.ToDateTime(startDateTime).ToString("yyyy-MM-dd HH:mm") + "' AND '" + Convert.ToDateTime(startDateTime1).ToString("yyyy-MM-dd HH:mm") + "'  ORDER BY start_datetime");
            return dtb;
        }
        //public DataTable doctor_appointments_doctor_load(DateTime startDateTime, DateTime startDateTime1, string doctor_id)
        //{
        //    DataTable dtb = db.table("SELECT id,pt_name,start_datetime,plan_New_procedure ,status,EHR_status,duration,dr_id FROM tbl_appointment where start_datetime between  '" + Convert.ToDateTime(startDateTime).ToString("yyyy-MM-dd HH:mm") + "' AND '" + Convert.ToDateTime(startDateTime1).ToString("yyyy-MM-dd HH:mm") + "' and dr_id=" + doctor_id + " ORDER BY start_datetime");
        //    return dtb;
        //}
        public DataTable all_dctr_appointment_lil()
        {
            //DataTable strSql = db.table("SELECT id,start_datetime,duration,pt_id,pt_name,dr_id,status FROM tbl_appointment where month(book_datetime)=month(now()) ORDER BY id");
            //DataTable strSql = db.table("SELECT id,start_datetime,duration,pt_id,pt_name,dr_id,status FROM tbl_appointment where WEEK(book_datetime)=WEEK(now())  ORDER BY id");
            DataTable strSql = db.table("SELECT id,start_datetime,duration,pt_id,pt_name,dr_id,status FROM tbl_appointment where YEAR(start_datetime)=YEAR(now()) and MONTH(start_datetime)=MONTH(now()) and WEEK(start_datetime)=WEEK(now())  ORDER BY id");
            return strSql;
        }
        public DataTable all_dctr_appointment_daywise(DateTime startDateTime, DateTime startDateTime1)
        {
            //DataTable strSql = db.table("SELECT id,start_datetime,duration,pt_id,pt_name,dr_id,status FROM tbl_appointment where book_datetime >now() ORDER BY id");
            DataTable strSql = db.table("SELECT id,start_datetime,duration,pt_id,pt_name,dr_id,status FROM tbl_appointment where start_datetime between'" + Convert.ToDateTime(startDateTime).ToString("yyyy-MM-dd HH:mm") + "' AND '" + Convert.ToDateTime(startDateTime1).ToString("yyyy-MM-dd HH:mm") + "'  ORDER BY id");
            return strSql;
        }
        public DataTable all_dctr_appointment()
        {
            ...
            DataTable strSql = db.table("SELECT id,start_datetime,duration,pt_id,pt_name,dr_id,status FROM tbl_appointment where YEAR(book_datetime)=YEAR(now()) and MONTH(book_datetime)=MONTH(now())  ORDER BY id");
            return strSql;
        ...
        {
          DataTable str_sql =db.table( "SELECT id,doctor_name,calendar_color FROM tbl_doctor Where id = " + doctor_id + " and activate_login='yes' and availability='Available'");
            return str_sql;
        }
        public DataTable load_all_doctor()
        {
            DataTable str_sql =db.table("SELECT id,doctor_name,calendar_color FROM tbl_doctor Where (login_type='doctor' or login_type='admin') and activate_login='yes' and availability='Available'");
            return str_sql;
        }
        public string Get_staff_type(string dr_id)
        {
            string drt = db.scalar("SELECT login_type FROM tbl_doctor where id=" + dr_id + " ORDER BY id");
            return drt;
        }
        ...
        public void update_appointment_status(string ContextEvent,string reason)
        {
           db.execute("update tbl_appointment set status='CANCELLED',cancelation_reason='"+reason+"'  where id='" + ContextEvent + "'");
        }
        ...
        {
            DataTable dtdr = db.table("SELECT pt_id,pt_name,id,book_datetime,dr_id from tbl_appointment where id='" + id1 + "'");
            return dtdr;
        }
    }
