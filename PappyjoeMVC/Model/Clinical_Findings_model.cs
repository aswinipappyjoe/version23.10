...
namespace PappyjoeMVC.Model
{
    public class Clinical_Findings_model
    {
        ...
        {
            System.Data.DataTable dt_cf_main = db.table("SELECT tbl_clinical_findings.id,tbl_clinical_findings.date,tbl_doctor.doctor_name FROM tbl_clinical_findings join tbl_doctor on tbl_clinical_findings.dr_id=tbl_doctor.id  where tbl_clinical_findings.pt_id='" + patient_id + "' ORDER BY tbl_clinical_findings.date DESC limit 10");
            return dt_cf_main;
        }
        ...
        {
            System.Data.DataTable dt_cf_main = db.table("SELECT tbl_clinical_findings.id,tbl_clinical_findings.date,tbl_doctor.doctor_name FROM tbl_clinical_findings join tbl_doctor on tbl_clinical_findings.dr_id=tbl_doctor.id  where tbl_clinical_findings.pt_id='" + patient_id + "' ORDER BY tbl_clinical_findings.date DESC limit " + count + ",10");
            return dt_cf_main;
        }
        ...
        {
            DataTable dt8 = db.table("select med_id from tbl_pt_medhistory where clinic_id='" + idd + "'");
            return dt8;
        }
        ...
        {
            int i = db.execute("delete from tbl_pt_allergy where clinical_findings_id='" + clinic_id + "'");
            return i;
        }
        ...
        {
            System.Data.DataTable dt_cf_note = db.table("SELECT current_meditation FROM tbl_pt_current_meditation where tbl_pt_current_meditation.clinic_id='" + dt_cf_main + "' ORDER BY tbl_pt_current_meditation.id");
            return dt_cf_note;
        }
        public DataTable dt_cf_advice(string dt_cf_main)
        {
            System.Data.DataTable dt_cf_note = db.table("SELECT discharge_advice FROM tbl_pt_discharge_advice where tbl_pt_discharge_advice.clinic_id='" + dt_cf_main + "' ORDER BY tbl_pt_discharge_advice.id");
            return dt_cf_note;
        }
    }
}
