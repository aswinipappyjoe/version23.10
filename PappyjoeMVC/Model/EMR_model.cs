...
namespace PappyjoeMVC.Model
{
    public class EMR_model
    {
        ...
        //allergy
        public DataTable Fill_allergygrid()
        {
            DataTable dt2 = db.table("select * from tbl_allergies order by id");
            return dt2;
        }
        public DataTable check_allergy(string note)
        {
            DataTable dt = db.table("Select * from tbl_allergies where allergies ='" + note + "'");
            return dt;
        }
        public void save_allergy(string _note)
        {
            int i = db.execute("insert into tbl_allergies (allergies) values ('" + _note + "')");
        }
        public void update_allergy(string id_notes, string _note)
        {
            int i = db.execute("update tbl_allergies set allergies='" + _note + "'where id='" + id_notes + "'");
        }
        public void delete_allergy(string id_notes)
        {
            int i = db.execute("delete from tbl_allergies where id='" + id_notes + "'");
        }
        public DataTable search_allergy(string text)
        {
            DataTable dt2 = db.table("select * from tbl_allergies where allergies like '%" + text + "%'");
            return dt2;
        }
    }
}
