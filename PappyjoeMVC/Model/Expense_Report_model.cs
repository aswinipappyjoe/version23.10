   ...
   public class Expense_Report_model
    {
        ...
        {
            DataTable query = db.table("select id,receipt_no,amount_paid,payment_date,dr_id from tbl_payment where payment_date between '"+ date1 + "' and '"+ date2 + "' ");
            return query;
        }//
        public DataTable Daily_P_L_count(string d1, string d2)
        {
            DataTable query = db.table("select  date_format(payment_date,'%a %Y') AS DAY,COUNT(*) AS 'RECEIPTS' from tbl_payment  where   payment_date between '" + d1 + "' and '" + d2 + "' group by date_format(payment_date,'%a %Y')");
            return query;
        }
    }
}
