...
namespace PappyjoeMVC.Model
{
    public class Receipt_Model
    {
        ...
        public DataTable receipt_number(MySqlConnection con, MySqlTransaction trans)//consultation
        {
            DataTable receipt = db.trans_table("select receipt_number,receipt_prefix from tbl_receipt_automationid where receipt_automation='Yes'", con, trans);
            return receipt;
        }
        ...
        public DataTable dt_due_clear(string invno,string ptid)
        {
            DataTable dt = db.table("select invoice_no,receipt_no,amount_paid,payment_due from tbl_payment where invoice_no='"+ invno + "' and pt_id='"+ ptid + "'");
            return dt;
        ...
        public void update_due(decimal total, string invoice_no, string patient_id)
        {
            db.execute("update tbl_payment set payment_due='" + total + "' where invoice_no='" + invoice_no + "' and pt_id='" + patient_id + "' ");
        }
        ...
        {
            int i = db.trans_execute("update tbl_invoices set total='" + total + "' where invoice_no='" + invoice_no + "' and pt_id='" + patient_id + "' and services='" + services + "'",con,trans);
            return i;
        }
        ...
        {
            db.trans_execute("update tbl_invoices_main set status='0' where id='" + invoice_main_id + "'",con,trans);
        }
        public int save_payment(string advance, string receipt_no, decimal amount_paid, string invoice_no, string procedure_name, string mode_of_payment, string pt_id, string payment_date, string dr_id, decimal payment_due, decimal total, string cost, string pt_name,string invoicemainid)
        {
            int inv1 = db.execute("insert into tbl_payment(advance,receipt_no,amount_paid,invoice_no,procedure_name,mode_of_payment,pt_id,payment_date,dr_id,payment_due,total,cost,pt_name,Invoice_mainId)values('" + advance + "','" + receipt_no + "','" + amount_paid + "','" + invoice_no + "','" + procedure_name + "','" + mode_of_payment + "','" + pt_id + "','" + payment_date + "','" + dr_id + "','" + payment_due + "','" + total + "','" + cost + "','" + pt_name + "','" + invoicemainid + "')");
            return inv1;
        }
        ...
        {
            int inv1 = db.trans_execute("insert into tbl_payment(advance,receipt_no,amount_paid,invoice_no,procedure_name,mode_of_payment,pt_id,payment_date,dr_id,payment_due,total,cost,pt_name)values('" + advance + "','" + receipt_no + "','" + amount_paid + "','" + invoice_no + "','" + procedure_name + "','" + mode_of_payment + "','" + pt_id + "','" + payment_date + "','" + dr_id + "','" + payment_due + "','" + total + "','" + cost + "','" + pt_name + "')",con,trans);
            return inv1;
        }
        ...
        public string receipt_autoid( MySqlConnection con, MySqlTransaction trans)//consultation
        {
            string rec = db.trans_scalar("select receipt_number from tbl_receipt_automationid",con,trans);
            return rec;
        }
        ...
        public void update_receiptAutoid(int receip, MySqlConnection con, MySqlTransaction trans)//consultation
        {
            db.trans_execute("update tbl_receipt_automationid set receipt_number='" + receip + "'",con,trans);
        }
        public DataTable get_patientDetails(string patient_id)
        {
            DataTable dt_patent = db.table("select pt_name,pt_id,age,gender,street_address,aadhar_id,primary_mobile_number from tbl_patient where id='" + patient_id + "'");
            return dt_patent;
        }
        public DataTable payment_details(string date, string patient_id)
        {
            DataTable Payments = db.table("select receipt_no,amount_paid,invoice_no,procedure_name,mode_of_payment,refund_amount from tbl_payment where payment_date='" + date + "' and pt_id='" + patient_id + "'");
            return Payments;
        }
        ...
        {
            DataTable Payment = db.table("select  distinct(payment_date) from tbl_payment where pt_id='" + patient_id + "'AND payment_date!='' order by payment_date desc limit 15");
            return Payment;
        }
        public DataTable get_paymentDate_load(string patient_id)
        {
            DataTable Payment = db.table("select  count(id) from tbl_payment where pt_id='" + patient_id + "'AND payment_date!='' order by payment_date desc ");
            return Payment;
        }
        public DataTable get_paymentDate_count(string patient_id,int count)
        {
            DataTable Payment = db.table("select  distinct(payment_date) from tbl_payment where pt_id='" + patient_id + "'AND payment_date!='' order by payment_date desc limit " + count + ",15");
            return Payment;
        }
        public DataTable payment_details_re(string date, string patient_id, string reve, string proname)
        {
            DataTable Payments = db.table("select receipt_no,amount_paid,invoice_no,procedure_name,mode_of_payment,refund_amount from tbl_payment where payment_date='" + date + "' and pt_id='" + patient_id + "' and invoice_no='" + reve + "' and procedure_name='" + proname + "'");
            return Payments;
        }
        public void update_refund(string status, string amount, string rece, string pro)
        {
            db.execute("update tbl_payment set refund_status='" + status + "',refund_amount='" + amount + "' where receipt_no= '" + rece + "' and procedure_name='" + pro + "'");
        }
        //public DataTable get_patientDetails(string patient_id)
        //{
