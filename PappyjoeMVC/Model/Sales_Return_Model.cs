...
namespace PappyjoeMVC.Model
{
    public class Sales_Return_Model
    {
        ...
        {
            DataTable dtb = db.trans_table("select SaleType,SalesTax,AST,Memo from tbl_SALES where InvNumber='" + Convert.ToInt32(InvNumber) + "'", con, trans);
            return dtb;
        }
        ...
        {
           db.trans_execute("insert into tbl_RETIT (RetNumber,Item_Code,Qty,Unit,Rate,UNIT2,Taxable,FreeQty)values('" + Convert.ToInt32(RetNumber) + "','" + Item_Code + "','" + Convert.ToDecimal(Qty) + "','" + unit + "','" + Convert.ToDecimal(Rate) + "','" + UNIT2 + "','Yes','" + Convert.ToDecimal(FreeQty) + "')", con, trans);
        }
        public string get_sales_retqty(string invoicenumbr,string itemcode, MySqlConnection con, MySqlTransaction trans)
        {
            string dt_UpdateRetqty_sales = db.trans_scalar("select RetQty from tbl_SALEIT where InvNumber='" + Convert.ToInt32(invoicenumbr) + "'  and Item_Code='" + itemcode + "' ",con,trans);
        ...
        {
            DataTable dt_salesRetqty = db.trans_table("select Qty,UNIT2 from tbl_RETIT where RetNumber='" + returnnumber + "' and Item_Code='" + itemcode + "' ",con,trans);
        ...
        {
            db.trans_execute("update tbl_SALEIT set RetQty='" + RetQty + "' where Item_Code='" + itemcode + "' and InvNumber='" + Convert.ToInt32(InvNumber) + "'", con, trans);
        }
        public string retqty_from_batchsale(string entryno, MySqlConnection con, MySqlTransaction trans)
        {
            string dt_UpdateRetqty = db.trans_scalar("select RetQty from tbl_BatchSale where Entry_No='" + Convert.ToInt32(entryno) + "' ", con, trans);
            return dt_UpdateRetqty;
        }
        public void update_batchsale(decimal Entry_No ,string bat_RetQty, MySqlConnection con, MySqlTransaction trans)
        {
            db.trans_execute("update tbl_BatchSale set RetQty='" + bat_RetQty + "'where Entry_No='" + Entry_No + "'", con, trans);
        }
        public int update_batchnumber(string qty,string Entry_No, MySqlConnection con, MySqlTransaction trans)
        {
          int  rowaffected = db.trans_execute("update tbl_BatchNumber set qty='" + Convert.ToDecimal(qty) + "' where Entry_No='" + Convert.ToInt32(Entry_No) + "'", con, trans);
            return rowaffected;
        }
