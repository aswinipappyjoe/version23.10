...
namespace PappyjoeMVC.Model
{
    public class Manufacture_model
    {
        ...
        public DataTable manufacture_name(string name)
        {
            DataTable dtb = db.table("select * from tbl_manufacturer where manufacturer='" + name + "'");
            return dtb;
        }
        public DataTable manufacture_code(string code)
        {
            DataTable dtb = db.table("select * from tbl_manufacturer where Code='" + code + "'");
            return dtb;
        }
        public DataTable manufacture_phn(string phn)
        {
            DataTable dtb = db.table("select * from tbl_manufacturer where Phone='" + phn + "'");
            return dtb;
        }
        public DataTable mnfctr(string id)
        {
            DataTable dt = db.table("select id,Code,manufacturer,ShortName from tbl_manufacturer  where manufacturer like '%" + id + "%'");
            return dt;
        }
    }
}
