...
namespace PappyjoeMVC.Model
{
    public class Booking_model
    {
        ...
        {
            DataTable dt = db.table("select CONCAT(pt_name, ', ', substring(gender,1,1), ', ',primary_mobile_number) pt_name,id from tbl_patient where (pt_name like '" + search + "%'  or pt_id like '" + search + "%' or primary_mobile_number like '" + search + "%' or email_address like '" + search + "%' or street_address  like '" + search + "%' or locality like '" + search + "%' or city like '" + search + "%') and Profile_Status='Active' ");
            return dt;
        }
        ...
        {
            DataTable dt_a = db.table("select pt_id,dr_id,note,plan_new_procedure,start_datetime,duration from tbl_appointment where  id=" + id + " ORDER BY id");
            return dt_a;
        }
