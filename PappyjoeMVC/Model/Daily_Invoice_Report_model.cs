...
namespace PappyjoeMVC.Model
{
    public class Daily_Invoice_Report_model
    {
        ...
        {
            DataTable dt = db.table("select DISTINCT id,doctor_name from tbl_doctor where  login_type = 'DOCTOR' and activate_login='yes' order by doctor_name");
            return dt;
        }
        ...
        {
            DataTable dt = db.table("select tbl_invoices.pt_name,tbl_invoices.date,tbl_invoices.total,invoice_no,services,cost,unit,discount_type,discountin_rs,tax,tax_inrs,tbl_doctor.doctor_name from tbl_invoices LEFT JOIN tbl_patient on tbl_invoices.pt_id=tbl_patient.id LEFT JOIN tbl_doctor on tbl_invoices.dr_id=tbl_doctor.id where tbl_patient.Profile_Status='Active' and tbl_invoices.date='" + Convert.ToDateTime(invdate).ToString("yyyy-MM-dd") + "' order by date desc");
            return dt;
        }
        public DataTable getinvoice2(string invdate, string doctrid)
        {
            DataTable dt = db.table("select tbl_invoices.pt_name,tbl_invoices.date,invoice_no,services,cost,unit,discount_type,discountin_rs,tax,tax_inrs,tbl_doctor.doctor_name from tbl_invoices  LEFT JOIN tbl_patient on tbl_invoices.pt_id=tbl_patient.id LEFT JOIN tbl_doctor on tbl_invoices.dr_id=tbl_doctor.id where tbl_patient.Profile_Status='Active' and tbl_invoices.date ='" + Convert.ToDateTime(invdate).ToString("yyyy-MM-dd") + "' and dr_id='" + doctrid + "' order by date desc");
            return dt;
        }
        ...
        {
            string e = db.scalar("Select id from tbl_doctor where doctor_name='" + drname + "' and login_type='doctor' or login_type='admin'");
            return e;
        }
