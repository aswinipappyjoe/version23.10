...
namespace PappyjoeMVC.Model
{
    public class LabtrackingReport_model
    {
        ...
        public DataTable getLabnames()
        {
            DataTable sf = db.table("select labname from tbl_laboratory");
            return sf;
        }
        public DataTable lbnmerel(string lbnme)
        {
            DataTable dt = db.table("SELECT distinct A.id as'JOB.NO',B.pt_name as 'PATIENT INFO',B.primary_mobile_number as 'PHONE NO',C.doctor_name as'DOCTOR' ,A.labname as'LAB',A.work_name as'WORK NAME',A.duedate as'DUE',A.trackingstatus as'STATUS' from tbl_lab_main A inner join tbl_patient B on A.pt_id=B.id inner join tbl_doctor C on A.dr_id=C.id WHERE A.type='Dental' and  A.trackingstatus is not null and A.labname='"+lbnme+"'");
            return dt;
        }
    }
}
