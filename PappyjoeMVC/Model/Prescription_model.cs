...
namespace PappyjoeMVC.Model
{
    public class Prescription_model
    {
        ...
        public DataTable drug_load(bool check_gene)
        {
            //DataTable dt4 = db.table("select id,CONCAT(name,' ', type ) as name, CONCAT(strength_gr ,' ' , strength ) as type,inventory_id from tbl_adddrug where inventory_id!='0'  ORDER BY name ASC");
            DataTable dt4 = new DataTable();
            if (check_gene == true)
            {
                dt4 = db.table("select id,CONCAT(generic,' ', type ) as name, CONCAT(strength_gr ,' ' , strength ) as type,inventory_id,generic from tbl_adddrug where generic <> '' ORDER BY generic ASC limit 200");
            }
            else
            {
                dt4 = db.table("select id,CONCAT(name,' ', type ) as name, CONCAT(strength_gr ,' ' , strength ) as type,inventory_id from tbl_adddrug  ORDER BY name ASC limit 200");
        ...
        public DataTable drug_instock(string id)
        {
            ...
            DataTable dtstock = db.table("select sum(Qty) 'Stock' from tbl_BatchNumber where item_code= '" + id + "'  and `ExpDate`> CURDATE()");
            return dtstock;
        }
        public DataTable check_expiry_date(string ItemCode)
        {
            DataTable dtb = db.table("select distinct b.Entry_No,b.BatchNumber,b.Qty,DATE_FORMAT( b.ExpDate, '%m-%Y') AS ExpDate from tbl_BatchNumber b  where  b.Item_Code='" + ItemCode + "'and b.Qty>0 and `ExpDate`> CURDATE()  order by b.ExpDate ");
            return dtb;
        }
        ...
        public DataTable drug_search(string search, bool check_gene)
        {
            DataTable dt4 = new DataTable();
            if (check_gene == true)
            {
                dt4 = db.table("select id,CONCAT(generic,' ', type ) as name, CONCAT(strength_gr, ' ', strength ) as type,inventory_id from tbl_adddrug where generic like'%" + search + "%' ORDER BY CASE WHEN generic = '" + search + "' THEN 0 WHEN  generic LIKE '" + search + "%' THEN 1 WHEN generic LIKE '%" + search + "%' THEN 2 WHEN generic LIKE '%" + search + "' THEN 3 ELSE 4 END , generic ASC limit 250");
                //dt4 = db.table("select id,CONCAT(generic,' ', type ) as name, CONCAT(strength_gr, ' ', strength ) as type,inventory_id from tbl_adddrug where concat(generic,'') like'%" + search + "%' ORDER BY id DESC limit 100");

            }
            else
            {
                dt4 = db.table("select id,CONCAT(name,' ', type ) as name, CONCAT(strength_gr, ' ', strength ) as type,inventory_id from tbl_adddrug where name like'%" + search + "%' ORDER BY CASE WHEN name = '" + search + "' THEN 0 WHEN  name LIKE '" + search + "%' THEN 1 WHEN name LIKE '%" + search + "%' THEN 2 WHEN name LIKE '%" + search + "' THEN 3 ELSE 4 END , name ASC limit 250");
        ...
        public void update_prescription_review(string date, int presid, MySqlConnection con, MySqlTransaction trans)//transaction
        {
            db.trans_execute("UPDATE tbl_prescription_main SET review='YES',Review_date='" + date + "' WHERE id='" + presid + "'",con,trans);
        }
        ...
        public void update_prescription_review_NO(string date, int presid, MySqlConnection con, MySqlTransaction trans)//trans
        {
            db.trans_execute("UPDATE tbl_prescription_main SET review='NO',Review_date='" + date + "'  WHERE id='" + presid + "'",con,trans);
        }
        ...
        {
            System.Data.DataTable dt_pre_main = db.table("SELECT tbl_prescription_main.id,tbl_prescription_main.date,tbl_doctor.doctor_name  FROM tbl_prescription_main join tbl_doctor on tbl_prescription_main.dr_id=tbl_doctor.id  where tbl_prescription_main.pt_id='" + patient_id + "' ORDER BY tbl_prescription_main.date DESC limit 25");
            return dt_pre_main;
        }
        public DataTable Get_maindtails_count(string patient_id,int count)
        {
            System.Data.DataTable dt_pre_main = db.table("SELECT tbl_prescription_main.id,tbl_prescription_main.date,tbl_doctor.doctor_name  FROM tbl_prescription_main join tbl_doctor on tbl_prescription_main.dr_id=tbl_doctor.id  where tbl_prescription_main.pt_id='" + patient_id + "' ORDER BY tbl_prescription_main.date DESC limit " + count + "");
            return dt_pre_main;
        }
        ...
        {
            System.Data.DataTable dt1 = db.table("select pt_id,pt_name,gender,age,age2,days,street_address,city,primary_mobile_number,locality,email_address,pincode,doctorname from tbl_patient where id='" + patient_id + "'");
            return dt1;
        }
