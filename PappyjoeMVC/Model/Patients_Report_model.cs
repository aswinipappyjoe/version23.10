...
namespace PappyjoeMVC.Model
{
    public class Patients_Report_model
    {
        ...
        {
            DataTable doctor_rs = db.table("select DISTINCT id,doctor_name from tbl_doctor where login_type = 'DOCTOR' order by doctor_name");
            return doctor_rs;
        }
        ...
        {
            //DataTable dt_inv = db.table("SELECT i.ip_id,p.pt_id,p.pt_name,p.age,p.gender,p.street_address,c.department,p.date,c.from_date,c.to_date,i.discharge_date FROM tbl_patient p inner join  tbl_casesheet c on p.id=c.patient_id inner join tbl_ippatient i on p.id=i.pt_id WHERE p.date between '" + dtp1 + "' and '" + dtp2 + "' ");
            DataTable dt_inv = db.table("SELECT i.ip_id, p.pt_id, p.pt_name, p.age, p.gender, p.street_address, c.department, p.date, c.from_date, c.to_date,p.Room_no,p.primary_mobile_number, i.discharge_date, g.group_id FROM tbl_patient p inner join  tbl_casesheet c on p.id = c.patient_id inner join tbl_ippatient i on p.id = i.pt_id inner join tbl_pt_group g on p.id = g.pt_id WHERE p.date between '" + dtp1 + "' and '" + dtp2 + "' ");

            //DataTable dt_inv = db.table("select p.pt_id,p.pt_name,p.age,p.gender,p.street_address,p.date FROM tbl_patient p where  p.date between '" + dtp1 + "' and '" + dtp2 + "' and p.op_id<>''");
            return dt_inv;
        }
        ...
        {
            DataTable dt_inv = db.table("select p.pt_id,p.pt_name,p.age,p.gender,p.street_address,p.Room_no	,p.primary_mobile_number	,p.date,p.op_id,g.group_id FROM tbl_patient p left join tbl_pt_group g on p.id = g.pt_id where  p.date between '" + dtp1 + "' and '" + dtp2 + "' and p.op_id<>''");
            return dt_inv;

        }
        public DataTable opPatientlist(string date1, string date2)
        {
            DataTable griddailytrreatmenttable = db.table("select DATE_FORMAT(date, '%d-%m-%Y') AS  'date',pt_id,pt_name,primary_mobile_number,email_address,doctorname,nationality,passport_no from tbl_patient  WHERE tbl_patient.Profile_Status='Active' and date  between '" + date1 + "' and '" + date2 + "' and tbl_patient.op_id='' GROUP BY id,pt_id,pt_name,date,primary_mobile_number,email_address,doctorname,nationality,passport_no  having COUNT(pt_id)=1 ");
            return griddailytrreatmenttable;
        }
    }
}
