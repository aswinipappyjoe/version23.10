...
namespace PappyjoeMVC.Model
{
    public  class sales_order_model
    {
        ...
        {
            //DataTable dtb = db.table(" select Sales_Rate_Max from tbl_ITEMS where id='" + itemid + "'");
            DataTable dtb = db.table(" select distinct strength from tbl_adddrug where inventory_id='" + itemid + "'");
            return dtb;
        }
        ...
        {
            string item = db.scalar("select item_code from tbl_items where id='" + id + "'");
            return item;
        }
        ...
        public int save_master(string doc,string docdate, string order, string orderdate, string name, string cusid, string address1, string adr2, string adr3, string phone, string TotalAmount,string discount_inr, string discount, string grandtotal, MySqlConnection con, MySqlTransaction trans)
        {
          int  i = db.trans_execute("insert into tbl_SalesOrder_Master(DocNumber,DocDate,OrderNo,OrderDate,CustomerName,Cus_Id,Address1,adr2,adr3,Phone,TotalAmount,Discount_inr,Discount_Amount,GrandTotal,Status)values('" + Convert.ToInt32(doc) + "','" + Convert.ToDateTime(docdate).ToString("yyyy-MM-dd") + "','" + order + "','" +Convert.ToDateTime(orderdate).ToString("yyyy-MM-dd") + "','" + name + "','" + cusid + "','" + address1 + "','" + adr2 + "','" + adr3 + "','" + phone + "','" + TotalAmount + "','"+ discount_inr + "','" + discount + "','" + grandtotal + "','O')", con,trans);
            return i;
        }
        public int save_items(string DocNumber, string DocDate, string ItemCode, string Discription, string strength, string unit,  string Qty, string Cost,string Total , MySqlConnection con, MySqlTransaction trans)
        {
          int  j = db.trans_execute("insert into tbl_SalesOrder(DocNumber,DocDate,ItemCode,Discription,Strength,Unit,Qty,Cost,TotalAmount)values('" + Convert.ToInt32(DocNumber) + "','" + DocDate + "','" + ItemCode + "','" + Discription + "','" + strength + "','" + unit + "','" + Qty + "','" + Cost + "','" + Total + "')", con,trans);
            return j;
        }
        public void delete_order(int invnum_Edit, MySqlConnection con, MySqlTransaction trans)
        {
            db.trans_execute("delete from tbl_SalesOrder_Master where DocNumber='" + invnum_Edit + "'",con,trans);
            db.trans_execute("delete from tbl_SalesOrder where DocNumber='" + invnum_Edit + "'",con,trans);
        }
