...
namespace PappyjoeMVC.Model
{
    public class Patients_model
    {
        ...
        {
            DataTable sqlstr = db.table("select  id as Pid,pt_id As 'Id'  ,pt_name  as 'Patient Name',gender as Gender,age as Age ,age2,days, primary_mobile_number  as   Mobile,street_address as 'Street Address',locality  as Locality,DATE_FORMAT(Visited,'%d/%m/%y') as Visited,Opticket as 'File NO',Room_no,Profile_Status from tbl_patient  where Profile_Status='Active' order by CAST(Visited AS DATE) DESC,pt_id DESC");
            //DataTable sqlstr = db.table("select  id as Pid,pt_id As 'Id'  ,pt_name  as 'Patient Name',gender as Gender,age as Age ,age2,days, primary_mobile_number  as   Mobile,street_address as 'Street Address',locality  as Locality,DATE_FORMAT(Visited,'%d/%m/%y') as Visited,Opticket as 'File NO' from tbl_patient  where   pt_id like '%" + itemname + "%' or       Profile_Status='Active' order by CAST(Visited AS DATE) DESC,pt_id DESC ");
            return sqlstr;
        }
        public DataTable Get_all_Patients_count()
        {
            DataTable sqlstr = db.table("select count(id) as Pid from tbl_patient  where Profile_Status='Active' order by CAST(Visited AS DATE) DESC,pt_id DESC ");
            return sqlstr;
        }
        public DataTable all_Patients_search(string pt_id)
        {
            //DataTable sqlstr = db.table("select  id as Pid,pt_id As 'Id'  ,pt_name  as 'Patient Name',gender as Gender,age as Age ,age2,days, primary_mobile_number  as   Mobile,street_address as 'Street Address',locality  as Locality,DATE_FORMAT(Visited,'%d/%m/%y') as Visited,Opticket as 'File NO' from tbl_patient  where Profile_Status='Active' order by CAST(Visited AS DATE) DESC,pt_id DESC ");
            DataTable sqlstr = db.table("select  id as Pid,pt_id As 'Id'  ,pt_name  as 'Patient Name',gender as Gender,age as Age ,age2,days, primary_mobile_number  as   Mobile,street_address as 'Street Address',locality  as Locality,DATE_FORMAT(Visited,'%d/%m/%y') as Visited,Opticket as 'File NO' from tbl_patient  where   pt_id like '%" + pt_id + "%' or pt_name like '%" + pt_id + "%' or primary_mobile_number like '%" + pt_id + "%' or street_address like '%" + pt_id + "%'   and    Profile_Status='Active' order by CAST(Visited AS DATE) DESC,pt_id DESC ");
            return sqlstr;
        }
        public DataTable patient_showmore(int count)
        {
            DataTable sqlstr = db.table("select  id as Pid,pt_id As 'Id'  ,pt_name  as 'Patient Name',gender as Gender,age as Age ,age2,days, primary_mobile_number  as   Mobile,street_address as 'Street Address',locality  as Locality,DATE_FORMAT(Visited,'%d/%m/%y') as Visited,Opticket as 'File NO' from tbl_patient  where Profile_Status='Active' order by CAST(Visited AS DATE) DESC,pt_id DESC  limit " + count + " ");
            return sqlstr;

        }
        public DataTable get_few_patients()
        {
            DataTable sqlstr = db.table("select  id as Pid,pt_id As 'Id'  ,pt_name  as 'Patient Name',gender as Gender,age as Age,age2,days , primary_mobile_number  as   Mobile,street_address as 'Street Address',locality  as Locality,DATE_FORMAT(Visited,'%d/%m/%y') as Visited,Opticket as 'File NO' from tbl_patient  where Profile_Status='Active' order by CAST(Visited AS DATE) DESC,pt_id DESC limit 300");
            return sqlstr;
        }
        ...
        public DataTable get_lab_notify()
        {
            DataTable dtl = db.table("select DISTINCT tl.pt_id  from tbl_lab_main tl inner join tbl_patient p on p.id=tl.pt_id where  status<>'completed'  ");
            return dtl;
        }
...
",tbl_appointment.start_datetime,tbl_appointment.status,tbl_appointment.schedule,tbl_appointment.waiting,tbl_appointment.engaged,tbl_appointment.checkout,tbl_appointment.id as a_id,tbl_appointment.cancelation_reason,tbl_doctor.doctor_name FROM  tbl_appointment LEFT JOIN tbl_patient on tbl_appointment.pt_id=tbl_patient.ID LEFT JOIN tbl_doctor ON tbl_appointment.dr_id=tbl_doctor.id WHERE  tbl_appointment.start_datetime between  '" + startDateTime.ToString("yyyy-MM-dd HH:mm") + "' AND '" + startDateTime1.ToString("yyyy-MM-dd HH:mm") + "' and tbl_patient. Profile_Status='Active' order by tbl_appointment.id DESC");
        ...
        {
            DataTable rs_log = db.table("select  l.log_id ,l.log_user_id ,l.log_type ,d.doctor_name,l.log_description,DATE_FORMAT(l.date,'%d/%m/%Y') as'Date',l.Time ,l.log_stage  from tbl_log l inner join tbl_doctor d on l.log_user_id=d.id ORDER BY log_id limit 250");
            return rs_log;
        }
        public DataTable log_action()
        {
            DataTable rs_log = db.table("select distinct log_stage  from tbl_log ORDER BY log_stage limit 250");
            return rs_log;
        }
        public DataTable log_details_count(string count)
        {
            DataTable rs_log = db.table("select  l.log_id ,l.log_user_id ,l.log_type ,d.doctor_name,l.log_description,l.date ,l.Time ,l.log_stage  from tbl_log l inner join tbl_doctor d on l.log_user_id=d.id group by log_id order by log_id desc limit " + count + "");
            return rs_log;
        }
        public DataTable log_search(string name)
        {
            DataTable rs_log = db.table("select  l.log_id ,l.log_user_id ,l.log_type,d.doctor_name,l.log_description,l.date ,l.Time ,l.log_stage from tbl_log l inner join tbl_doctor d on l.log_user_id=d.id where d.doctor_name like'%" + name + "%'  ORDER BY l.log_id");
            return rs_log;
        }
        public DataTable log_search_wit_action(string name, string action)
        {
            DataTable rs_log = db.table("select  l.log_id ,l.log_user_id ,l.log_type,d.doctor_name,l.log_description,l.date ,l.Time ,l.log_stage  from tbl_log l inner join tbl_doctor d on l.log_user_id=d.id  where d.doctor_name like '%" + name + "%' and l.log_stage='" + action + "'  ORDER BY log_id");
            return rs_log;
        }
        public DataTable log_search_action(string name)
        {
            DataTable rs_log = db.table("select  l.log_id ,l.log_user_id ,l.log_type ,d.doctor_name,l.log_description,l.date ,l.Time ,l.log_stage  from tbl_log l inner join tbl_doctor d on l.log_user_id=d.id  where l.log_stage like '%" + name + "%'  ORDER BY l.log_id");
            return rs_log;
        }
        ...
        {
            DataTable dtb = db.table("SELECT  DISTINCT p.id AS Pid,P.pt_id as 'Id',P.pt_name as 'Patient Name', P.gender as Gender,P.age as Age,p.age2,p.days, P. primary_mobile_number as Mobile,P.street_address as 'Street Address',P.locality as Locality,DATE_FORMAT(Visited,'%d/%m/%y') as Visited,P.Opticket as 'File No' FROM  tbl_appointment A LEFT JOIN tbl_patient P on  A.pt_id=P.ID  where start_datetime between '" + d.ToString("yyyy-MM-dd HH:mm") + "' and '" + todate.ToString("yyyy-MM-dd HH:mm") + "' and A.status<>'CANCELLED' and A.status <>'scheduled' ");
            return dtb;
        }
        ...
        {
            DataTable dtb = db.table("SELECT  DISTINCT A.pt_id AS Pid,P.pt_id as 'Id',P.pt_name as 'Patient Name', P.gender as Gender,P.age as Age,p. age2,p.days,P. primary_mobile_number as Mobile,P.street_address as 'Street Address',P.locality as Locality,DATE_FORMAT(Visited,'%d/%m/%y') as Visited,P.Opticket as 'File No' FROM  tbl_appointment A LEFT JOIN tbl_patient P on A.pt_id=P.ID WHERE  start_datetime >=  '" + startDateTime.ToString("yyyy-MM-dd") + "' and p.Profile_Status='Active' and  status !='Cancelled'");//  order by id desc";
            return dtb;
        }
        ...
        {
            DataTable dtb = db.table("select p.id as Pid,P.pt_id as 'Id',P.pt_name as 'Patient Name',P.gender as Gender,P.age as Age,p. age2,p.days,p.primary_mobile_number as Mobile,p.street_address as 'Street Address',p.locality as Locality,DATE_FORMAT(Visited,'%d/%m/%y') as Visited,p.Opticket as 'File No' from  tbl_appointment A left join tbl_patient P on A.pt_id=P.id  where status='Cancelled' order by p.id");
            return dtb;
        }
        public DataTable innactive_patients()
        {
            DataTable dtb = db.table(" SELECT P.id as Pid,P.pt_id as 'Id',P.pt_name as 'Patient Name', P.gender as Gender,P.age as Age,age2,days FROM tbl_patient P where  Profile_Status ='Cancelled' ORDER BY P.id DESC");            //}
            return dtb;
        ...
        {
            DataTable dtb = db.table(" SELECT P.id AS Pid,P.pt_id AS 'Id',p.pt_name AS 'Patient Name',p. gender AS Gender,p.age AS Age,p. age2,p.days,p. primary_mobile_number AS  Mobile,p.street_address AS 'Street Address',p.locality AS Locality,DATE_FORMAT(p.Visited,'%d/%m/%y') as Visited,p.Opticket as 'File No' FROM tbl_patient P inner join tbl_pt_group G on P.id = G.pt_id where G.group_id='" + id4 + "' and Profile_Status='Active' ORDER BY P.id DESC");
            return dtb;
        }
        ...
        {
            DataTable dtb = db.table("SELECT id Pid, pt_id 'Id',pt_name 'Patient Name', gender Gender,age Age, age2,days, primary_mobile_number Mobile,street_address 'Street Address',locality Locality,DATE_FORMAT(Visited,'%d/%m/%y') as Visited, Opticket as 'File No',Room_no,Profile_Status FROM tbl_patient  WHERE Profile_Status='Active' and  pt_id like '" + name + "%' or pt_name LIKE '%" + name + "%' or primary_mobile_number like '" + name + "%'  ORDER BY CASE WHEN pt_name = '" + name + "' THEN 0 WHEN  pt_name LIKE '" + name + "%' THEN 1 WHEN pt_name LIKE '%" + name + "%' THEN 2 WHEN pt_name LIKE '%" + name + "' THEN 3 ELSE 4 END , pt_name ASC ");
            //DataTable dtb = db.table("SELECT id Pid, pt_id 'Id',pt_name 'Patient Name', gender Gender,age Age, age2,days, primary_mobile_number Mobile,street_address 'Street Address',locality Locality,DATE_FORMAT(Visited,'%d/%m/%y') as Visited, Opticket as 'File No' FROM tbl_patient where (pt_id like '%" + name + "%' or pt_name like '%" + name + "%' or primary_mobile_number like '" + name + "%' or email_address like '" + name + "%' or Opticket like '" + name + "%' or street_address like '" + name + "%') and  Profile_Status='Active' ");
            return dtb;
        }
        public DataTable search_count(string name)
        {
            DataTable dtb = db.table("SELECT count(id) Pid FROM tbl_patient  WHERE pt_name LIKE '%" + name + "%' ");
            return dtb;
        }
        public DataTable recently_visited_search(DateTime d, DateTime todate, string name)
        {
            DataTable dtb = db.table("SELECT  DISTINCT A.id AS Pid,P.pt_id 'Id',P.pt_name 'Patient Name', P.gender Gender,P.age Age,p. age2,p.days,P. primary_mobile_number Mobile ,P.street_address 'Street Address',P.locality Locality,P.Opticket as 'File No' FROM  tbl_appointment A LEFT JOIN tbl_patient P on  A.pt_id=P.ID  where start_datetime between '" + d + "' and '" + todate + "' and A.status<>'CANCELLED' and A.status <>'scheduled' and (P.pt_id like '%" + name + "%' or P.pt_name like '%" + name + "%' or primary_mobile_number like '%" + name + "%' or email_address like '%" + name + "%' or Opticket like '%" + name + "%' or street_address like '%" + name + "%') ");
            return dtb;
        }
        ...
        {
            DataTable dtb = db.table("SELECT  DISTINCT P.id AS Pid, P.pt_id AS 'Id', P.pt_name as 'Patient Name', P.gender as Gender, P.age as Age, p.age2,p.days, P.primary_mobile_number as Mobile, P.street_address as 'Street Address', P.locality as Locality, P.Opticket as 'File No' FROM  tbl_appointment A LEFT JOIN tbl_patient P on A.pt_id = P.ID WHERE(A.pt_id like '%" + name + "%' or P.pt_name like '%" + name + "%' or primary_mobile_number like '%" + name + "%' or email_address like '%" + name + "%' or Opticket like '%" + name + "%' or street_address like '%" + name + "%') and start_datetime >= '" + startDateTime.ToString("yyyy-MM-dd") + "' and p.Profile_Status = 'Active'");
            return dtb;
        }
        ...
        {
            DataTable dtb = db.table("select p.id as Pid,P.pt_id as 'Id',P.pt_name as 'Patient Name',P.gender as Gender,P.age as Age, p.age2,p.days,p.primary_mobile_number as Mobile,p.street_address as 'Street Address',p.locality as Locality,p.Opticket as 'File No' from tbl_appointment A left join tbl_patient P on A.pt_id=P.id  where status='Cancelled' and (A.pt_id like '%" + name + "%' or A.pt_name like '%" + name + "%' or primary_mobile_number like '%" + name + "%' or email_address like '%" + name + "%' or Opticket like '%" + name + "%' or street_address like '%" + name + "%') order by p.id DESC");
            return dtb;
        }
        ...
        {
            DataTable dtb = db.table(" SELECT p.id as Pid,P.pt_id as 'Id',P.pt_name as 'Patient Name',P.gender as Gender,P.age as Age, p.age2,p.days ,p.primary_mobile_number as Mobile,p.street_address as 'Street Address',p.locality as 'Locality',p.Opticket as 'File No' FROM tbl_patient P inner join tbl_pt_group G on P.id = G.pt_id where G.group_id='" + id4 + "' and Profile_Status='Active' and (P.pt_id like '%" + name + "%' or P.pt_name like '%" + name + "%' or primary_mobile_number like '%" + name + "%' ) ORDER BY P.id DESC");//or email_address like '%" + name + "%' or Opticket like '%" + name + "%' or street_address like '%" + name + "%'
            return dtb;
        }
        ...
        public DataTable log_date(string id)
        {
            DataTable rs_log = db.table("select  l.log_id ,l.date ,l.Time ,l.log_stage  from tbl_log l where log_id='" + id + "' ");
            return rs_log;
        }

        /////visa expiry 

        public DataTable visa_details()
        {
            DataTable rs_log = db.table("select id,pt_name,pt_id,primary_mobile_number, latest_visa_expiry_date from tbl_patient  where latest_visa_expiry_date BETWEEN CURDATE() and DATE_ADD(CURDATE(), INTERVAL 12 DAY) ");
            return rs_log;
        }

        //bhj
        public DataTable pur_log_detail(string id)
        {
            DataTable rs_log = db.table("select l.log_id,l.log_user_id,l.log_type,l.log_type_id,d.doctor_name from tbl_log l inner join tbl_doctor d  on l.log_user_id=d.id where l.log_id='" + id + "' ");
            return rs_log;
        }
        //bhj
        public DataTable Get_all_dischargePatients()
        {
            DataTable dt = db.table("select  p.id as Pid,p.pt_id As 'Id'  ,p.pt_name  as 'Patient Name',p.gender as Gender,p.age as Age ,p.age2,p.days, p.primary_mobile_number  as   Mobile,p.street_address as 'Street Address',p.locality  as Locality,DATE_FORMAT(p.Visited,'%d/%m/%y') as Visited,p.Opticket as 'File NO',p.Profile_Status,p.Room_no from tbl_patient p inner join tbl_ippatient ip on p.id=ip.pt_id where ip.discharge_date<>'' and p.Profile_Status='Active' order by CAST(Visited AS DATE) DESC,p.pt_id DESC");

            // DataTable dt = db.table("select ip.id,ip.pt_id,ip.patient_id,ip.ip_id,p.id,p.pt_name,p.gender,p.age,p.primary_mobile_number,p.street_address,p.locality, from tbl_ippatient ip inner join tbl_patient p on p.id=ip.pt_id ");
            return dt;
        }

        public DataTable Get_all_ipPatients()
        {
            DataTable dt = db.table("select  p.id as Pid,p.pt_id As 'Id'  ,p.pt_name  as 'Patient Name',p.gender as Gender,p.age as Age ,p.age2,p.days, p.primary_mobile_number  as   Mobile,p.street_address as 'Street Address',p.locality  as Locality,DATE_FORMAT(p.Visited,'%d/%m/%y') as Visited,p.Opticket as 'File NO',Profile_Status,Room_no from tbl_patient p inner join tbl_ippatient ip on p.id=ip.pt_id where p.Profile_Status='Active' order by CAST(Visited AS DATE) DESC,p.pt_id DESC");
