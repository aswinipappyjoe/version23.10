...
namespace PappyjoeMVC.Model
{
    public class purchase_order_model
    {
        ...
        {
            db.trans_execute("insert into tbl_Purchase_order_master (Pur_order_no,Purch_order_date,Suppleir_id,status) values('" + OrerNo + "','" + OrderDate + "','" + Supplierid + "','O')", con, trans);
        }
        ...
        {
            db.trans_execute("insert into tbl_PurchaseOrder (Pur_order_no,Item_code,Description,Qty,UnitCost,Amount) values('" + OrerNo + "','" + Item_Id + "','" + description + "','" + col_qty + "','" + Unit_Cost + "','" + Amount + "')", con, trans);
        }
        
        public void delete_order(string orderno, MySqlConnection con, MySqlTransaction trans)
        {
            db.trans_execute("delete from tbl_Purchase_order_master where Pur_order_no='" + orderno + "'", con, trans);
            db.trans_execute("delete from tbl_PurchaseOrder where Pur_order_no='" + orderno + "'", con, trans);
        }
        //purchase order list
