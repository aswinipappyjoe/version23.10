...
namespace PappyjoeMVC.Model
{
    public class Consultation_model
    {
        ...
        {
            DataTable dt4 = db.trans_table("select id,inventory_id from tbl_adddrug where id='" + drugid + "' and inventory_id<>0 ORDER BY id DESC",con,trans);
            return dt4;
        }
        ...
        {
            db.trans_execute("insert into tbl_prescription_main (pt_id,dr_id,date,pay_status,notes) values('" + patient_id + "','" + d_id + "','" + DateTime.Now.Date.ToString("yyyy-MM-dd") + "','" + Prescription_bill_status + "','" + notes + "')", con, trans);
        }
        ...
        {
            db.trans_execute("insert into tbl_completed_id (completed_date,patient_id,review) values('" + DateTime.Now.Date.ToString("yyyy-MM-dd") + "','" + patient_id + "','NO')", con, trans);
        }
        ...
        {
            string dt_CMain = db.trans_scalar("select MAX(id) from tbl_completed_id",con,trans);
            return dt_CMain;
        }
        ...
        {
            int d = db.trans_execute("insert into tbl_completed_procedures (plan_main_id,pt_id,procedure_id,procedure_name,quantity,cost,discount_type,discount,total,discount_inrs,note,status,date,dr_id,completed_id,tooth) values('" + plan_main_id + "','" + pt_id + "','" + procedure_id + "','" + procedure_name + "','1','" + cost + "','INR','0','" + total + "','0','" + note + "','0','" + DateTime.Now.Date.ToString("yyyy-MM-dd") + "','" + dr_id + "','0','')",con,trans);
        }
        public string max_completeProcedure(MySqlConnection con, MySqlTransaction trans)
        {
            string dt_Compl_proce = db.trans_scalar("select MAX(id) from tbl_completed_procedures",con,trans);
        ...
        {
            db.trans_execute("insert into tbl_invoices_main (date,pt_id,pt_name,invoice,status,type) values('" + DateTime.Now.ToString("yyyy-MM-dd") + "','" + patient_id + "','" + pt_name + "','" + invoice + "','0','service')",con,trans);
        }
        ...
        {
            DataTable dt = db.table("SELECT Patientid,patient_name,payment_date,doctorname,fee,visited_date FROM tbl_patient_followup WHERE Patientid ='" + id + "' order by visited_date desc limit 1");
            return dt;
        }
        ...
        public void save_followup(string Patientid, string patient_name, string visited_date, string payment_date,string doctor, string fee,string payment_status,MySqlConnection con,MySqlTransaction tran)
        {
            db.trans_execute("insert into tbl_patient_followup (Patientid,patient_name,visited_date,payment_date,doctorname,fee,payment_status )values('" + Patientid + "','" + patient_name + "','" + visited_date + "','" + payment_date + "','" + doctor + "','" + fee + "','"+ payment_status + "')",con,tran);
        }
        public string get_consultation()
        {
            string str = db.scalar("SELECT name FROM tbl_addproceduresettings WHERE name='consultation'");
            return str;
        }
