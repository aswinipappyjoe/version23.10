...
namespace PappyjoeMVC.Model
{
    public class Show_Appointment_model
    {
        ...
        public DataTable show(string patid)
        {
            ...
            ",tbl_appointment.start_datetime,tbl_appointment.status,tbl_appointment.schedule,tbl_appointment.waiting,tbl_appointment.engaged,tbl_appointment.checkout,tbl_appointment.id as a_id,tbl_appointment.cancelation_reason, tbl_doctor.doctor_name FROM  tbl_appointment LEFT JOIN tbl_patient on tbl_appointment.pt_id=tbl_patient.ID LEFT JOIN tbl_doctor ON tbl_appointment.dr_id=tbl_doctor.id  WHERE  tbl_appointment.pt_id= '" + patid + "' limit 50");
            return dt;
        }
        public DataTable show_count(string patid,int count)
        {
            DataTable dt = db.table("SELECT  tbl_patient.id,tbl_patient.pt_id,tbl_patient.pt_name, tbl_patient.gender,tbl_patient.age, tbl_patient.primary_mobile_number " +
            ",tbl_appointment.start_datetime,tbl_appointment.status,tbl_appointment.schedule,tbl_appointment.waiting,tbl_appointment.engaged,tbl_appointment.checkout,tbl_appointment.id as a_id,tbl_appointment.cancelation_reason, tbl_doctor.doctor_name FROM  tbl_appointment LEFT JOIN tbl_patient on tbl_appointment.pt_id=tbl_patient.ID LEFT JOIN tbl_doctor ON tbl_appointment.dr_id=tbl_doctor.id  WHERE  tbl_appointment.pt_id= '" + patid + "' limit " + count + "");
            return dt;
        }
