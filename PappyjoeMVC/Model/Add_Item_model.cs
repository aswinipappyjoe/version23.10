...
namespace PappyjoeMVC.Model
{
    public class Add_Item_model
    {
        ...
        {
            DataTable dt_Category = db.table("select * from tbl_supplier order by Supplier_Code");
            return dt_Category;
        }
        ...
        {
            db.execute("insert into tbl_drug_type (dr_type) values ('" + name + "')");
        }
        ...
        public int Save_data(string _itemname, string _itemcode, string _manufacture, string _category,string HSN_Number, string _location, string _packing, string _isbatch, string _Sales1, string _SalesMin, string _SalesMax, string _Purch_Rate, string _punit, string _sUnit, int _unitmf, string _Purch_Rate2, string _Sales2, string _SalesMin1, string _SalesMax1, string _isOneUnitOnly, string _ReorderQty, string _CostBase, string _istax, string _MinimumStock,string shelfno, string barcode,string op,string sup_code, string product_type, string gstvat,string op_date)
        {
           int i = db.execute("insert into tbl_ITEMS(item_name,item_code,manufacturer,Cat_Number,HSN_Number,Location,Packing,Dep_Number,ISBatch,IsExpDate,IsSerial,Sales_Rate,Sales_Rate_min,Sales_Rate_Max,Open_Cost,Open_Stock,Current_Stock,FreeStock,LOWEST_COST,PurchRateType,Purch_Rate,Sup_Code,Unit1,Unit2,UnitMF,Purch_Rate2,Sales_Rate2,Sales_Rate_min2,Sales_Rate_Max2,OneUnitOnly,ReorderQty,CostBase,PayAfterSale,ItemGroup,Taxable,MinimumStock,Shelf_No,Barcode,Product_type,GstVat,opening_stock_date)values('" + _itemname + "','" + _itemcode + "','" + _manufacture + "','" + _category + "','" + HSN_Number + "','" + _location + "','" + _packing + "','1','" + _isbatch + "','False','False','" + _Sales1 + "','" + _SalesMin + "','" + _SalesMax + "','0','"+op+"','0','0','0','0','" + _Purch_Rate + "','"+sup_code+"','" + _punit + "','" + _sUnit + "','" + _unitmf + "','" + _Purch_Rate2 + "','" + _Sales2 + "','" + _SalesMin1 + "','" + _SalesMax1 + "','" + _isOneUnitOnly + "','" + _ReorderQty + "','" + _CostBase + "','False','False','" + _istax + "','" + _MinimumStock + "','"+ shelfno + "','" + barcode + "','"+ product_type + "','"+ gstvat + "','" + op_date + "')");
            return i;
        }
        ...
        {
            int i = db.execute("update tbl_ITEMS set item_name='" + _itemname + "' ,item_code='" + _itemcode + "',manufacturer='" + _manufacture + "',Cat_Number='" + _category + "',Location='" + _location + "',Packing='" + _packing + "',Dep_Number='1',ISBatch='" + _isbatch + "',IsExpDate='False',IsSerial='True',Sales_Rate='" + _Sales1 + "',Sales_Rate_min='" + _SalesMin + "',Sales_Rate_Max='" + _SalesMax + "',Open_Cost='0',Open_Stock='" + open_stock + "',Current_Stock='0',FreeStock='0',LOWEST_COST='0',PurchRateType='0',Purch_Rate='" + _Purch_Rate + "',Sup_Code='" + sup_code + "',Unit1='" + _punit + "',Unit2='" + _sUnit + "',UnitMF='" + _unitmf + "',Purch_Rate2='" + _Purch_Rate2 + "',Sales_Rate2='" + _Sales2 + "',Sales_Rate_min2='" + _SalesMin1 + "',Sales_Rate_Max2='" + _SalesMax1 + "',OneUnitOnly='" + _isOneUnitOnly + "',ReorderQty='" + _ReorderQty + "',CostBase='" + _CostBase + "',PayAfterSale='False',ItemGroup='False',Taxable='" + _istax + "',MinimumStock='" + _MinimumStock + "',Shelf_No='" + shelfno + "',Barcode='" + barcode + "',Product_type='" + product_type + "',GstVat='" + gstvat + "', opening_stock_date= '" + op_date + "' where id='" + Item_Id + "'");
            return i;
        }
        public DataTable check_have_batch(string Item_Code, string BatchNumber)
        {
            DataTable dtb = db.table("select BatchNumber from tbl_BatchNumber where Item_Code='" + Item_Code + "' and BatchNumber='" + BatchNumber + "' ");
            return dtb;
        }
        ...
        public DataTable get_tbl_PURCHIT_details(string Item_Code)
        {
            DataTable dt_purch = db.table("select Item_Code from tbl_PURCHIT where Item_Code='" + Item_Code + "'");
            return dt_purch;
        }
        public DataTable get_batch_details(string itemid)
        {
            DataTable dtb_Avg = db.table("select * from tbl_batchnumber where Item_Code='" + itemid + "' and PurchNumber='0'");
            return dtb_Avg;
        }
