...
namespace PappyjoeMVC.Model
{
    public class Add_Labwork_model
    {
        ...
        {
            DataTable dt = db.table("SELECT id ,Name FROM lab_medi_test order by Name");
            return dt;
        }
        public DataTable lab_search(string temname)
        {
            DataTable dt = db.table("SELECT id as'id',TemplateName as 'package' FROM Lab_Medi_TemplateMain where TemplateName like '%"+ temname + "%'");
            return dt;
        }
        public DataTable lab_Test_search(string temname)
        {
            DataTable dt = db.table("SELECT id ,Name FROM lab_medi_test where Name like '%" + temname + "%'");
            return dt;
        }
        ...
        {
            int i = db.execute("INSERT INTO Lab_Medi_LabResult(Resultmain_id,id,maintest_id,test_id,pt_id,results,Units,Normalvalue,template_id,Template_Type)VALUES('" + rsltmid + "','" + id + "','" + maintstid + "','" + testid + "','" + Convert.ToInt32(patid).ToString() + "','','" + units + "','" + normvalue + "','"+ temp + "','" + template_type + "')"); 
            return i;
        }
        ...
        public DataTable testrslt(string id)
        {
            ...
            DataTable dt = db.table("SELECT MainTestId ,TestId,Units,NormalValue FROM Lab_Medi_Template A where A.id = '" + id + "'");
            return dt;
        }
        public DataTable  test_details(string id)
        {
            DataTable dt = db.table("SELECT a.Name,a.id,a.TestTypeID,a.NormalValueM,a.NormalValueF,b.Name as unit FROM Lab_Medi_Test A inner join lab_medi_unit b on b.id= a.UnitId  where a.id = '" + id + "'");
            return dt;
        }
        public DataTable dt_test(string id)
        {
            DataTable dt = db.table("SELECT Name,id,TestTypeID FROM Lab_Medi_Test A where id = '" + id + "'");
            return dt;
        }
        public DataTable dt_test_type(string id)
        {
            DataTable dt = db.table("select Name from tbl_Lab_Medi_TestType where  id= '" + id + "'");
            return dt;
        }
        public DataTable dt_maintest(string id)
        {
            DataTable dt = db.table("select Main_test,id  from tbl_Lab_Medi_MainTest where id='" + id + "'");
            return dt;
        }
    }
}
