...
namespace PappyjoeMVC.Model
{
    public class profile_Details_model
    {
        ...
        public void save_ippatient(string pt_id, string patient_id, string ip_id)
        {
            db.execute("insert into  tbl_ippatient (pt_id,patient_id,ip_id,date,discharge_date)values('" + pt_id + "','" + patient_id + "','" + ip_id + "','" + Convert.ToDateTime(DateTime.Now.Date).ToString("yyyy/MM/dd") + "','') ");

        }
        public int update_ipPatient(string patid,string ipid ,string id,string room)
        {
            int i = db.execute("update tbl_patient set   pt_id='" + ipid + "',op_id='"+patid+"', Room_no='" + room+"',date='"+ Convert.ToDateTime(DateTime.Now.Date).ToString("yyyy/MM/dd") + "' where id='" +id+"'");
            return i;
        }
