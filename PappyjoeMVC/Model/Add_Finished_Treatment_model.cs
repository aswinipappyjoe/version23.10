...
namespace PappyjoeMVC.Model
{
    public class Add_Finished_Treatment_model
    {
        ...
        {
            DataTable dt_pt = db.table("SELECT id,name,cost FROM tbl_addproceduresettings ORDER BY id limit 200");
            return dt_pt;
        }
        ...
        {
            DataTable dt_treatment = db.table("select procedure_name,quantity,cost,discount_type,discount,discount_inrs,note,tooth,total,ToNurse from tbl_treatment_plan where id ='" + plan_p_id + "'");
            return dt_treatment;
        }
        ...
        public DataTable get_completed_id(string patient_id, string date, MySqlConnection con, MySqlTransaction trans)
        {
            DataTable dt_pt = db.trans_table("SELECT id FROM tbl_completed_id where patient_id='" + patient_id + "' and completed_date='" + date + "' ORDER BY id",con,trans);
            return dt_pt;
        }
        ...
        {
            db.execute("insert into tbl_completed_procedures (plan_main_id,pt_id,procedure_id,procedure_name,quantity,cost,discount_type,discount,total,discount_inrs,note,status,date,dr_id,completed_id,tooth,ToNurse)" + " values('" + plan_main_id + "','" + patient_id + "','" + procedure_id + "','" + procedure_name + "','" + quantity + "','" + cost + "','" + discount_type + "','" + discount + "','" + total + "','" + discount_inrs + "','" + note + "','1','" + date + "','" + dr_id + "','" + completed_id + "','" + tooth + "','No')");//,ToNurse
        }
        ...
        public void save_completed_items_trans_stats0(int plan_main_id, string patient_id, string procedure_id, string procedure_name, string quantity, string cost, string discount_type, string discount, string total, string discount_inrs, string note, string date, string dr_id, string completed_id, string tooth, string nurse, MySqlConnection con, MySqlTransaction trans)
        {
        ...
        {
            db.trans_execute("update  tbl_treatment_plan set status='0' where id= '" + id + "'",con,trans);
        }
        ...
        public void update_review(string date, int j_Review, MySqlConnection con, MySqlTransaction trans)//trans
        {
            db.trans_execute("UPDATE tbl_completed_id SET review='YES',Review_date='" + date + "' WHERE id='" + j_Review + "'",con,trans);
        }
        ...
        {
            DataTable dtb = db.table("SELECT plan_main_id,completed_id FROM tbl_completed_procedures  where id='" + treatment_complete_id + "'");
            return dtb;
        }
